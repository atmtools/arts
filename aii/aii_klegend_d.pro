;;----------------------------------------------------------------------
;; Tried to change klegend so that it works with data coordinates.
;; S. Buehler 03.04.97
;; SAB 04.10.97 Added keyword thick that controls the thickness of the
;; line samples drawn in the legen.
...
; Examples:
;	The call:
;		klegend_d,['Plus sign','Asterisk','Period'],psym=[1,2,3]
;	produces:
;		-----------------
;		|		|
;		|  + Plus sign	|
...
; Usage:
;	klegend_d,items,linestyle=linestyle	; vertical legend at upper left
;	klegend_d,items,psym=psym			; ditto except using symbols
;	klegend_d,items,psym=psym,/horizontal	; horizontal format
;	klegend_d,items,psym=psym,box=0		; sans border
;	klegend_d,items,psym=psym,delimiter='='	; embed an '=' betw psym & text
;	klegend_d,items,psym=psym,margin=2		; 2-character margin
;	klegend_d,items,psym=psym,position=pos	; position of legend
;	klegend_d,items,psym=psym,number=2		; plot two symbols, not one
;	klegend_d,items,/fill,psym=[8,8,8],colors=[10,20,30]; 3 filled squares
; Inputs:
...
;	    c2_line = [0,2,1]
;	    klegend_d,c1_items,psym=c1_psym,corners=c1,box=0
;	    klegend_d,c2_items,line=c2_line,corners=c2,box=0,pos=[c1(2),c1(3)]
...
;	    klegend_d,['1','22','333','4444'],linestyle=indgen(4),corners=corners
...
;	    klegend_d,['1','22','333','4444'],linestyle=indgen(4),pos=pos
...
;	  28 Aug 92, FKK
;-
pro aii_klegend_d,help=help,items,linestyle=linestyle,psym=psym,vectorfont=vectorfont $
  ...
  ,fill=fill,usersym=usersym,corners=corners,thick=thick,font=font
;
;	=====>> HELP
;
on_error,2
if keyword_set(help) then begin & doc_library,'klegend' & return & endif
;
...
if n eq 0 then message,'No inputs!  For help, type klegend_d,/help.'
if ni eq 0 then begin
  items = replicate('',n)				; DEFAULT BLANK ARRAY
endif else begin
  szt = size(items)
  if (szt(szt(0)+1) ne 7) then message,'First parameter must be a string array.  For help, type klegend_d,/help.'
...
for i = 0,n-1 do begin
  ...
  if vectorfont(i) ne '' then begin
    ...
    xyouts,xp,yp,vectorfont(i),width=width,color=colors(i) $
      ,size=charsize,align=0.5,font=font
  endif else begin
    if symline and (linestyle(i) ge 0) then plots,xp,yp,color=colors(i) $
      ,linestyle=linestyle(i),psym=psym(i),symsize=charsize,thick=thick
  endelse
  if vertical then x = x + xt else x = max(xp)
  if symline then x = x + xspacing
  xyouts,x,y,delimiter,width=width,color=textcolors(i),size=charsize,font=font
  x = x + width
  if width gt 0 then x = x + 0.5*xspacing
  xyouts,x,y,items(i),width=width,color=textcolors(i),size=charsize,font=font
  x = x + width
...
if box then plots,[x,maxx,maxx,x,x],y + [0,0,ywidth,ywidth,0]
return
end
