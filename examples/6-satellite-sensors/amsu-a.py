...
CHANNELS = [
	...
	ChannelSpec(11, "f0 ± 0.3222 ± 0.048", F0_O2, 36e6, "QH", 0.40, (0.3222e9, 0.048e9)),
...
def lo_recipe(spec: ChannelSpec) -> str:
	if not spec.lo_offsets_hz:
		return "direct"

	return " -> ".join(f"split ±{offset / 1e9:.4f} GHz" for offset in spec.lo_offsets_hz)
...
def print_channel_tables(channel_specs, realized):
	spec_rows = []
	realized_rows = []

	for spec in channel_specs:
		channel_data = realized[spec.number]
		centers_ghz = ", ".join(f"{center / 1e9:.6f}" for center in channel_data["lobe_centers_hz"])
		spec_rows.append(
			[
				spec.number,
				spec.spec_text,
				f"{spec.bandwidth_hz / 1e6:.1f}",
				spec.polarization,
				f"{spec.nedt_k:.2f}",
				lo_recipe(spec),
			]
		...
		realized_rows.append(
			[
				spec.number,
				len(channel_data["lobe_centers_hz"]),
				centers_ghz,
				len(channel_data["freq_grid_hz"]),
				f"{channel_data['freq_grid_hz'][0] / 1e9:.6f}",
				f"{channel_data['freq_grid_hz'][-1] / 1e9:.6f}",
			]
		)

	print("\nAMSU-A-like channel sheet used in this example")
	print(format_table(
		["Ch", "Spec-sheet RF description", "BW [MHz]", "Pol", "NEΔT [K]", "LO recipe"],
		spec_rows,
	))

	print("\nRealized RF lobes after combining the LO offsets")
	print(format_table(
		["Ch", "Lobes", "Lobe centers [GHz]", "Internal pts", "f_min [GHz]", "f_max [GHz]"],
		realized_rows,
	))


def print_measurement_table(channel_specs, realized, manual_measurement, measurement_vec):
	rows = []
	for index, spec in enumerate(channel_specs):
		...
		rows.append(
			[
				spec.number,
				f"{channel_data['reference_hz'] / 1e9:.6f}",
				f"{manual_measurement[index]:.3f}",
				f"{measurement_vec[index]:.3f}",
				f"{1e3 * (measurement_vec[index] - manual_measurement[index]):.3f}",
			]
		)

	print("\nChannel-weighted results")
	print(format_table(
		["Ch", "Representative f [GHz]", "Manual weighted Tb [K]", "measurement_vec [K]", "Δ [mK]"],
		rows,
	))
...
print_channel_tables(CHANNELS, realized)
...
for spec in CHANNELS:
	channel_data = realized[spec.number]
	spectral_tb = compute_channel_spectral_rad(ws, pos, los, channel_data["freq_grid_hz"])
...
print_measurement_table(CHANNELS, realized, manual_measurement, measurement_vec)
...
ax_channels.plot([spec.number for spec in CHANNELS], measurement_vec, linewidth=1.0, alpha=0.8)
