...
\section[Particle size distributions]{Representation of the particle size distribution}
...
After setting $\alpha$ = 1, inserting \eqnref{eq:gamma_coeff} and
some simple algebra we obtain
\begin{equation}
  \label{eq:pnd_gamma}
  n^p(\imc, \Reff) = \frac{2}{\pi} \frac{\imc}{\rho \Reff^3}.
\end{equation}
Comparing \eqnref{eq:pnd_mono} and \eqref{eq:pnd_gamma}, we see
...
\secref{sec:clouds:ARTS_SSP_structure}. Using PyARTS one can calculate the size
...
The workspace methods related to cloud definition in ARTS are
implemented in the file \fileindex{m\_cloudbox.cc}.
Work space methods related to the optical properties of the clouds are
implemented in the file \fileindex{m\_optproperties.cc}. The coodinate system
...
First we have to define to cloudbox region, i.e. the region where
...
ParticleTypeAdd{
filename_scat_data="ssd_sphere_50um_p20.xml"
filename_pnd_field="pnd_sphere_50um_p20.xml"
}
\end{verbatim}
In the workspace method \wsmindex{ParticleTypeAdd} the single
scattering properties for one particle type are
read. The first keyword \artsstyle{filename\_scat\_data} must be set to the filename of a
datafile including scattering data (class \typeindex{SingleScatteringData})
in xml-format. The second keyword \artsstyle{filename\_pnd\_field} must
contain the filename of the corresponding particle number density
field in xml-format (class \typeindex{GriddedField3}). If the cloud is
composed of several different particle types
\wsmindex{ParticleTypeAdd} can be used repeatedly for all particle
types, e.g.: 
\begin{verbatim}
ParticleTypeAdd{
filename_scat_data="ssd_cylinder_30um_p30.xml"
filename_pnd_field="pnd_cylinder_30um_p30.xml"
}
\end{verbatim}
...
\charef{sec:scattering} and the Monte Carlo module described in
\charef{sec:montecarlo}.
