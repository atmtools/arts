...
\figref{fig:RT_theory_coordinates}.  The z-axis corresponds to the
...
\figref{fig:scattering:part_frame}. For symmetry reasons the single
...
\begin{figure}[htbp]
 \begin{center}
   ...
   \zlabel{fig:scattering:part_frame}  
 \end{center}
\end{figure}

\subsection{Scattering datafile structure}
\zlabel{sec:clouds:ARTS_SSP_structure}
...
\begin{table}
\zlabel{tab:scattering:datastructure}
...
\zlabel{sec:clouds:particle_types}
...
generic input \artsstyle{filename\_scat\_data} must be set to
the filename of a datafile including scattering data (class
\typeindex{SingleScatteringData}) in xml-format. The generic input
\artsstyle{filename\_pnd\_field} must contain the filename of the
