...
\section{Clear sky simulations with sun like sources}
\label{sec:rtetheory:ClearskyDirectSources}

In this section we describe the radiative transfer in clearsky with sun
like sources.
...
from the sun is attenuated by the atmosphere and scattered at the surface and
in the atmosphere (black dots) towards the receiver.

\begin{figure}[t]
 \begin{center}
   \includegraphics*[width=0.6\hsize]{star_reciever_geometry}
   \caption{Radiative transfer of direct radiation form a sun to
   a nadir pointing receiver outside of the atmosphere assuming single
   scattering.}
  \label{fig:rtetheory:star_receiver_geometry}  
 \end{center}
\end{figure}

 The sun is assumed to be far away from the planet or to be more precise
 \begin{equation}
  r_{\astrosun}\ll d_{\astrosun p}
 \end{equation}
 with $r_{\astrosun}$ the sun radius,
 and $d_{\astrosun p}$ the distance between sun and TOA of the planet.
 This means that the angular variability of the direction of the  radiation
 from the sun can be neglected (parallel beam or collimated  beam approximation)
 ...
 \end{equation}
 with $\Omega'$ the direction of the sun. We neglect polarization here and in the follwoing part of this section for
 the sake of simplicity.

The flux of the incoming radiation from the sun at the top of the atmosphere
$F_{\astrosun}$ is
\begin{align}
	F_{\astrosun} &=
...
\end{align}
with $I_{\astrosun}$ the sun spectral radiance, which is assumed
to be isotropic, and $\alpha$ the angular radius of the sun seen
...
single scattering of the incoming radiation from the sun needs to be considered.
...
atmosphere in the direction of the sun (yellow rays in Fig.~\ref{fig:rtetheory:star_receiver_geometry}).
 

As in section \ref{sec:rte_theory:ClearskyDirectSources} described the radius $r_{\astrosun}$ of
and distance $d_{\astrosun}$ to the sun like object must be known. The position of this object
