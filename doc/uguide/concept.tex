...
\item[Scattering] can be consider in seval manners. It can be included in
  transmission type calculations. For scattering of thermal emission two
  modules are at hand, DOIT (Section~\ref{sec:scattering}) and MC (\theory,
  Section~\ref{T-sec:montecarlo}). The \textindex{scattering} particles are
...
workspace functions and workspace variables, and are explained further below).
...
\wsaindex{abs\_mat\_per\_species\_agenda}. Several radiative transfer methods use this
...
\begin{code}
*-------------------------------------------------------------------*
Workspace variable = abs_mat_per_species_agenda
---------------------------------------------------------------------
...
\caption{Top: Built-in documentation for variable
  \builtindoc{abs\_mat\_per\_species\_agenda}, obtained by command
  `\shortcode{arts -d abs\_mat\_per\_species\_agenda}', or on page
  \url{http://www.sat.ltu.se/arts/docserver/agendas/abs_mat_per_species_agenda}. Bottom
...
\builtindoc{abs\_mat\_per\_species} in its list of output workspace
variables, at least one workspace method which generates
\builtindoc{abs\_mat\_per\_species} must be added to the agenda in the
controlfile.
...
distribution. \FIXME{Revise this section after general.arts has been changed.}
