...
\artsstyle{somefile.arts} is executed by running
\begin{quote}
\begin{verbatim}
  arts somefile.arts
\end{verbatim}
...
given in Figure \ref{fig:hello}. In this example, the variable \artsstyle{s} is
...
Workspace variables (such as the variable \artsstyle{s} in Figure
...
\artsstyle{StringCreate}, \artsstyle{VectorCreate}, and so on. The
idea is that each group can have its own create method. (Currently
implemented are only create methods for groups where it was actually
needed, but adding them for other groups is trivial and will be done
on demand.)
...
documentation also lists all methods that take \artsstyle{f\_grid} as
input and all methods that produce \artsstyle{f\_grid} as output.
...
*-------------------------------------------------------*
\end{lstlisting}
\caption{Built-in documentation for variable \artsstyle{f\_grid}, obtained by
  command `\artsstyle{arts -d f\_grid}', or on page
...
variables) in parentheses. (`\artsstyle{Print(s)}' prints the content of variable
\artsstyle{s}.)
...
that \artsstyle{v} and \artsstyle{filename} are generic input
arguments.
...
*---------------------------------------------------------*
\end{lstlisting}
\caption{Built-in documentation for method \artsstyle{WriteXML}, obtained by
  command `\artsstyle{arts -d WriteXML}', or on page
  \url{http://www.sat.ltu.se/arts/docserver/methods/WriteXML}.}
\label{fig:WriteXML}
\end{figure}

What this means is that \artsstyle{output\_file\_format} already automatically
exists as a variable, whereas \artsstyle{v} and \artsstyle{filename}
...
\artsstyle{output\_file\_format} can be used to change the format of all produced
files at the same time. However, the use of a specific variable in the
controlfile is not mandatory, so `\artsstyle{WriteXML( output\_file\_format, v,
  "test.xml")}', `\artsstyle{WriteXML( "ascii", v, "test.xml" )}', and
`\artsstyle{WriteXML( my\_format, v, "test.xml" )}' are all allowed. (But in the
last example the variable \artsstyle{my\_format} must have been defined before.)
...
\artsstyle{v} are `Any', which means that \artsstyle{v} can belong to any of the
known groups. The group for \artsstyle{filename} is `String', which means that a
string is expected here. Method arguments can be a literal, as in
`\artsstyle{WriteXML( "ascii", v, "test.xml" )}', or a variable, as in
`\artsstyle{WriteXML( "ascii", v, s )}', where in the latter case variable
\artsstyle{s} must be already defined.

The built-in documentation further states that argument
\artsstyle{filename} has a \textindex{default value} (in this case the
empty string). Because of this, the argument can actually be omitted,
so `\artsstyle{WriteXML( "ascii", v )}' will also work.
...
\caption{Top: Built-in documentation for variable
  \artsstyle{abs\_scalar\_gas\_agenda}, obtained by command
  `\artsstyle{arts -d abs\_scalar\_gas\_agenda}', or on page
...
\artsstyle{abs\_scalar\_gas} in its list of output workspace
variables, at least one workspace method which generates
\artsstyle{abs\_scalar\_gas} must be added to the agenda in the
controlfile.
...
\artsstyle{INCLUDE "general.arts"}.  This mechanism is used to
...
directory.  This can be set by the command line option \artsstyle{-I
  <includepath>}, or by the environment variable
\artsstyle{\textindex{ARTS\_INCLUDE\_PATH}}.  If none of these are set, ARTS will
...
file resides, and that the \artsstyle{arts} executable is in
your path. 
...
repository. If your ARTS installation is healthy, \artsstyle{make
