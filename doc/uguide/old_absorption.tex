...
and output workspace variables of \artsstyle{ absCalc }. This method
...
\artsstyle{ abs } and \artsstyle{ abs\_per\_tag }. The input workspace variables are~: 

\begin{description}
\item{\artsstyle{ tgs }~:} defining the available tag groups for the 
  calculation of the absorption coefficients;
\item{\artsstyle{ f\_mono }~:} the monochromatic frequency grid [Hz];
\item{\artsstyle{ p\_abs }~:} the pressure grid for the absorption coefficients [Pa]; 
\item{\artsstyle{ t\_abs }~:} temperature associated with the pressures
  in \artsstyle{ p\_abs } [K]; 
\item{\artsstyle{ n2\_abs }~:} the total nitrogen profile associated with
  the pressures in \artsstyle{ p\_abs } [-];
\item{\artsstyle{ h2o\_abs }~:} the total water profile associated with the
  pressures in \artsstyle{ p\_abs } [-];
\item{\artsstyle{ vmrs }~:} the VMRs (unit: absolute number) on the 
\artsstyle{ p\_abs } grid; 
\item{\artsstyle{ lines\_per\_tg }~:} a list of spectral line data for each tag;
\item{\artsstyle{ lineshape  }~:} lineshape specification: function, norm, cutoff;
\item{\artsstyle{ cont\_description\_names }~:} a list of
names of continuum models;  
\item{\artsstyle{ cont\_description\_parameters }~:} continuum model parameters. 
\end{description}
...
\artsstyle{ absCalc }. Any exception of this will lead to an error message.\\
...
\end{verbatim}
thus producing \artsstyle{ f\_mono } . The grid can be optionally saved into
a file through \artsstyle{ VectorWriteAscii } \artsstyle{ (f\_mono) }
...
\end{verbatim}
with output \artsstyle{ p\_abs }. Again, it can be optionally written into
a file with
\begin{verbatim}
...
\end{verbatim}
The next variable to be set is \artsstyle{ lines\_per\_tg } , done by the
method
\begin{verbatim}
...
and thus creating the variable \artsstyle{ lines } , used afterward as
input in 
\begin{verbatim}
lines_per_tgCreateFromLines{}
\end{verbatim}
in order to get finally \artsstyle{ lines\_per\_tg } defined. Here is the
...
\artsstyle{ lines } from them. The ARTS own catalog format is read by
\begin{verbatim}
linesReadfromArts {
   filename = "ozone.al"
   fmin     = 0
...
\end{verbatim} 
for MYTRAN.\\
The last input variable for \artsstyle{ absCalc } to be discussed is 
\artsstyle{ lineshape } . It is created either generally for all tag groups
through 
\begin{verbatim}
lineshapeDefine{
  shape = "Voigt_Kuntz6" 
...
\artsstyle{ absCalc }. Therefore this part of the subsection is a little bit short
