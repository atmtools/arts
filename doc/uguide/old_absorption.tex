...
%
\starthistory
  2001-11-21 & Restarted for ARTS-1-1 by Stefan Buehler.\\
...
\subsection{Line Shape Functions} Up to date there is no way to calculate
...
\subsection{Line Catalogs} There are several spectroscopic catalogs
...
\artsstyle{""}  for maximum flexibility.\\
...
Table \zref{table:impl_secies} lists the implemented species in ARTS.
...
Table \zref{table:isoratio} gives an overview of the isotopic ratios
...
As in \zref{table:impl_secies} the first two columns give the ARTS
...
Table \zref{table:part_fct} refers to the partition function data.  It
...
species are to be found in \artsstyle{/arts/src/partition\_function\_data.cc}.
...
and output workspace variables of \artsstyle{ abs\_coefCalc }. This method
...
\artsstyle{ abs\_coef } and \artsstyle{ abs\_per\_tag }. The input workspace variables are~: 

\begin{description}
\item{\artsstyle{ tgs }~:} defining the available tag groups for the 
  calculation of the absorption coefficients;
\item{\artsstyle{ f\_grid }~:} the monochromatic frequency grid [Hz];
\item{\artsstyle{ abs\_p }~:} the pressure grid for the absorption coefficients [Pa]; 
\item{\artsstyle{ abs\_t }~:} temperature associated with the pressures
  in \artsstyle{ abs\_p } [K]; 
\item{\artsstyle{ n2\_abs }~:} the total nitrogen profile associated with
  the pressures in \artsstyle{ abs\_p } [-];
\item{\artsstyle{ h2o\_abs }~:} the total water profile associated with the
  pressures in \artsstyle{ abs\_p } [-];
\item{\artsstyle{ vmrs }~:} the VMRs (unit: absolute number) on the 
\artsstyle{ abs\_p } grid; 
\item{\artsstyle{ abs\_lines\_per\_species }~:} a list of spectral line data for each tag;
\item{\artsstyle{ abs\_lineshape  }~:} lineshape specification: function, norm, cutoff;
\item{\artsstyle{ abs\_cont\_names }~:} a list of
names of continuum models;  
\item{\artsstyle{ abs\_cont\_parameters }~:} continuum model parameters. 
\end{description}
...
\artsstyle{ abs\_coefCalc }. Any exception of this will lead to an error message.\\
...
\end{verbatim}
which produces as output \artsstyle{ tgs }. In our case case this will
...
\end{verbatim}
thus producing \artsstyle{ f\_grid } . The grid can be optionally saved into
a file through \artsstyle{ VectorWriteAscii } \artsstyle{ (f\_grid) }
 \artsstyle{ {""} } . The  pressure grid  is created through the method 
\begin{verbatim}
VectorNLogSpace(abs_p){
        start = 100000
        stop  = 10
        n     = 140
}
\end{verbatim}
with output \artsstyle{ abs\_p }. Again, it can be optionally written into
a file with
\begin{verbatim}
VectorWriteAscii(abs_p){""} 
\end{verbatim}
...
workspace variables \artsstyle{ raw\_ptz } and \artsstyle{ raw\_vmrs }. The
...
scenarios, in this case the one \artsstyle { midlatitude-summer }.
...
abs_n2Set{}
\end{verbatim}
needed for the input variables \artsstyle{ h2o\_abs } and \artsstyle{ n2\_abs }.
...
\end{verbatim}
The next variable to be set is \artsstyle{ abs\_lines\_per\_species } , done by the
method
\begin{verbatim}
...
and thus creating the variable \artsstyle{ abs_lines } , used afterward as
input in 
\begin{verbatim}
abs_lines_per_speciesCreateFromLines{}
\end{verbatim}
in order to get finally \artsstyle{ abs\_lines\_per\_species } defined. Here is the
...
\artsstyle{ abs_lines } from them. The ARTS own catalog format is read by
\begin{verbatim}
...
\end{verbatim} 
for MYTRAN.\\
The last input variable for \artsstyle{ abs\_coefCalc } to be discussed is 
\artsstyle{ abs\_lineshape } . It is created either generally for all tag groups
through 
\begin{verbatim}
abs_lineshapeDefine{
  shape = "Voigt_Kuntz6" 
...
\end{verbatim}
More elucidation on the three parameters \artsstyle{ shape } ,
\artsstyle{ normalizationfactor } and \artsstyle{ cutoff }  is needed. The first
one accounts for the different lineshape models now available in
ARTS. It's done by setting \artsstyle{ shape } to a given model name
:
\begin{description} 
\item{\artsstyle{ Lorenz }~:} Lorenz line shape;
\item{\artsstyle{ Doppler }~:} Doppler line shape
\item{\artsstyle{ Voigt\_Kuntz6 }~:} Kuntz approximation to the Voigt line
  shape with relative accuracy better than $ 2 \times 10^{-6} $;
\item{\artsstyle{ Voigt\_Kuntz4 }~:} Kuntz approximation to the Voigt line
  shape with relative accuracy better than $ 2 \times 10^{-4} $;
\item{\artsstyle{ Voigt\_Kuntz3 }~:} Kuntz approximation to the Voigt line
  shape with relative accuracy better than $ 2 \times 10^{-3} $;
\item{\artsstyle{ Voigt\_Drayson }~:} Drayson approximation to the Voigt
  profile;
\item{\artsstyle{ Rosenkranz\_Voigt\_Kuntz6 }~:} Rosenkranz oxygen
  absorption including overlap correction, at high altitudes Kuntz
  routine, requires the ARTS catalog with auxiliary overlap
  parameters;
  profile;
\item{\artsstyle{ Rosenkranz\_Voigt\_Drayson }~:} Rosenkranz oxygen
  absorption including overlap correction, at high altitudes Drayson
  routine, requires the ARTS catalog with auxiliary overlap
  parameters
\end{description}
The \artsstyle{ normalizationfactor } sets the different normalization
factors of the line shape:
\begin{description} 
\item{\artsstyle{ no\_norm }~:} 1 (unity);
\item{\artsstyle{ linear }~:} $(\nu / \nu_0)$;
\item{\artsstyle{ quadratic }~:} $(\nu / \nu_0)^2$
\end{description}
The \artsstyle{ cutoff } can  be set to -1 for no cutoff or to a positive
number for a cutoff at a given frequency in~ [Hz], e.g. 650e9.

Thus we have generated all input workspace variables for 
\artsstyle{ abs\_coefCalc }. Therefore this part of the subsection is a little bit short
...
\begin{verbatim}
MatrixWriteAscii (abs_coef) {""}
\end{verbatim}
or
\begin{verbatim}
ArrayOfMatrixWriteAscii (abs_coef_per_species) {""}
\end{verbatim}
...
is already applied. The adjustment of Eq. \zref{eq:abs_cont} to the data 
is performed through the parameter set $\cso$, $\xs$, $\cdo$, and $\xf$. 
Table \zref{tab:wvcontparam} gives some commonly used continuum parameter sets.
\begin{table}[!hbt]
  \begin{center}
...
\end{center}
 \caption{Values of commonly used continuum parameter sets. The last line (MPM93$^*$)
   ...
   in the form of Eq. \zref{eq:abs_cont}.}
 \zlabel{tab:wvcontparam}
\end{table}

\subsubsection{The MPM93 Continuum Parameterization}
\zlabel{leveld:mpm93:contabs}
%-----------------------------------------------
...
\end{eqnarray}
%
Table~\zref{tab:mpm93_cont_param} lists the values of this continuum 
...
\zlabel{tab:mpm_psl_broad}
\end{center}
\end{table} 

This change of continuum parameterization makes it difficult 
to compare MPM93 with the models which use Eq. (\zref{eq:abs_cont}). 
...
shown for four different cases in Table\,\zref{tab:mpm_psl_broad}:
\begin{equation}
 \zlabel{eq:mpm93:contfapp1}
 \shapec \approx 2 \cdot \frac{\gamc}{\nucc^2}
\end{equation}
Inserting Eq. (\zref{eq:mpm93:contfapp1}) into Eq. (\zref{eq:mpm93:contabs})
gives a quadratic frequency dependence of the MPM93 continuum,
similar to the continuum parameterization expressed in 
Eq. (\zref{eq:abs_cont}). By additionally approximating 
...
\end{eqnarray}
%
one can rearrange the pseudo-line continuum to fit Eq. (\zref{eq:abs_cont})
(denoted by MPM93$^*$). The so deduced continuum parameter set is given in 
Table \zref{tab:wvcontparam}.\\
...
general way like
\begin{equation} 
 \zlabel{eq:o2_cont_gamma_general}
...
in Equation \zref{eq:abs_cont}:
\begin{equation}
  \zlabel{eq:abs_cont_pwr_n2}
...
Equations \zref{eq:abs_cont_MPM89_n2} and \zref{eq:abs_cont_MPM93_n2}
...
Equation \zref{eq:abs_cont_pwr_co2}. Because nitrogen is usually a more
...
\subsection{ARTS Workspace Variables and Methods}
\zlabel{levelc:ArtsImplementationContinuum}
% -------------------------------------------
...
% the arts control file. Table \zref{tab:artscontlist} gives a short summary 
...
tags. Why this is so can be seen from Eq. \zref{eq:abs_cont} and 
Table \zref{tab:wvcontparam}. For a single function for the water vapor 
...
variables {\it abs\_cont\_names, abs\_cont\_models, 
and abs\_cont\_parameters}) for the selected continuum tags, 
...
arts method {\it abs\_cont\_descriptionAppend}, which has itself 
...
valid range of the input variables for {\it abs\_cont\_descriptionAppend}. 
For a condensed overview of the possible continuum tags and their 
referenced models see Table \zref{tab:artscontlist} and the 
...
Below in the detailed description of {\it abs\_cont\_descriptionAppend} you 
can find correct examples for all the continuum tags.
 
\begin{itemize}
\item[$\bullet$] The general water vapor continuum function
as described in Eq. (\zref{eq:abs_cont}) is divided up into 
...
method {\it abs\_cont\_descriptionAppend}. The valid models are
...
{\it abs\_cont\_descriptionAppend} the value of the input 
...
\item[$\bullet$] The general water vapor continuum function
as described in Equation~(\zref{eq:abs_cont}) is divided up into 
two separate tags in arts. The term proportional 
...
function of Eq. \zref{eq:abs_cont} is therefore not valid. In MPM93 
...
in the arts method {\it abs\_cont\_descriptionAppend} are only 
...
\item[$\bullet$] The oxygen continuum models of Rosenkranz
  ...
  Section~\zref{levelc:o2cont}, are only slightly dif\-ferent in some
  respect so that
  ...
  (\zref{eq:abs_cont_pwr_o2}) and (\zref{eq:o2_cont_gamma_general}).
  The units of these parameters are: $[$1/(m$\cdot$Pa$\cdot$Hz)$]$,
  $[$Hz/Pa$]$, $[$1$]$, $[$1$]$, $[$1$]$, $[$1$]$. For a description
  of these parameters see Eqs. (\zref{eq:abs_cont_pwr_o2}),
  (\zref{eq:o2cont_const_vmr_1}), and (\zref{eq:o2_cont_gamma_general}).
...
\zref{levelc:n2cont} has the arts tag name {\tt "N2-SelfContStandardType"}. For this tag 
...
Eq. \zref{eq:abs_cont_MPM89_n2}. To be able to use this referenced model, one 
...
\item[$\bullet$] The \cite{pwr:93} $\coz$-$\coz$ continuum term is
  according to Equation~(\zref{eq:abs_cont_pwr_co2}) of
  Section~\zref{levelc:co2cont} proportional to $\pcoz^2$. This
...
\item[$\bullet$] The \cite{pwr:93} $\coz$-$\nz$ continuum term is
  according to as explained in Equation~(\zref{eq:abs_cont_pwr_co2}) of
  Section \zref{levelc:co2cont} proportional to $\pcoz \cdot \pnz$.
...
example for the arts method {\it abs\_lineshape\_per\_tgDefine} is presented 
...
% --------------------- only begin of table ------------------------------
 \hline
 continuum & \multicolumn{3}{c}{{\it abs\_cont\_descriptionAppend} input} & 
...
% --------------------- every page begin of table ------------------------
 \hline
 continuum & \multicolumn{3}{c}{{\it abs\_cont\_descriptionAppend}} & 
 reference/ & arts source code function\\
 & \multicolumn{3}{c}{input parameter} & arts uguide & \\
 \hline
 \endhead
...
% --------------------- only end of table ------------------------------
 ...
 \caption{This table gives an overview of the implemented referenced 
   ...
   {\it abs\_cont\_descriptionAppend}. Additionally the reference and the arts 
...
% --------------------------------------------------------------
 ...
 $\hzo-\hzo$    & model &=& "Rosenkranz" & {\small Sec. \zref{leveld:h2o_Cont}} & \\
                & userparameters&=&[ ]                                       & & \\

 Rosenkranz     & tagname &=& {\small \tt "H2O-ForeignContStandardType"}       & 
                  {\small \cite{pwr:98}}  & 
                  {\small Standard\_H2O\_foreign\_continuum}\\
 $\hzo-$dry air & model &=& "Rosenkranz" & {\small Sec. \zref{leveld:h2o_Cont}} & \\
                & userparameters &=& [ ]                                     & & \\
\hline
% --------------------------------------------------------------
 ...
 $\hzo-\hzo$    & model &=& "CruzPol" & {\small Sec. \zref{leveld:h2o_Cont}} & \\
                & userparameters &=& [ ]                                  & & \\

 Cruz-Pol       & tagname &=& {\small \tt "H2O-ForeignContStandardType"}    & 
                  {\small \cite{cruzpol:98}} & 
                  {\small  {\small Standard\_H2O\_foreign\_continuum}}\\
 $\hzo-$dry air & model &=& "CruzPol" &  {\small Sec. \zref{leveld:h2o_Cont}}&  \\
                & userparameters &=& [ ]                                &   &  \\
...
$\oz-$air   & model &=& "Rosenkranz" & {\small Sec. \zref{levelc:o2cont}} &  \\  
            & userparameters &=& [ ] &   & \\
% --------------------------------------------------------------
 MPM93      & tagname &=& {\small \tt "O2-SelfContStandardType"} & {\small \cite{liebeetal:93}} & 
              {\small Standard\_O2\_continuum}\\
$\oz-$air   & model &=& "MPM93"  & {\small Sec. \zref{levelc:o2cont}}  &  \\
            & userparameters &=& [ ] &   & \\
 \hline
...
$\nz-\nz$   & model &=& "Rosenkranz" & {\small Sec. \zref{levelc:n2cont}}  &  \\  
            & userparameters &=& [ ] &   & \\
% --------------------------------------------------------------
...
\zlabel{leveld:ArtsContExampleControlFile}
% -------------------------------------------
...
\begin{flushleft}
 \includegraphics[scale=0.65, angle=0]{abs_cont_page1}
\end{flushleft}
\begin{flushleft}
 \includegraphics[scale=0.65, angle=0]{abs_cont_page2}
\end{flushleft}
\begin{flushleft}
 \includegraphics[scale=0.65, angle=0]{abs_cont_page3}
\end{flushleft}
...
values are listed in Table \zref{tab:mpm87linelist}. To describe the line 
...
The line shape function, $\shape$, in Eq.~(\zref{eq:mpm87:absline}) 
...
values are listed in Table \zref{tab:mpm89linelist}. To describe the line 
...
\begin{equation}
  \zlabel{eq:mpm89:absline}
  \alphampmml = 0.1820 \cdot \nuk \cdot \phzo \cdot 
  \sum_{k}{\inten \cdot \shape}\hspace*{10mm}\mbox{dB/km}
\end{equation}
where $\inten$ is the line intensity described by the parameterization
\begin{equation}
  \zlabel{eq:mpm89:strength}
  \inten = \bek \cdot \phzo \cdot \Theta^{3.5} 
           \cdot \exp{(\bzk \cdot [1-\Theta])}\hspace*{10mm}\mbox{kHz}
\end{equation}
whit $\nuk$ as the line center frequency, $\phzo$ the water
vapor partial pressure and $\Theta = 300\,\mbox{K}/T$.\\
The line shape function, $\shape$, in Eq.~(\zref{eq:mpm89:absline}) 
is the standard Van Vleck-Weisskopf (VVW) function, given by 
\begin{eqnarray}
% Van Vleck-Weisskopf function
  \zlabel{eq:mpm89:VVW}
...
($\bdk$) is used (see Eq.~(\zref{eq:mpm87:gamma})).
%
...
\paragraph{Water Vapor Continuum Absorption:}
\zlabel{levele:mpm89_h2ocont}
%----------------------------------------
The MPM89 continuum absorption coefficients in, $\alphampmmc$, 
are identical as those in MPM87 (see Sec. \zref{levele:mpm87_h2ocont} for 
details):
\begin{equation} 
  \zlabel{eq:mpm89:cont}
  \alphampmmc = \nu^2 \cdot \phzo \cdot 
                (\cso \cdot \phzo \cdot \Theta^{\xs} + 
                 \cdo \cdot \pda  \cdot \Theta^{\xf}),
\end{equation}
...
\begin{equation}
  \zlabel{eq:mpm93:absline}
  \alphampmnl = 0.1820 \cdot \nuk \cdot \phzo \cdot 
  \sum_{k}{\inten \cdot \shape}\hspace*{10mm}\mbox{dB/km}
\end{equation}
where $\inten$ is the line intensity described by the parameterization
\begin{equation}
  \zlabel{eq:mpm93:strength}
  \inten = \bek \cdot \phzo \cdot \Theta^{3.5} 
           \cdot \exp{(\bzk \cdot [1-\Theta])}\hspace*{10mm}\mbox{kHz}
\end{equation}
with $\nuk$ as the line center frequency, $\phzo$ the water
...
\zlabel{levele:mpm93:h2ocont}
%-----------------------------------------------
In the MPM93 version the water vapor continuum is parameterized as an
ordinary spectral line (Eqs. (\zref{eq:mpm93:strength}, 
\zref{eq:mpm93:VVW})). The parameters of this continuum "pseudo-line" 
($\nu^*$, $\beks$, $\bzks$, $\bdks$, $\bvks$, $\bfks$, $\bsks$) 
are given in Table \zref{tab:mpm93linelist}. More details about 
this continuum parameterization and its microwave approximation can be 
found in Section \zref{leveld:h2o_Cont} of this guide.
...
\begin{equation}
 \zlabel{eq:cp98_cont_scaling}
 \alphacpc = C_C \cdot \alphampmoc 
\end{equation}
The scaling factor $C_C$, as given in Table \zref{tab:cp_orr}, 
gives a 23.69\,\% increased continuum absorption compared 
with MPM87 (see Table \zref{tab:wvcontparam} for a comparison of the 
...
\paragraph{Water Vapor Line Absorption:}
\zlabel{levele:pwr98_h2oline}
%------------------------------------
The local line absorption is defined as 
\begin{eqnarray} 
 \zlabel{eq:pwr98absline}
...
establishes a limit to the summation in Eq.~(\zref{eq:pwr98absline}) where lines
...
combination of Eq.~(\zref{eq:pwr98absline}) with the line shape function would 
be equivalent to a Van Vleck--Weisskopf \citep{vanvleck:45} line shape: 
\begin{equation}
 \zlabel{eq:pwr98lineshape}
 \hspace*{-8mm}\shapefpm = 
...
$\wf$, and $\xf$ are listed in Table \zref{tab:pwr98linelist} and the 
...
\zlabel{tab:pwr98linelist}
\end{table}



\paragraph{Water Vapor Continuum Absorption:}
\zlabel{levele:pwr98_h2ocont}
%-----------------------------------------
The continuum absorption in R98 has the same functional dependence on frequency,
pressure, and temperature like in MPM87/MPM89 (see Sec. \zref{levele:mpm87_h2ocont}
for details):
\begin{equation} 
  \zlabel{eq:pwr98:abscont}
  \alphapwrc = \nu^2 \cdot \phzo \cdot 
               (\cso \cdot \phzo \cdot \Theta^{\xs} + 
                \cdo \cdot \pda  \cdot \Theta^{\xf})
\end{equation}
...
\zlabel{leveld:O2_pwr98}
%-------------------------------------


\paragraph{Resonant Oxygen Absorption}
%\zlabel{levele:02_pwr98_line}
\zlabel{levele:pwr93_o2lines}
%----------------------------------
...
tabulated in Table \zref{tab:pwr02line}.
%
%\setlength{\LTcapwidth}{200mm} % with of the caption in longtable
...
\begin{eqnarray}
  \zlabel{eq:pwr_o2cont}
  \alphac &=&  C \cdot \pda \cdot \Theta^2 \cdot 
             \frac{\nu^2 \cdot \gamma}{\nu^2+\gamma^2}\\
%
  \zlabel{eq:pwr_o2cont_1}
  \gamma &=&  w \cdot (\pda \cdot \Theta^{0.8} + 1.1 \cdot \phzo \cdot
  \Theta)
\end{eqnarray}
...
\zlabel{levelb:O2_mpm93}
%-------------------------------------

\paragraph{Oxygen Line Absorption:}
\zlabel{levele:mpm93_o2lines}
%-------------------------------
...
Table \zref{tab:mpm9302line}. One has to note that in the MPM93 code is a 
threshold value for $\alphampml$ implemented:
\begin{equation}
 \zlabel{eq:mpm93O2limit}
  \alphampml = 
   \left \{ \begin{array}{r@{\quad:\quad}l} 
...
\begin{eqnarray}
  \zlabel{eq:mpm93_o2cont}
  \alphac &=&  C \cdot \pda \cdot \Theta^2 \cdot 
               \frac{\nu^2 \cdot \gamma}{\nu^2+\gamma^2}\\
%
  \zlabel{eq:mpm93_o2cont_1}
...
\subsection{ARTS Workspace Variables and Methods}
\zlabel{levelc:ArtsImplementationCompleteModels}
% ---------------------------------------------
...
valid range of the input variables for {\it abs\_cont\_descriptionAppend}. 
For a condensed overview of the possible continuum tags and their 
referenced models see Table \zref{tab:artsfullmodlist} and the 
...
\item[$\bullet$] The water vapor model of MPM87 \citep{liebeandlayton:87} 
     ...
     vapor absorption model are described in Section \zref{leveld:mpm87}. 
     ...
     Each line intensity $\inten$ (see Eq. (\zref{eq:mpm87:strength})) 
     is multiplied with the scaling factor $CL$, while $CW$ scales 
     each line width, $\gamk$, (see Eq. (\zref{eq:mpm87:gamma})). 
     The continuum absorption, $\alphampmoc$, 
     (see Eq. (\zref{eq:mpm87:cont})) also scales with $CC$.\\
     In the following all the valid possibilities for the
...
\item[$\bullet$] The full water vapor absorption model MPM89 \cite{liebe:89} 
     ...
     $CC$, $CL$, and $CW$, with the input variable {\it userparameters}, 
     {\it userparameters}\,=\,$[$$CC$, $CL$, $CW$$]$. 
     Each line intensity $\inten$ (see Eq. (\zref{eq:mpm89:strength})) 
     is multiplied with the scaling factor $CL$, while $CW$ scales 
     each line width, $\gamk$, (see Eq. (\zref{eq:mpm89:gamma})). 
     The continuum absorption, $\alphampmmc$, 
     (see Eq. (\zref{eq:mpm89:cont})) also scales with $CC$.\\
     In the following all the valid possibilities for the
...
\item[$\bullet$] The water vapor model of MPM93 \cite{liebeetal:93}
     ...
     absorption model are described in Section \zref{leveld:mpm93}.
     ...
     $CC$, $CL$, and $CW$, with the input variable {\it userparameters}, 
     {\it userparameters}\,=\,$[$$CC$, $CL$, $CW$$]$. 
     Each line intensity $\inten$ (see Eq. (\zref{eq:mpm93:strength})) 
     is multiplied with the scaling factor $CL$, while $CW$ scales 
     each line width, $\gamk$, (see Eq. (\zref{eq:mpm93:gamma})). 
     The continuum absorption, $\alphampmnc$, 
     (see Eq. (\zref{eq:mpm93:strength})) also scales with $CC$.\\
     In the following all the valid possibilities for the
...
\item[$\bullet$] The water vapor model of CP98 \citep{cruzpol:98}
     ...
     vapor absorption model are described in Section \zref{leveld:cp98}. 
     ...
     $CC$, $CL$, and $CW$, with the input variable {\it userparameters}, 
     {\it userparameters}\,=\,$[$$CC$, $CL$, $CW$$]$. 
     Each line intensity $\inten$ (see Eq. (\zref{eq:cp98:inten})) 
     is multiplied with the scaling factor $CL$, while $CW$ scales 
     each line width, $\gamk$, (see Eq. (\zref{eq:cp98:width})). 
     The continuum absorption, $\alphacpc$, 
     (see Eq. (\zref{eq:cp98_cont_scaling})) also scales with $CC$.\\
     In the following all the valid possibilities for the
...
\item[$\bullet$] The water vapor model of PWR98 \citep{pwr:98}
     ...
     vapor absorption model are described in Section \zref{leveld:pwr98_h2o}. 
     The standard way to use the full (=continuum+lines) CP98 water 
     vapor absorption model is to set the input variable {\it model} 
     ...
     $CC$, $CL$, and $CW$, with the input variable {\it userparameters}, 
     {\it userparameters}\,=\,$[$$CC$, $CL$, $CW$$]$. 
     Each line intensity $\inten$ (see Eq. (\zref{eq:pwr98absline})) 
     is multiplied with the scaling factor $CL$, while $CW$ scales 
     each line width, $\gamk$, (see Eq. (\zref{eq:pwr98gamma})). 
     The continuum absorption, $\alphapwrc$, 
     (see Eq. (\zref{eq:pwr98:abscont})) also scales with $CC$.\\
     In the following all the valid possibilities for the
...
\item[$\bullet$] The MPM93 full absorption model for oxygen \citep{liebeetal:93}
     ...
     oxygen absorption model are described in Section \zref{levelb:O2_mpm93}. 
     ...
     Each line intensity $\inten$ (see Eq. (\zref{eq:MPM93:O2_inten})) 
     is multiplied with the scaling factor $CL$, while $CW$ scales 
     each line width, $\gamk$, (see Eq. (\zref{eq:MPM93:O2_gamma})) and 
     $CO$ the line coupling parameter (see Eq. (\zref{eq:MPM93:O2_coupling})). 
     The continuum absorption, (see Eq. (\zref{eq:mpm93_o2cont})) 
     also scales with $CC$.\\
     In the following all the valid possibilities for the
...
\item[$\bullet$] The PWR93 full absorption model for oxygen \citep{pwr:93}
     ...
     oxygen absorption model are described in Section \zref{levele:pwr93_o2lines}. 
     ...
     $CC$, $CL$, $CW$, and $CO$, with the input variable {\it userparameters}, 
     e. g. {\it userparameters}\,=\,$[$$CC$, $CL$, $CW$, $CO$$]$. 
     Each line intensity $\inten$ (see Eq. (\zref{eq:PWR93:O2_abs_inten})) 
     is multiplied with the scaling factor $CL$, while $CW$ scales 
     each line width, $\gamk$, (see Eq. (\zref{eq:PWR93:O2_gamma})) and 
     $CO$ the line coupling parameter (see Eq. \zref{eq:PWR93:O2_coupling})). 
     The continuum absorption, (see Eq. (\zref{eq:pwr_o2cont})) 
     also scales with $CC$.\\
     In the following all the valid possibilities for the
...
% --------------------- only end of table ------------------------------
 ...
 \caption{This table gives an overview of the implemented referenced 
   ...
   in the arts method {\it abs\_cont\_descriptionAppend}. Additionally the 
...
\zlabel{leveld:ArtsFullModelExampleControlFile}
% --------------------------------------------------------
Below you will find an example of a control file for all 
...
\zlabel{labela:cloudabsorption}
%=============================

\section{Liquid water and ice particle absorption}
\zlabel{levelb:lipartabs}
%*************************************************
...
\begin{eqnarray}
  \zlabel{eq:comp_perm_iwc}
  \ree    & = & 3.15\nonumber\\
%
  \ime    & = & \frac{a}{\nu}~+~b\cdot\nu\nonumber\\
%
  \zlabel{eq:comp_perm_iwc_a}
  a       & = & (\Theta-0.171)\cdot\exp{(17.0-22.1\cdot\Theta)}\\
  \zlabel{eq:comp_perm_iwc_b}
...
\zref{tab:lwc} reflects this variability by summarizing different
...
relaxation frequency ($\gamma_1$ in Equation \zref{eq:comp_perm_lwc}) 
are given, one polynomial in $\Theta$ as presented in 
Equation \zref{eq:comp_perm_lwc}) and an exponential function derived
...
Figure \zref{fig:refrac_water_comp}. A more detailed
discussion about this source of uncertainty is given in Section
\zref{levelb:ref_uncert_clouds}.
%
\begin{table}[!htb]
\begin{center}
\begin{tabular}{llll}
...
\zlabel{levelb:ref_uncert_clouds}
%--------------------------------------------------------
...
As it was stated above (Section \zref{levelb:lipartabs}) the Rayleigh 
approximation is valid for particle sizes $<$~500\,$\mu$m. Figure 
\zref{fig:cloud_part_dist} shows a particle size distribution for water
...
Figure \zref{fig:refrac_water_comp} shows a comparison of 
...
Equation \zref{eq:abs_cloud}). Additionally the same calculations with 
...
(see Table \zref{tab:lwc}). Even within a single cloud the $LWC$ ($IWC$) changes
...
\zlabel{levelb:ArtsImplementationCloudAbsorption}
% ----------------------------------------------
...
{\it abs\_cont\_names, abs\_cont\_models, 
and abs\_cont\_parameters}) for the selected continuum tags, 
which can simply be done by putting the following line into the 
arts control file:
\begin{verbatim}
...
referenced models see Table \zref{tab:artscloudlist} and the 
online documentation can be found under 
{\it arts/doc/doxygen/html/continua\_cc.html}.
...
\item[$\bullet$] The liquid water cloud absorption model of 
     ...
     \zref{labela:cloudabsorption}. The standard way to use 
     ...
     (see Eq. \zref{eq:abs_cloud}) while $CG$ scales the first 
     relaxation frequency, $\gamma_1$, (see Eq. 
     (\zref{eq:comp_perm_lwc_gamma_1})) and  $CE$ scales $\epsilon_o$ 
     (see Eq. \zref{eq:comp_perm_lwc_epsilon_0})
\begin{verbatim}
abs_cont_descriptionAppend{
...
\item[$\bullet$] The ice water cloud absorption model of 
     ...
     (see Eq. \zref{eq:abs_cloud}) while $CA$ scales $a$, (see Eq. 
     (\zref{eq:comp_perm_iwc_a})) and $CB$ scales $b$ 
     (see Eq. \zref{eq:comp_perm_iwc_b})
\begin{verbatim}
...
\subsubsection{ARTS Example Control File for the Full Model Tags}
\zlabel{leveld:ArtsCloudModelExampleControlFile}
% --------------------------------------------------------
Below you will find an example of a control file for all 
