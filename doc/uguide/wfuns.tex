...
The analytical calculations are introduced in Sec.~\ref{sec:wfuns:atmvars}. To
...
(\aOth{i}÷ defined in Eq.~\ref{eq:taustep})
\begin{equation}
  \aTrn{i} = e^{-\aOth{i}},
\end{equation}
...
transmission matrix matching $\Trn'$ and \aTrnMat{i} is defined in
...
\begin{eqnarray}
  ...
  \label{eq:chainrule2}
\end{eqnarray}
This equation is consistent with its scalar correspondance
...
$\aTrn{i}$ with respect to \aSttElm{i}\ and \aSttElm{i+1}\ can then be written
as
\begin{eqnarray}
  \frac{\PartD\aTrn{i}}{\PartD\aSttElm{i}} &=&
...
% The relationship between \aSttElm{i}\ and
...
Th terms involving $\aSttElm{i+1}$ are identical. Expressions for higher Stokes
...
\item[interp] The Jacobian is derived from existing data, by an interpolation
  ...
  minimum spacing of \builtindoc{mblock\_za\_grid} for accurate results. As
