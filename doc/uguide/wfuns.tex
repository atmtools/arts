...
is \builtindoc{jacobianAddTemperature}. See Sec.~\ref{sec:wfuns:aval} for other
retrieval quantities. The order in which these ``add methods'' is called does
not matter.
...
This section lists the workspace method to set-up the different weighting
function calculations, discusses important considerations and give the
analytical expressions applied.


\FIXME{The part below is so far just notes, to be revised}
...
\begin{description}
\item[vmr] Volume mixing ratio (a value between 0 and 1).
\item[nd] Number density. The WF corresponds here to that one molecule is added.
\item[rel] Relative/fractional change. In perfect linear case, the WF
  corresponds here to that the gas amount is doubled.
\item[logrel] As above, but the natural logarithm of relative changes is
  considered. In perfect linear case, the WF
  corresponds here to that the gas amount is increased with a factor of $e$.
\end{description}
For analytical calculations, the only remaining consideration is to select
retrieval grids.
...
the basis function for $x$ at the location of $x_i$, see
\cite{buehler:artst:05}. This is a slight approximation with respect to the
