...
\levelb{The forward model}
 ...
 model), and the \qindex{forward model}, \FrwMdl, used in practice can
 be expressed as
 \begin{equation}
 ...
 \qindex{measurement errors}. The parameters are divided in such way
 that \SttVct, the \qindex{state vector}, contains the parameters to
 be retrieved, and the remainder is given by \FrwMdlVct, the
 \qindex{model parameter vector}. The total state vector is
 \begin{equation}
 ...
 transfer for \qindex{pencil beam} (infinite spatial resolution)
 \qindex{monochromatic} (infinite frequency resolution) signals
 \citep{eriksson:99},
 \begin{equation}
...
\levelb{The sensor transfer matrix} 
 ...
 \end{equation}
 where \SnsMtr\ is here denoted as the \qindex{sensor transfer
   matrix}.  The matrix \SnsMtr\ can further incorporate effects of a
 \qindex{data reduction} and the total transfer matrix is then
 \begin{equation}
 ...
 \end{equation}
 where \aSnsMtr{d}\ is the \qindex{data reduction matrix},
...
\levelb{Weighting functions} 
 ...
 A \qindex{weighting function} is the partial derivative of the
...
% detail in Sections \ref{sec:wfuns} and \ref{sec:wfuns_sens}.
 ...
 \levelc{Transformation between \qindex{vector space}s}
