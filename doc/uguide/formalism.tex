...
\levelb{The forward model}
 ...
 model), and the forward model, \FrwMdl, used in practice can be
 expressed as
 \begin{equation}
 ...
 the atmospheric and sensor states, and \MsrErrVct\ is the measurement
 errors. The parameters are divided in such way that \SttVct, the
 state vector, contains the parameters to be retrieved, and the
 remainder is given by \FrwMdlVct, the model parameter vector. The
 total state vector is
 \begin{equation}
...
\levelb{The sensor transfer matrix} 
 ...
 \end{equation}
 where \SnsMtr\ is here denoted as the sensor transfer matrix.  The matrix
 \SnsMtr\ can further incorporate effects of a data reduction and
 ...
 \end{equation}
 where \aSnsMtr{d}\ is the data reduction matrix, \aSnsMtr{s}\ the sensor
 matrix, and $\MsrVct'$ and $\MsrErrVct'$ are the measurement vector
 and the measurement errors, respectively, before data reduction.
...
\levelb{Weighting functions} 
 ...
 \MsrVct\ with respect to some variable used by the forward model.  As the
 input of the forward model is divided between \SttVct\ or \FrwMdlVct, the
