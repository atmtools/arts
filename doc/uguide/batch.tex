...
\artsstyle{ybatch\_calc\_agenda}, where \wsvindex{ybatch\_index}, set
by \artsstyle{ybatchCalc}, tells the agenda which case to select.
More in detail, \artsstyle{ybatchCalc} makes the following operations:

\begin{itemize}

\item[1.] Performs a-c with \artsstyle{ybatch\_index} looped from 0
to (\artsstyle{ybatch\_n}-1)

\begin{itemize}
\item[a.] If \artsstyle{ybatch\_index} = 0, resizes
\artsstyle{ybatch} based
on \artsstyle{ybatch\_n} and length of \artsstyle{y} 
\item[b.] Executes \artsstyle{ybatch\_calc\_agenda}.
\item[c.] Makes copy of \artsstyle{y} in column \artsstyle{ybatch\_index} of \artsstyle{ybatch}.
\end{itemize}
