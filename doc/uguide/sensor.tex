...
sensor transfer matrix which is precalculated for the entire atmospheric simulation, and applied on the monochromatic pencil beam spectra, equation~\ref{eq:formalism:H}. To calculate the total sensor response the each individual part of the sensor has to calculated and combined.
The individual part for a microwave measurements consists in most cases of an antenna, a sideband filter and mixer and a spectrometer. These are the instruments that has been used as a starting point for the sensor modelling. There exist therefore WSM to compute these sensor responses. The operations performed by the instruments on the incoming spectra can either be described by a weighting, over viewing angles or frequencies, or a summation of signals in different sidebands.
...
\FIXME{For more information \ldots reference to Sensor modelling article.}

\subsection{Weighting}
The instruments that performs a weighting of the spectra are the antenna which weights different viewing angles by the normalised antenna response, $w_\mathrm{a}$, and the spectrometer that weights signals of neighbouring frequencies by the channel response, $w_\mathrm{ch}$. The influence of the antenna can be expressed as
\begin{equation}
...
$\SnsMtr$, where row and element positions depend on considered
...
\begin{figure}[!h]
\begin{center}
  \includegraphics{Figs/sensor/integration.mps}
...
other band (\FIXME{figure~2}), if not the RF grid is perfectly symmetric
around the LO frequency.
 
Using the same notation as in equation (\ref{eq:sensor:sum:I_IF}), the elements
of the response vector $\mathbf{h}$ can then be written as
\begin{eqnarray}
\mathbf{h}_i & = & \frac{\mathbf{v}_{i+1}-\nu}
  ...
  \mathbf{h}_{j+1} & = & \frac{\nu'-\mathbf{v}_j}
     ...
     \frac{w_\mathrm{sb}(\nu')}{w_\mathrm{sb}(\nu)+w_\mathrm{sb}(\nu')},
\end{eqnarray} 
where $\mathbf{v}$ is the RF grid and all other elements of
$\mathbf{h}$ are zero. Either $\mathbf{h}_{i+1}$ or
...
%-----------------------------
\section{Polarisation response}
\label{sec:sensor:polarisation}
...
The \artsstyle{sensor\_pol} matrix is most easily read from XML-file.
% As an example the \artsstyle{sensor\_pol} matrix can be read from XML-file;
% \begin{verbatim}
...
\FIXME{Name of WSM \artsstyle{sensor_responsePolarisation, and describe where it should be applied (before or after antenna)}

%------------------------
\section{Antenna response}
\label{sec:sensor:antenna}
The influence of the antenna can as described in above in equation~\ref{eq:sensor:weight:antenna} be expressed as a weighting between the antenna response, $w_\mathrm{a}$, as a function of viewing direction and the monochromatic pencil beam spectra.
% \begin{equation}
%   I_\mathrm{a} = \int_{4\pi}\!\!\!\!I(\Omega+\Omega_0) 
...
ARTS is prepared to handle a full 3D environment with a 2D antenna, but so far only methods for 1D antennae have been developped. 
...
\end{eqnarray}
where $\theta_0$ is the reference angle of the antenna. The weighting is only performed over the zenith angles and therefor the only variables needed before calling the calculating function apart from the output variables from the initialisation function are \artsstyle{antenna\_dim}, \artsstyle{antenna\_diagram}, \artsstyle{antenna\_los} and \artsstyle{mblock\_za\_grid}.

\FIXME{Name of WSM \artsstyle{sensor_responseAntenna1D})
...
As described in section~\ref{sec:sensor:sum} the mixer folds the two RF sidebands together into the IF band. At the same time the sideband ratio can be altered by the mixer itself and a sideband filter, these two effects are grouped together to what we refer to as the sideband filter.  

\subsection{Single mixer}
\begin{verbatim}
ReadXML( lo ) {
  "../xml/lo.xml"
}
