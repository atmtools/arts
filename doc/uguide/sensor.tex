...
%
\startsymbols
$w_\mathrm{a}$  & \builtindoc{antenna\_response} & The antenna pattern \\
$w_\mathrm{b}$ & & The backend channel response \\
$w_\mathrm{sb}$ & & The sideband filter function\\
%   & \builtindoc{antenna\_dim} & The dimensionality of the antenna pattern \\
\aFrq{ch}  & \builtindoc{f\_backend} & The frequency grid of the backend channels \\
%   & \shortcode{f\_mixer} & Output frequency grid from the mixer \\
\aFrq{\mathrm{LO}}  & \builtindoc{lo} & Local oscillator frequency \\
% \AzmAng & \builtindoc{mblock\_za\_grid} & The zenith angle grid for each measurement block \\
% \ZntAng & \builtindoc{mblock\_aa\_grid} & The azimuthal angle grid for each measurement block \\
\MtrStl{P} & \builtindoc{sensor\_pol} & The sensor polarisation response \\
\SnsMtr & \builtindoc{sensor\_response} & The total sensor response matrix \\
%   & \builtindoc{sensor\_response\_f} & The sensor frequency grid \\
%   & \builtindoc{sensor\_response\_za} & The sensor zenith angle grid \\
%   & \builtindoc{sensor\_response\_aa} & The sensor azimuth angle grid \\
%   & \builtindoc{sensor\_rot} & The rotation of the sensor between measurement blocks \\
...
represented by the WSV \builtindoc{sensor\_response}. To denote 
...
of the grid \shortcode{GaussianResponse} creates a matrix where the
...
controlled by the WSV \builtindoc{sensor\_norm}. For normalised sensor
...
there exist two WSMs, \builtindoc{sensorOff} and
\builtindoc{sensor\_responseInit}. At least one of these has to be
...
is needed \builtindoc{sensor\_off}. It sets all the necessary sensor
...
complete list use the online help for \builtindoc{sensor\_off}. The
...
\builtindoc{sensor\_responseInit} takes care of this. It sets the
...
\begin{tabular}{l l}
\builtindoc{sensor\_response\_f} & the frequency grid \\
\builtindoc{sensor\_response\_za} & the zenith angle grid \\
\builtindoc{sensor\_response\_aa} & the azimuthal angle grid \\
\builtindoc{sensor\_response\_pol} & the number of polarisations.
\end{tabular}
...
\builtindoc{sensor\_responseInit} together with
\builtindoc{sensor\_norm}, the position and line-of-sight of the sensor
...
structure \builtindoc{antenna\_response} that consists of an array of
...
\begin{center}
  ...
  \caption{Visualisation of the antenna pattern WSV \builtindoc{antenna\_response} for   
...
\builtindoc{antenna\_los}. This is a one- or two column matrix
...
WSM \builtindoc{AntennaSet1D}.

The 1D antenna is included in the sensor by using the WSM
\builtindoc{sensor\_responseAntenna1D} and it updates both \SnsMtr\ and
...
\builtindoc{sensor\_pol} matrix is most easily read from XML-file, and
the calculation of the sensor response of polarisation is done by
calling \builtindoc{sensor\_responsePolarisation}. After this method
...
\builtindoc{sensor\_rot}, and is used to assign a rotation angle for
each antenna line-of-sight. If the rotation is constant the
\builtindoc{sensor\_rot} vector can contain a single element describing
...
\builtindoc{sensor\_responseRotation} must precede the call to the
...
by its definition normalised, so even if \builtindoc{sensor\_norm} is
...
\builtindoc{lo} which in this case should have length one, and
...
arbitrary matrix variable \shortcode{matrix\_1} before being passed as
...
\end{code}
The method \builtindoc{sensor\_responseMixer} changes the sensor
response grid for frequencies to be in IF, this grid is also stored in
\builtindoc{f\_mixer}. Depending on the RF grid configuration, the
...
\builtindoc{sensor\_responseMultiMixerBackend}.  This method works a
...
the channels.
% For a sensor configuration studying more than one polarisation it is sometimes desirable to specify several different mixers. To make this efficient the backend response is also added directly to the mixer response, and this is handled by the method \builtindoc{sensor\_responseMultiMixerBackend}. 
The WSV that hold these variables are \builtindoc{sensor\_pol},
\builtindoc{lo} and \builtindoc{f\_backend}, and these should all have
...
important difference to the \builtindoc{sensor\_responseBackend} method
...
\builtindoc{ConvertIFToRF} that given the generic input for which
...
radiances, for this purpose the WSV \builtindoc{f\_backend} is a vector
...
\builtindoc{sensor\_responseBackend} by the generic input
\shortcode{arrayofmatrix\_1}. This WSV can be set up by reading
XML-files or by calling the function \builtindoc{ArrayOfMatrixSet}, see
