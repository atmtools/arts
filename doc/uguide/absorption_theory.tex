...
thermodynamic equilibrium calculations, please see later subsections.
...
\subsubsection{Line functions - theory} 
\label{sec:abs_theory:shape}

\paragraph*{Line shapes} \ 
...
of the line intensities in local thermodynamic equilibrium (Equation \ref{eq:abs_theory:line_intensity} on
...
\end{equation}
where $T_N$ is the pseudo-temperature of the $N$:th level.  Note that this
...
\begin{equation}
 e = \frac{h\nu_0}{4\pi} A_{21} r_2.
\end{equation}
Note that this set of equations describes LTE when $e/B \equiv k$, where $B$ is the
Planck blackbody emission function.
...
There have therefore been many iterations of ARTS absorption line data format.  The
...
\ref{tab:abs_theory:lineshape:lineshapetype},
\ref{tab:abs_theory:lineshape:cutofftype}, 
\ref{tab:abs_theory:lineshape:normalizationtype},
\ref{tab:abs_theory:lineshape:mirroringtype}, and
\ref{tab:abs_theory:lineshape:populationtype},
...
Each individual line in the catalog has the parameters
described in table~\ref{tab:abs_theory:lineshape:abscatline} defined.
...
\centering
 \begin{tabular}{c}
  Values for individual lines\\\hline
 \end{tabular}
\begin{tabular}{lll}
...
coefficients.  This is done for efficiency and to help keep the code 
structure easy.
...
To compute the Zeeman splitting, you do
\begin{equation}
...
\begin{equation}
 S^{(src)}_{Vib-NLTE} = S_{LTE}\exp\left({E_2 (T - T_2)}\middle/{kTT_2}\right) - S^{(abs)}_{Vib-NLTE}.
\end{equation}
and
\begin{equation}
...
What follows are the profiles available in ARTS written out in full.
Note that this will be complicated towards the end, especially as 
we have limited experience applying SDVP and HTP calculations in real
life.

\paragraph*{Doppler profile}
\begin{equation}
 F_{DP} = \frac{1}{\sqrt{\pi}\Gamma_D}e^{\left.- \left[\nu - \nu_0 - \nu_z\right]^2 \middle/ \Gamma_D^2\right.}
\end{equation}

\paragraph*{Lorentz profile}
\begin{equation}
 F_{LP} = \frac{1}{\pi} \frac{1}{\Gamma_0+i\Delta_0+i\delta\nu+i\nu_0+i\nu_z-i\nu}
\end{equation}

\paragraph*{Voigt profile}
\begin{equation}
 F_{VP} = \frac{1}{\sqrt{\pi}\Gamma_D} w(z),
\end{equation}
where
\begin{equation}
 z = \frac{\nu-\Delta_0-\delta\nu-\nu_0-\nu_z + i\Gamma_0}{\Gamma_D},
\end{equation}
and $w(\cdots)$ is the Faddeeva function.

\paragraph*{The Hartman-Tran profile and Speed-dependent Voigt profile}
will get very complicated.  Note before reading this that we convert all
values into CGS in ARTS before performing the following calculations.
The reasoning we have is that this is how the line shape was published,
and we are using a translation of the original code. This will hopefully
change in the future as we grow confident in our use of the code.
To translate these values back, to ARTS, we have to apply not just a conjunction
of the results but also scale the results by a factor $1/100c$.

\begin{equation}
 F_{HTP} = \frac{1}{\pi}\frac{A}{\left[\left(C_0 - \left.3C_2\middle/2\right.\right)\eta - \nu_{FVC}\right] A + \eta C_2 B + 1}
\end{equation}
where
\begin{equation}
...
\end{equation}
Now it will soon get complicated by braching out depending on values.
...
\end{equation}
where $z_1 = \sqrt{x+y} - \sqrt{y}$ and $z_2 = \sqrt{x+y} + \sqrt{y}$.
...
\end{equation}
where $z_1 = \sqrt{x+y}$ and $z_b = \sqrt{x}$.
...
\subsubsection{Partial derivatives of $F\left(\cdots\right)$}

The partial derivatives are listed below in order of the different profiles. 

\paragraph*{Partial derivative w.r.t.\ $T$}

\begin{itemize}
 \item[DP:] $\frac{\partial F_{DP}}{\partial T} = F_{DP}\left(2\left[\frac{\nu - \nu_0 - \nu_z}{\Gamma_D}\right]^2-1\right) \frac{\partial \Gamma_D}{\partial T} \frac{1}{\Gamma_D}$
 ...
 \item[SDVP:] Formulation too long. Analytical solution available in ARTS.
 
 \item[HTP:] Formulation too long. Analytical solution available in ARTS.
\end{itemize}
Also note that the partial derivatives for the parameters of 
Table~\ref{tab:abs_theory:lineshape:temperaturemodes} are trivial from said table's equations.

\paragraph*{Partial derivative w.r.t.\ $\nu$}
...
\begin{itemize}
 \item[DP:] $\frac{\partial F_{DP}}{\partial \nu_0} = 2F_{DP} \left[\frac{\nu-\nu_0-\nu_z}{\Gamma_D}\right] \left(\frac{\left[\frac{\nu - \nu_0 - \nu_z}{\Gamma_D}\right]-1}{\nu_0 + \nu_z}+ \frac{1}{\Gamma_D}\right)$
 
 \item[LP:] $\frac{\partial F_{LP}}{\partial\nu_0} = -\frac{iF_{LP}}{\Gamma_0+i\Delta_0+i\delta\nu+i\nu_0+i\nu_z-i\nu}$
 
 \item[VP:] $\frac{\partial F_{VP}}{\partial \nu_0} = \frac{2}{\sqrt{\pi}\Gamma_D}\left(\frac{i}{\sqrt{\pi}} - z w\left(z\right) \right) \left(-\frac{1}{\Gamma_D}-\frac{z}{\Delta_0+\delta\nu+\nu_0+\nu_z}\right) - \frac{F_{VP}}{\Delta_0+\delta\nu+\nu_0+\nu_z}$
...
\paragraph*{Partial derivative w.r.t.\ $\delta\nu$}

See ``Partial derivative w.r.t.\ $\nu_0$'' for identical solution.  Also note that chaining for
any coefficient in Table~\ref{tab:abs_theory:lineshape:temperaturemodes} is possible.

\paragraph*{Partial derivative w.r.t.\ $\Delta_0$}

See ``Partial derivative w.r.t.\ $\nu_0$'' for identical solution.  Also note that chaining for
any coefficient in Table~\ref{tab:abs_theory:lineshape:temperaturemodes} is possible.

\paragraph*{Partial derivative w.r.t.\ $||\vec{B}||$}

See ``Partial derivative w.r.t.\ $\nu_0$'' for almost identical solution.
Scale expressions by $\left. \mu_B \left(g_{z_1}M_1-g_{z_2}M_2\right) \middle/h\right.$ to be correct.

\paragraph*{Partial derivative w.r.t.\ $\Gamma_0$}
...
\paragraph*{Partial derivative w.r.t.\ $\Gamma_2$}
...
\paragraph*{Partial derivative w.r.t.\ $\Delta_2$}
...
\paragraph*{Partial derivative w.r.t.\ $\eta$}
...
Note that chaining for any coefficient in Table~\ref{tab:abs_theory:lineshape:temperaturemodes} is possible.
