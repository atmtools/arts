...
Line mixing is an effect where the line shape of multiple lines must be described as one
...
\paragraph*{The ARTSCAT-4 line format is:}
\begin{code}
...
\shortcode{arts-xml-data} package.

\paragraph*{The ARTSCAT-5 line format is:}
\begin{code}
...
function, and QN for quantum numbers.
...
\hline
\end{tabularx}
\caption{Pressure broadening and pressure shift input variables for ARTSCAT-3 and ARTSCAT-4. ARTSCAT-5 
inputs depends on the tag system, and is presented in the text.}
...
\subsubsection{ARTSCAT-5 pressure broadening}
The ARTSCAT-5 pressure broadening format is determined by its pressure broadening tag.
The pressure broadening tag is \verb|PB| and it is followed by one of \verb|N2|, 
\verb|AP|, or \verb|WA|.  

\paragraph*{If} the subtag is \verb|N2| for air broadening, 
the following 10 data parameters are identical to ARTSCAT-3 parameters.
In order they should be given as: SGAM, NSELF, AGAM, NAIR, PSF, DSGAM, DNSELF, DAGAM, DNAIR, and DPSF.
The pressure broadening then proceeds as specified by the ARTSCAT-3 method above.

\paragraph*{If} the subtag is \verb|AP| for Agnes Perrin Broadening,
the following 20 data parameters are identical to, 
and in order of, columns 9-28 of ARTSCAT-4 line format. The pressure broadening  calculations
follow the ARTSCAT-4 method above.

\paragraph*{If} if the subtag is \verb|WA| for water and air broadening, there are 9 data 
parameters to follow.  These are a mixture of parameters from ARTSCAT-3 and ARTSCAT-4.
The parameters are: SGAM, NSELF, SPSF, %FIXME:  Is it it even valid to have self pressure shift?  I only have it in for the symmetry of the data format.
AGAM, NAIR, APSF, GAMMA\_H2O, N\_H2O, and DELTA\_H2O.
The only new variables are SPSF and APSF, which are PSF but for self and air, respectively.

If there is no water in the atmosphere, or if the pressure effects on a water
line is being calculated, this method is identical to air broadening or ARTSCAT-3.
However, if water is present and the line is not a water line, pressure broadening is calculated as
\begin{eqnarray}
  \gamma_L &=& x_\mathrm{self} \; p \; \mathrm{SGAM} \; 
             \left(
             \frac{\mathrm{T0}}{T}
...
and the pressure shift is calculated as
\begin{eqnarray}
  \gamma_L &=& x_\mathrm{self} \; p \; \mathrm{SPSF} \; 
             \left(
             \frac{\mathrm{T0}}{T}
...
\subsubsection{ARTSCAT-5 line mixing}
The ARTSCAT-5 line mixing is determined by its line mixing tag \verb|LM|.  If
\verb|LM| it is assumed there are no line mixing parameters, so no calculations
are performed.  If \verb|LM| is given, it must be followed by one of \verb|LL|, 
%\verb|NR|, 
\verb|L1|, or \verb|L2|.
Below follows the calculations performed to calculate the line mixing
parameters for equation~\ref{eq:abs:lm:2ndorder} depending on method.

\paragraph*{If} the subtag is \verb|LL| for LBLRTM line mixing,
the following 12 data parameters are a 4-vector temperature grid,
a 4-vector gridded $y_{\mathrm{lm}}$, and a 4-vector gridded
$g_{\mathrm{lm}}$.  The value $\delta\nu_{\mathrm{lm}}$ is set to nil,
and the value to be used for the other two are interpolated to the atmospheric temperature.

\paragraph*{If} the subtag is \verb|L1| for first order line mixing styled as by \citet{tretyakov05:_60-ghz_jms},
the following 3 data parameters are in order the reference temperature of the measurement $T_{\mathrm{lm}}$, the value of $y_{\mathrm{lm}}$
at the reference temperature $y'_{\mathrm{lm}}(T_{\mathrm{lm}})$, and a temperature dependency exponent $n_{y_{\mathrm{lm}}}$.  
The values of $\delta\nu_{\mathrm{lm}}$ and $g_{\mathrm{lm}}$ are set to nil, and
\begin{equation}
  y_{\mathrm{lm}} =  y'_{\mathrm{lm}}(T_{\mathrm{lm}})\left(\frac{T_{\mathrm{lm}}}{T}\right)^{n_{y_{\mathrm{lm}}}}
\end{equation}
at each calculation of the line shape. 

\paragraph*{If} the subtag is \verb|L2| for second order line mixing styled as by \citet{makarov11:_60-ghz_jqsrt},
the following 9 data parameters are in order
the first order first correction term at the reference temperature $y'_{\mathrm{lm}}(T_{\mathrm{lm}})$, 
the first order second correction term  at the reference temperature $y''_{\mathrm{lm}}(T_{\mathrm{lm}})$,
the second order first absolute correction term at the reference temperature $g'_{\mathrm{lm}}(T_{\mathrm{lm}})$, 
the second order second absolute correction term  at the reference temperature $g''_{\mathrm{lm}}(T_{\mathrm{lm}})$, 
the second order first frequency correction term at the reference temperature $\delta\nu'_{\mathrm{lm}}(T_{\mathrm{lm}})$, 
the second order second frequency correction term  at the reference temperature $\delta\nu''_{\mathrm{lm}}(T_{\mathrm{lm}})$, 
the reference temperature $T_{\mathrm{lm}}$, 
the first order temperature exponent $n_{y_{\mathrm{lm}}}$, 
the second order absolute correction temperature exponent $n_{g_{\mathrm{lm}}}$, and 
the second order frequency correction temperature exponent $n_{\delta\nu_{\mathrm{lm}}}$.
When the line shape is calculated, the following considerations are done for each line mixing
term:
\begin{eqnarray}
