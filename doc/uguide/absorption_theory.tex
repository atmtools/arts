...
\subsubsection{The Zeeman Effect O$_2$}
...
In quantum physics terminology, for oxygen, the rule of transition only allows $\Delta J =\pm 1$ ($J$ is the total angular momentum number) and $\Delta M = 0, \pm 1$ ($M$ is the quantum number denoting the projection of the molecular magnetic moment),
where $N-1\leq J \leq N+1$ ($N$ is the main rotational quantum number) and $M = -J,$ $(-J+1),\cdots,$ $0,\cdots,$ $(J-1),$ $J$. 
A transition with $\Delta J = +1$ is called an $N^+$ transition and
 a transition with $\Delta J = -1$ is called an $N^-$ transition. A simple schematic of the allowed transitions can be found in Figure~\ref{fig:zeeman_schematic}.

Rewriting Equation~\ref{eq:abs_coeff} to take into account the Zeeman effect for oxygen gives
\begin{equation}\label{eq:abs_zeeman_mat}\scriptsize
 \mbox{\boldmath${\alpha}$}(\nu)=\frac{1}{2}nS(T)\sum_{M,\Delta M} \left\{ \zeta(N,M,\Delta J, \Delta M) F(\nu, \nu_0 + \Delta\nu_0[N,M,\Delta J, \Delta M, H])  \mbox{\boldmath${K}'$}(\theta, \eta,\Delta M)  \right\}.
\end{equation}
Term by term we know what $n$, $S(T)$ and $F(\nu,\nu_0)$ are supposed to represent. The additional terms $\zeta$, $\Delta\nu_0$ and \boldmath${K'}$\unboldmath are due to quantum physics and some angles relating to the coordinate system and propagation path. The factor $\frac{1}{2}$ is due to normalization issues and could be incorporated into $\zeta$ if need be. It is however kept in the theory for consistency with the tabular values of Liebe and Hufford (1989). The relative strength, $\zeta$, and central frequency shift, $\Delta\nu_0$, can be found in Tables~\ref{tab:delta_nu_0}~and~\ref{tab:zeta} respectively.
...
\begin{minipage}{280pt}
  \begin{tabular}{l|ll}
    ...
    \\[5pt]\hline
  \end{tabular}
\caption{Frequency shift due to the Zeeman effect. $H$ is the magnitude of the external field in Tesla and $K$ is a constant of $2.8026\times10^{10}$
...
From Lenoir (1968) the rotating \boldmath$\rho$\unboldmath-matrices in coherency terms are
\begin{equation}
...
\end{equation}
The angle, $\theta$, is between the propagation direction of the radiation and the magnetic field vector. The {\boldmath$\rho$}-matrix relevant depends on the transition type. If $\Delta M = \pm 1$ use {\boldmath$\rho$}$_{\sigma\pm}$ and if $\Delta M = 0$ use {\boldmath$\rho$}$_{\pi}$.
...
\end{equation}
gives us easy to use means of deriving {\boldmath$K$} since we now can treat each Stokes parameter separately
...
the isotopologue ratios, in order to allow calculations for other
...
\item[JPL:] Only Species (including individual isotopologues) not covered by TIPS\_2003.
