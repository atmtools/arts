...
path is valid for the first and last element in \wsvindex{f\_grid}, as this is
the frequency given to \wsaindex{refr\_index\_agenda}.
...
\builtindoc{f\_grid} in parallel, for efficiency reasons. One aspect of this is
that only a single propgation path is calculated, that is assumed to be common
for all frequencies. With other words, \textindex{dispersion} is not
...
\wsaindex{iy\_main\_agenda} as follows:
\begin{code}
...
\end{code}
The approach is simple, \wsvindex{iyLoopFrequencies} calls
\builtindoc{iy\_sub\_agenda} for each single frequency of \builtindoc{f\_grid}
and appends the output data. Or, with some practical details,
\builtindoc{iyLoopFrequencies} performs a loop over the \builtindoc{f\_grid},
creates an internal \builtindoc{f\_grid} of length 1 holding the frequency of
concern and calls \builtindoc{iy\_sub\_agenda} with this length-1 frequency
...
first and last element in \builtindoc{f\_grid} (also commented above).

The feature provided by \builtindoc{iyLoopFrequencies} can also be used to
