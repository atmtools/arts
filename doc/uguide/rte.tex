...
%==============================================================================
\zlabel{sec:fm_defs:polarisation}
...
are found in Section~\zref{sec:polarization}.
...
by Equation~\zref{eq:rtetheory:VRTE}. If scattering can be neglected,
...
below. Section~\zref{sec:fm_defs:rte} gives further insights in how the
sensor is treated in ARTS.


\subsection{Sensor position\index{sensor position}}
%===================
\zlabel{sec:fm_defs:sensorpos}
...
atmosphere (see Section~\zref{sec:fm_defs:ppaths}). An obviously
...
Section~\zref{sec:fm_defs:seqsandblocks}.


\subsection{Line-of-sight\index{line-of-sight}}
%===================
\zlabel{sec:fm_defs:los}
...
(Section~\zref{sec:fm_defs:ppaths}).  The zenith and azimuthal angles
...
\begin{figure}[!t]
 \begin{center}
  ...
  \begin{minipage}[c]{0.4\textwidth}
   ...
   \zlabel{fig:fm_defs:los}
  \end{minipage}
 \end{center}
\end{figure}           
 
The \textindex{zenith angle}, \ZntAng, is simply the angle between the
line-of-sight and the zenith direction (Figure~\zref{fig:fm_defs:los}).
...
(Section~\zref{sec:ppath:geoid}). A nadir observation,
...
%===================
\zlabel{sec:fm_defs:seqsandblocks}
...
below in Equation~\zref{eq:fm_defs:measseq}.
...
Section~\zref{sec:fm_defs:sensorchar}. The pencil beam spectra for each
...
%==============================================================================
\zlabel{sec:fm_defs:rte}

An introduction to radiative transfer theory is given in
Section~\zref{sec:rte_theory}. This section describes how the radiative
...
outlined in Algorithm~\zref{alg:fm_defs:RteCalc}. For further details
...
\begin{algorithm}[!t]
 ...
 \caption{Outline of the overall clear sky radiative transfer calculations
   (\artsstyle{RteCalc}).}
 \zlabel{alg:fm_defs:RteCalc}
\end{algorithm}


\subsection{Propagation paths}
%===================
\zlabel{sec:fm_defs:ppaths}
...
described in Section~\zref{sec:ppath:Ppath}. The path points are
...
\begin{figure}[!p]
 \begin{center}
  ...
  \caption{Examples on allowed propagation paths for a 2D atmosphere. 
    The atmosphere is plotted as in Figure~\zref{fig:fm_defs:2d} beside
    ...
    a specified limit.}
  \zlabel{fig:fm_defs:ppath_cases2}
 \end{center}
\end{figure}
...
\begin{figure}[!p]
 \begin{center}
  ...
  \caption{Examples on allowed propagation paths for a 1D atmosphere
    ...
    Figure~\zref{fig:fm_defs:ppath_cases2}. When the sensor is placed 
  ...
  \zlabel{fig:fm_defs:ppath_cases1}
 \end{center}
\end{figure}
% This figure was produced by the Matlab function mkfigs_ppath_cases.
...
\begin{figure}[!t]
 \begin{center}
  ...
  \caption{Examples on \emph{not} allowed propagation paths for a 2D 
    ...
    text.}
  \zlabel{fig:fm_defs:ppath_badcases}
 \end{center}
\end{figure}
% This figure was produced by the Matlab function mkfigs_ppath_cases.
...
(Section~\zref{sec:fm_defs:surface}). Example on propagation
paths are shown in Figures~\zref{fig:fm_defs:ppath_cases2} and 
\zref{fig:fm_defs:ppath_cases1}.
...
(Figure~\zref{fig:fm_defs:ppath_badcases}). In addition, if the sensor
...
\item[Space] When the propagation path starts at the top of the
  ...
  Algorithm~\zref{alg:fm_defs:RteCalc}) equals the output of
...
\item[The surface] The sum of surface emission and radiation reflected
  ...
  Section~\zref{sec:fm_defs:surface}.
...
\artsstyle{RteCalc} (see Algorithm~\zref{alg:fm_defs:RteCalc}). In
...
be used in this way is (cf.  Equation~\zref{eq:rtetheory:layer})
\begin{equation}
  \Mpi_{i+1}(\Frq) = \Mpi_i(\Frq)e^{-\aOth{i}} + B(\Frq,\aTmp{i})(1-e^{-\aOth{i}})
  \zlabel{eq:fm_defs:rte_step}
\end{equation}
...
Section~\zref{sec:fm_defs:polarisation}).



\subsection{Surface effects}
%===================
\zlabel{sec:fm_defs:surface}
...
(Figure~\zref{fig:fm_defs:surface_refl})
\begin{equation}
  \MpiVct_s^u = \MpiVct_e + \sum_l^{} \mathbf{R}_l \MpiVct_l^d
  \zlabel{eq:fm_defs:surfacerefl}
\end{equation}
...
discussed further in Section~\zref{sec:surface}.

\begin{figure}[!tb]
 \begin{center}
  \includegraphics*[width=0.95\hsize]{ground_refl}
  \caption{Schematic of Equation \zref{eq:fm_defs:surfacerefl}.}
  \zlabel{fig:fm_defs:surface_refl}
 \end{center}
\end{figure}
...
%    surfaces.}
%  \zlabel{fig:fm_defs:ppath_surface}
% \end{center}
%\end{figure}
%% This figure was produced by the Matlab function mkfigs_ppath_cases.
...
Equation~\zref{eq:fm_defs:rte_step} is used, the underlaying
