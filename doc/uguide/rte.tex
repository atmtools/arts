...
radiative transfer calculations are not part of \artsstyle{yCalc}.
...
agendas, such as \artsstyle{ppath\_step\_agenda}. All methods developed for
...
atmospheric grids (\artsstyle{p\_grid}, \artsstyle{lat\_grid} and
\artsstyle{lon\_grid}). A path point is also placed at the sensor if it is
...
\artsstyle{ppath\_lmax}, see further Sections \ref{sec:fm_defs:accuracy} and
\ref{sec:ppath:usage}.
...
for \artsstyle{ppath\_step\_agenda}, refraction will be considered etc.
...
\artsstyle{iy\_clearsky\_agenda}. 
...
\artsstyle{iyEmissionStandardClearsky}. The value of $\mathcal{T}$ at the
radiative background can also be outputted as auxilary data
(\wsvindex{iy\_aux}). See further the built-in documentation.

For inclusion in \artsstyle{iy\_clearsky\_basic\_agenda}, there is a version
