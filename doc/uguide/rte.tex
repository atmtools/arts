...
the actual radiative transfer calculations. This includes an overview of how
...
\builtindoc{iy\_clearsky\_agenda}, in the order they are executed.
...
\builtindoc{iy\_clearsky\_agenda}. For inclusion in
\builtindoc{iy\_clearsky\_basic\_agenda}, there is a version denoted as
...
\begin{equation}
  \label{eq:planck}
  B(T) = \frac{2h\Frq^3}{c^2(exp(h\Frq/k_BT)-1)},
\end{equation}
where $h$ is the Planck constant, $c$ the speed of light and $k_B$ the
Boltzmann constant. This expression gives the total power, per unit frequency
per unit area per solid angle. (The Planck function can also be defined as a
