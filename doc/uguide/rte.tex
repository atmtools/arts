\chapter{Radiative transfer: definitions and approach}
 \label{sec:rte}


 \starthistory
 110428 & Extension and general revision of the chapter (PE).\\
 050613 & First complete version finished by PE.\\
 \stophistory

\graphicspath{{Figs/rte/}}

This chapters defines some concepts and quantities, and gives an overview of
the approach to handle the radiative transfer calculations. This includes an
overview of how effects caused by the sensor and surface are incorporated.

The default assumption is that there is no particle scattering and the
(atmospheric) absorption/emission is unpolarised. This is for simplicity
denoted as clear-sky calculations. Cases involving scattering or polarised
absorption are handled by the ``cloud box'' (Sec.~\ref{sec:fm_defs:cloudbox}).
In short, the more demanding calculations are restricted to a smaller domain of
the model atmosphere, and the radiative transfer approach to apply in this
domain is specified seperately.

This chapter deals with general aspects of radiative transfer and the
algorithms applied outside the cloudbox. Even though the atmospheric absorption
...
The structure of the part handling the radiative transfer calculations is
fixed. The corresponding workspace method is \wsmindex{yCalc}. The calculation
procedure of \artsstyle{yCalc} is outlined in
Algorithm~\ref{alg:fm_defs:yCalc}. For further details of each calculation
step, see the indicated equation or section.
 ...
 \begin{algorithmic}
  ...
  \FORALL{sensor positions}
   \FORALL[Section \ref{sec:fm_defs:seqsandblocks}]
    ...
    \STATE{call \artsstyle{iy\_clearsky\_agenda}}
...
The primary unit for emission spectra, the unit of
\aMpiVct{b} in Equation~\ref{eq:fm_defs:measseq}, is
[W/(Hz$\cdot$m$^2\cdot$sr)]. The emission intensity corresponds directly with
the definition of the Planck function in \theory,
Equation~\ref{T-eq:rtetheory:planck}. This radiance can be converted to other
emission units, a selction between existing options is made through 
\wsvindex{y\_unit}.


\section{Radiative transfer theory and algorithms}
%---
\label{sec:fm_defs:rte}

This section describes how the core radiative transfer equation is solved
practically in ARTS. The general presentation assumes that the simulations deal
with an emission measurement. Focus is put on emission measurements as ARTS is
...
\subsection{The agenda for clear sky radiative transfer, \artsstyle{iy\_agenda}}
%===================
\label{sec:fm_defs:rte_agenda}

The task of \wsmindex{iy\_clearsky\_agenda} is to perform the clear sky
radiative transfer calculation for the specified line-of-sight. That is, it
calculates monochromatixc pencil beam spectra. All methods for
...
\subsection{Standard solution approach}
%===================
...
as the mean of the absorption at the end points of the step, multiplicated with
...
The emission is unpolarised for the conditions assumed here, and the expression
for higher Stokes component is
\begin{equation}
  \Mpi_{i+1}(\Frq) = \Mpi_i(\Frq)e^{-\aOth{i}}.
  \label{eq:fm_defs:rte_step2}
\end{equation}
These expressions are implemented in the method
\wsmindex{iyEmissionStandardClearsky}.
...
This function is normally only called from \artsstyle{yCalc}, but it
...
surface can be calculated by a recursive call of \artsstyle{yCalc}
...
calls of \artsstyle{yCalc} (with cloud box deactivated) and the
...
\section{Surface scattering and emission}
%===================
