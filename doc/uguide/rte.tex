...
\end{equation}
where \IdnMtr\ is the identity matrix. The extinction matrices at point $i$ and
$i+1$ are averaged (element-wise) to give $\bar{\ExtMat}$, in line with
Equation~\ref{eq:taustep}. The \VctStl{b}-vectors are handled likewise.
The matrix exponential \dots
