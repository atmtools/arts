\chapter{Clear-sky radiative transfer calculations}
 \label{sec:rte}
...
thermal emission in absence of particle scattering are taken as the standard
case, and the basic theory for such simulations is also covered.
...
\item[\aMpiVct{b}]: Monochromatic pencil beam data for a measurement block. The
  ...
  \wsvindex{iyb}, but can be considered as an internal variable and should not
  be of concern for the user.
\item[\aMpiVct{y}]: Monochromatic data for a single (pencil beam) line-of-sight
  The corresponding workspace variable is \wsvindex{iy}. (\aMpiVct{b} consists
  of one or several \aMpiVct{y} appended.)
...
(line-of-sight) separately. It is the task of \wsvindex{iy\_main\_agenda}
...
longitude ranges (Figure~\ref{fig:fm_defs:ppath_badcases}). Propagation paths
can be calculated separately by the method \wsmindex{ppathCalc}, but for
standard calculations the propagation paths are calculated internally by
\builtindoc{yCalc}. The specification of propagation path calculations is
discussed in Section~\ref{sec:ppath:usage}.
...
\builtindoc{iy\_main\_basic\_agenda} (with cloud box deactivated) and the
...
Beside the actual radiances, \builtindoc{iy\_main\_agenda} can provide
...
\wsvindex{iy\_main\_basic\_agenda} and returns only radiances.
...
\builtindoc{iy\_main\_agenda}. For inclusion in
\builtindoc{iy\_main\_basic\_agenda}, there is a version denoted as
\wsmindex{iyEmissionStandardClearskyBasic}. This version is not calculating
$\mathcal{T}_i$, or provides any auxiliary data. 

The term $B$ is set by \wsvindex{emission\_agenda}. The setting of this agenda
...
\builtindoc{emission\_agenda} is \wsmindex{emissionPlanck}. The Planck function
...
\wsvindex{iy\_space\_agenda} is \wsmindex{MatrixCBR} (together with some calls
of \builtindoc{Ignore}). Please, note that \builtindoc{emission\_agenda} and 
\builtindoc{iy\_space\_agenda} must be defined in a consistent manner, that
