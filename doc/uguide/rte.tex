...
calculations}
 ...
 \starthistory
 110610 & Extended and general revision (Patrick Eriksson).\\
 050613 & First complete version by Patrick Eriksson.\\
 \stophistory

\graphicspath{{Figs/rte/}}

This section defines some concepts and quantities, and gives an overview of
the approach to handle radiative transfer calculations. This includes an
...
the model atmosphere, and the radiative transfer in this
domain is treated by dedicated workspace methods.
...
\end{description}
The \artsstyle{yCalc} method is outlined in
...
\theory, Section~\ref{T-sec:rte_theory}.
...
next (compare \theory, Equation~\ref{T-eq:rtetheory:layer})
\begin{equation}
...
points of the step. The start value of \Mpi\ is determined by the ultimate 
radiative background (Section \ref{sec:fm_defs:rad_bkgr}).

The emission is unpolarised for the conditions assumed here, and the emission
term vanishes. The extinction matrix is diagonal, with all diagonal elements
equal. Accordingly, the expression for the second Stokes component is
\begin{equation}
...
emission units, a selection between existing options is made through 
\wsvindex{y\_unit}.
