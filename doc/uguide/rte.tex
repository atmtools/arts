...
absorption is independent of absorption). For unpolarised absorption the
...
instead handled separately by \wsmindex{iyApplyYunit}.
...
and appends the output data. With some details, \builtindoc{iyLoopFrequencies}
...
\builtindoc{iyLoopFrequencies} can not be combined with auxiliary data of
...
the options for each workspace method of concern. For example:
\begin{code}
...
\item[Radiative properties, along-the-path] The radiance at each propagation
  path point.
\item[Overall radiative properties] The total optical depth along the path.
\end{description}
...
returned by \wsmindex{iy\_main\_agenda}. The \builtindoc{ppath} contains the
information needed to geo-position, for example, ``along-the path
temperatures''.
