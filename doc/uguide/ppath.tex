...
the line-of-sight at the new point is determined (Figure~[**]). The
found line-of-sight is used to calculate path step etc. This can be
seen as an Euler solution to the differential problem given by
...
needed for 2D and 3D. These calculations are implemented in
the function \funcindex{raytrace\_1d\_linear\_euler}.
...
position after a ray tracing step. The latitude of the found
point is kept, and the influence of refraction is considered to only
influence the zenith angle:
\begin{equation}
  ...
  \label{eq:ppath:za2d}
\end{equation}
This expression is found in \funcindex{raytrace\_2d\_linear\_euler}.
The term including the gradients of the refractive index can be seen
as a correction to the zenith angle that would have been obtained
if refraction had been neglected.
...
\begin{eqnarray}
  ...
  \label{eq:ppath:za3d} 
\end{eqnarray}
%  \aAzmAng{i+1} &=& \aAzmAng{g} + \frac{l_g}{n_i} \sin\aZntAng{i} 
%   \left[
%    -\sin\aAzmAng{i} 
%          \left( \frac{\PartD n}{\PartD \Lat} \right)_{(\aRds{i},\aLon{i})}
%    +\cos\aAzmAng{i} 
...
\funcindex{raytrace\_3d\_linear\_euler}
