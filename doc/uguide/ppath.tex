...
is selected by specifying \wsaindex{ppath\_step\_agenda} (see
...
\begin{description}
  ...
  \item[dim] [Index] The atmospheric dimensionality. This field shall always 
   ...
   \item[start\_pos] [Vector] The practical start position of the propagation
     ...
     exception if radio link calculations where the transmitter is placed above
     the model atmosphere, where this field gives the position of the
     transmitter.

   \item[start\_los] [Vector] Line-of-sight at end point of propagation
   ...
   \item[lspace] [Numeric] Propagation length through free space above the
     model atmosphere. Only points inside the atmosphere are stored and the sum
     of \shortcode{lstep} does not cover the propagation between the top-of-the
     atmosphere and a sensor placed in space.
