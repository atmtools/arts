...
\chapter{Propagation paths and the geoid}
 \zlabel{sec:ppath}
...
propagation paths is given in Section~\zref{sec:fm_defs:ppaths} and it
...
longitude grid (Figure~\zref{fig:ppath:ex1}). There is one exception to
...
\begin{figure}[!t]
 \begin{center}
  ...
  \caption{Tracking of propagation paths. For legend, see 
    Figure~\zref{fig:ppath:ex2}. The figure tries to visualize how the
...
\begin{figure}[!t]
 \begin{center}
  ...
  \caption{As Figure~\zref{fig:ppath:ex1}, but with a length criterion 
...
the generic input \artsstyle{lmax} (see Figure~\zref{fig:ppath:ex2}).
...
Figure~\zref{fig:fm_defs:surface_refl}). The variable \artsstyle{ppath}
...
\begin{description}
  ...
  \item[dim] [Index] The atmospheric dimensionality. This field shall always 
   ...
   \item[constant] [Numeric] The propagation path constant. Such a
     ...
     Sections~\zref{sec:ppath:basicgeom} and [**]. This field can be
   ...
   \item[pos] [Matrix] The position of the propagation path points.
     ...
     Section~\zref{sec:fm_defs:sensorpos}). The number of columns for
     ...
     further Section~\zref{sec:fm_defs:atmdim}).
  ...
  \item[z] [Vector] The geometrical altitude for each path position. The
   ...
   \item[gp\_p] [ArrayOfGridPos] Index position with respect to the
     ...
     Section~\zref{sec:interpolation:gridpos}. 
   ...
   \item[los] [Matrix] The line-of-sight of the propagation path at
     ...
     Section~\zref{sec:fm_defs:los}. If the radiative background is the
...
\item[p] [Vector] The pressure for each path position. Length is
  accordingly \artsstyle{np}. Set by \funcindex{iy\_calc}.

\item[t] [Vector] The temperature for each path position. Length is
  accordingly \artsstyle{np}. Set by \artsstyle{iy\_calc}.
  
\item[vmr] [Matrix] The VMR of each gas species at each path
  position. Set by \artsstyle{iy\_calc}.
...
\item[next\_parts] [ArrayOfIndex] The index in
  ...
  \artsstyle{surfaceCalc}.
\end{description}


\section{Structure of implementation}
%===================
\zlabel{sec:ppath:structure}
...
Algorithm~\zref{alg:ppath:ppath_calc}. The function can be divided into
...
geometrical calculations (see Sections \zref{sec:ppath:basicgeom} and
\zref{sec:ppath:stepcalc}) as the refractive index is assumed to have the
...
\begin{algorithm}[!t]
 ...
 \zlabel{alg:ppath:ppath_step_geom_2d}
\end{algorithm}
...
step function. Algorithm~\zref{alg:ppath:ppath_step_geom_2d} summerizes
this for geometrical 2D steps.


\section{General comments}
%===================
\zlabel{sec:ppath:comments}

The calculation of propagation paths involves a number of mathematical
expressions and they are presented in
Sections~\zref{sec:ppath:basicgeom}~-~\zref{sec:ppath:refreuler}. In
...
recommended to read Section~\zref{sec:interpolation} before you
continue here.
...
Equation~\zref{eq:ppath:g_geom}, we get that
...
triangle and Equation~\zref{eq:ppath:r2l} corresponds to the
...
taking the difference of Equation~\zref{eq:ppath:r2l} for the two
radii.
...
Equation~\zref{eq:ppath:geomconst} (\funcindex{geomppath\_r\_at\_za})
\begin{equation}
  \Rds(\ZntAng) = \frac{p_c}{sin(\ZntAng)}
  \zlabel{eq:ppath:za2r}
\end{equation}
...
is obtained by combining Equations \zref{eq:ppath:za2lat} and
\zref{eq:ppath:za2r}.
...
given by Equations~\zref{eq:ppath:geomconst}~-~\zref{eq:ppath:za2lat}.


\subsection{2D}
%===================
\zlabel{sec:ppath:2Dgeom}

The definitions given in Sections~\zref{sec:fm_defs:atmdim} results in
...
Figure~\zref{fig:ppath:psurf_crossing} gives a schematic description of
...
\begin{figure}
 ...
 \begin{minipage}[c]{0.55\textwidth}
  ...
  \zlabel{fig:ppath:psurf_crossing}
 \end{minipage}%
\end{figure}   
...
\end{eqnarray}
Equation~\zref{eq:ppath:psurf1} becomes with these replacements a
polynomial equation of order 4:
\begin{eqnarray}
  \zlabel{eq:ppath:psurf2}
...
interest (Figure~\zref{fig:ppath:ex3}).



\subsection{3D}
%===================
\zlabel{sec:ppath:3Dgeom}
...
Equation~\zref{eq:ppath:rcoss3Db} below is a reminiscence of that work.
...
\begin{algorithm}[!t]
 ...
 \caption{The method applied in \artsstyle{do\_gridcell\_3d} to find the 
          ...
          spherical coordinates (Equation \zref{eq:ppath:cart2sph}).}
 \zlabel{alg:ppath:dogridcell3d}
\end{algorithm}

A straightforward trail-and-error algorithm was then tested
(Algorithm~\zref{alg:ppath:dogridcell3d} and
Figure~\zref{fig:ppath:3Dsearch}). The main advantage of the algorithm
...
\begin{eqnarray}
 ...
 \zlabel{eq:ppath:sph2cart}
\end{eqnarray}
and
\begin{eqnarray}
 \zlabel{eq:ppath:cart2sph}
...
\begin{figure}[!t]
 \begin{center}
  ...
  \caption{Schematic of Algorithm~\zref{alg:ppath:dogridcell3d}. The
...
\begin{equation}
 ...
 \left[ \begin{array}{c}
  ...
  \DiffD \Lon
 \end{array} \right]
 \zlabel{eq:ppath:los2cart}
\end{equation}
Note that the radial terms (\Rds) in Equations
\zref{eq:ppath:los2sphvec} and \zref{eq:ppath:los2cart} cancel each
...
definition (Section~\zref{sec:fm_defs:los}).
...
spherical coordinates by Equation~\zref{eq:ppath:cart2sph} and the new
...
\begin{eqnarray}
  ...
  \AzmAng &=& -\arccos(\Rds\DiffD\Lat/\sin(\ZntAng)), 
                      \qquad \textrm{for} \quad \DiffD\Lon < 0  \nonumber
 \zlabel{eq:ppath:conv2zaaa}
\end{eqnarray}
Special expressions must be used for positions at the north and south
pole (see the code) as the azimuth angle has there a special
definition (Section~\zref{sec:fm_defs:los}). These calculations are performed in
...
\begin{equation}
  ...
  \zlabel{eq:ppath:rcoss3Db}
\end{equation}
Once $l$ is determined, the latitude and longitude are easily
calculated by Equations~\zref{eq:ppath:xdl} and
\zref{eq:ppath:cart2sph}. These calculations are implemented in the
...
(Figure~\zref{fig:ppath:ppath_refr1}), resulting in that inclusion of
...
(Figure~\zref{fig:ppath:euler}). The found line-of-sight is used to
calculate the next ray tracing step etc. This can be seen as an Euler
solution to the differential problem given by
Equation~\zref{eq:ppath:refrbend}. The main difference between handling
...
\begin{equation}
 ...
 \zlabel{eq:ppath:snellspherical}
\end{equation}
...
noteworthy that with $n=1$, Equations~\zref{eq:ppath:geomconst} and
\zref{eq:ppath:snellspherical} are identical.
...
rearrangement of Equation~\zref{eq:ppath:snellspherical} gives
\begin{equation}
  \ZntAng = \arcsin( \Rds n / p_c )
 \zlabel{eq:ppath:za1D}
\end{equation}
...
Figure~\zref{fig:ppath:N} shows the vertical variation of the
refractive index.
...
\begin{figure}[!t]
 \begin{center}
  ...
  \caption{Latitude gradient of the refractive index due to varying radius 
    ...
    Figure~\zref{fig:ppath:dndr}e. The wet atmosphere from
    Figure~\zref{fig:ppath:dndr} was used for all latitudes, and the
    ...
    hemisphere.}
  \zlabel{fig:ppath:dndlat}  
 \end{center}
\end{figure}
% This figure was produced by the Matlab function mkfigs_refraction

Equation~\zref{eq:ppath:refrbend} expressed in polar coordinates
...
Equation~\zref{eq:ppath:za2lat}. The geometrical zenith angle at ray
...
found in Equation~\zref{eq:ppath:za2d}, and this term is accordingly
...
from the position of concern. Figures~\zref{fig:ppath:dndr} and
\zref{fig:ppath:dndlat} show example on gradients of the refractive
index.
...
\aRds{p}. These radii are indicated in Figure~\zref{fig:ppath:lats}.
...
\begin{equation}
 ...
 \zlabel{eq:ppath:ellipsradius} 
\end{equation}
The radius given by Equation~\zref{eq:ppath:ellipsradius} can be
...
Section~\zref{sec:fm_defs:los}. The latitude and azimuth angle to
apply in Equations \zref{eq:ppath:curvradius}~-~\zref{eq:ppath:rew}
...
shown in Figure~\zref{fig:ppath:lats}. It could be mentioned that a
...
Figure~\zref{fig:ppath:lats}. This latter definition is preferred
...
between geocentric and geodetic latitude (Equation~\zref{eq:ppath:lats}).
...
%===================
\zlabel{sec:ppath:geodatums}

Table~\zref{tab:ppath:geodatums} gives the equatorial and polar radii
...
\begin{table}[!h]
  \begin{center}
    ...
    \caption{Equatorial and polar radius of reference ellipsoids. Values 
      ...
      derived by the other two values and Equation~\zref{eq:ppath:flattening}.}
    \zlabel{tab:ppath:geodatums}
  \end{center}
\end{table}
