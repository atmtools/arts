...
%  \Rds           & \artsstyle{r}     & radius from the geoid center         \\
...
\begin{description}
  ...
  \item[z] [Vector] The geometrical altitude for each path position. The
   ...
   \item[l\_step] [Vector] The length along the propagation path
     ...
     length between the first and secend point etc. For \artsstyle{np}
...
\levelb{Crossings of pressure surfaces}
%===================
\label{sec:ppath:psurf}

This section deals with how to find the crossing between a geometrical
path and pressure surface which radius is allowed to vary. The
definitions given in Sections~\ref{sec:fm_defs:atmdim} results in that
for a 2D case the radius of a pressure surface varies linearly from
one point of the latitude grid to next. Furthermore, the radius of a
pressure surface in a 3D case has a bi-linear variation on the face of
grid cell and a 2D cross-section through a 3D grid cell will then give
a quadratic variation of the pressure surface radius. A general
function to calculate geometrical crossings with pressure surfaces
must accordingly be able to handle quadratric radius variations.
...
\begin{figure}
 ...
 \begin{minipage}[c]{0.55\textwidth}
  \caption{Quantities used to describe how to find the crossing between a 
    geometrical propagation path and a pressure surface. The angle
    \Lat\ is the angular distance from a reference point on the path.
    The problem at hand is to find \Lat\ for the crossing point.
    The radius of the pressure surface at $\Lat = 0$ is denoted as $r_0$.}
...
point:
\begin{equation}
  \frac{\sin\Theta_p}{\aRds{0}+c_1\Lat+c_2\Lat^2} = 
                                \frac{\sin(\pi-\Lat-\Theta_p)}{\aRds{p}}
\end{equation}
which can be re-written to
\begin{equation}
   \aRds{p} \sin(\Theta_p) = (\aRds{0}+c_1\Lat+c_2\Lat^2) 
...
polynomial equation of order 5:
\begin{eqnarray}
  0 & = & p_0 + p_1 \aLat{1} + p_2 \aLat{2} + p_3 \aLat{3} + p_4 \aLat{4} 
              + p_5 \aLat{5} \\
  ...
  p_3 & = & -(\aRds{0}\cos\Theta_p)/6 - (c_1\sin\Theta_p)/2 + 
                                                c_2\cos\Theta_p \nonumber \\ 
  p_4 & = & -(c_1 \cos\Theta_p)/6 - (c_2 \sin\Theta_p)/2  \nonumber \\ 
  p_5 & = & -(c_2 \cos\Theta_p)/6 \nonumber  
\end{eqnarray}
...
is an intersection with the ground) must have a fractional
...
\funcindex{ppath\_step\_geom\_1d}.  The lowest and highest radius
value along the path step is first determined. If the line-of-sight is
upwards ($\quad |\ZntAng| \leq 90\degree$), then the start point of
...
\artsstyle{ppath\_step\_geom\_1d}.
