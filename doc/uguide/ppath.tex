...
choice of reference ellipsiod for the geoid.
...
\end{itemize}
The definition of the structure to describe propagation paths,
\structindex{Ppath} if found in the first file. The second file
includeds functions to perform calculations to determine propagation
paths. The third file contains the workspace functions releated to
...
\artsstyle{ppath.cc}. Functions to set the geoid radius are found in
\artsstyle{m\_atmosphere.cc}.
...
\wsfindex{ppath\_stepGeometric} and
\wsfindex{ppath\_stepGeometricWithLmax}.

Propagation paths are calculated with the workspace function
\wsfindex{ppathCalc}. The communication between this function and
...
\artsstyle{ppath\_step} as returned to \wsfindex{ppathCalc} and the
...
initialisation is not handled by \artsstyle{ppath\_step\_agenda}. All
fields of \artsstyle{ppath\_step} are set by
\artsstyle{ppath\_step\_agenda}. The field \artsstyle{constant} is set
to be negative by \wsfindex{ppathCalc} before calling
\artsstyle{ppath\_step\_agenda} for the first time. It is set to the
correct value by \artsstyle{ppath\_step\_agenda} at the first call.

The agenda performs only calculations to next crossing of a grid, all
other tasks are performed by \wsfindex{ppathCalc}, with one exception.
...
\artsstyle{ppath\_step}. Beside this, \wsfindex{ppathCalc} checks
...
\artsstyle{Ppath}, where the data type is given inside square
brackets, are:
\begin{description}
  ...
  \item[dim] [Index] The atmospheric dimensionality. This field shall always 
   ...
   \item[np] [Index] Number of positions to define the propagation
     ...
     \artsstyle{np} - 1 as the number of radiative transfer steps along
     the path is this number. If \artsstyle{np} $\leq$ 1, the observed
     spectrum is identical to the radiative background. For cases
     where the sensor is placed inside the model atmosphere and
     \artsstyle{np} = 1, the stored postion is identical to the sensor
   ...
   \item[refraction] [Index] A flag (0 or 1) to indicate if refraction
     has been considered whenm determining the path. A value of 1 means
   ...
   \item[constant] [Numeric] The propagation path constant. Such a
     ...
     initiated to a negative value to indiacte that the constant is
     undefined or not yet set. For cases where the constant applies,
     \wsvindex{ppath\_step\_agenda} sets this constant is set
...
inside parenthesis above each equation, if not stated explicitely. A
part of a geomtrical propagation path is shown in
...
Equation~\ref{eq:ppath:g_geom}, we get that (\funcindex{geomppath\_l\_at\_r})
\begin{equation}
...
decreases towards zero when following the path in the line-of-sigth
...
1 if the viewing direction is towards highes indexes. The direction
