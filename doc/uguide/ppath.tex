...
Equation~\ref{eq:ppath:geomconst} (\funcindex{r2psi})
\begin{equation}
...
(real or imaginary) is (\funcindex{r2l})
\begin{equation}
...
By noting that the zenith angle and the latitude distance to the
tangent point sum up to 90\degree, we have that (\funcindex{r2alpha})
\begin{equation}
  \Lat(\Rds) - \Lat(p_c) = 90\degree - \sin^{-1}(p_c/\Rds)
  \label{eq:ppath:r2alpha}
\end{equation}
