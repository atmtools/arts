...
understand the finer details of ARTS. It turned out that the main obstacle to
handle 2D and 3D cases was to find stable algorithms for the propagation path
calculations, and the sub-sections starting with
Section~\ref{sec:ppath:approach} aim also at documenting the experience of that
development work.
...
The main consideration for using \artsstyle{ppath\_stepGeometric} is to select
...
actually returned is controled by \artsstyle{ppath\_lmax} as above.
...
that is of the type \artsstyle{Ppath} (see Section \ref{sec:ppath:Ppath}). The
...
that is of the same type as \artsstyle{ppath}.
...
\artsstyle{ppath\_step} as returned to \artsstyle{ppath\_calc} and the
...
The field \artsstyle{constant} is set by \artsstyle{ppath\_calc}
...
other tasks are performed by \artsstyle{ppath\_calc}, with one exception.
...
\artsstyle{ppath\_step}. Beside this, \artsstyle{ppath\_calc} checks if
