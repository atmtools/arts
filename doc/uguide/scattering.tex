...
is implemented for 1D and 3D atmodphere, it is strongly recommended to
use it only for 1D, because the Monte Carlo module
(\charef{sec:montecarlo}) is much more appropriate for 3D
...
ARTS package in the tests directory. The file is called
\artsstyle{simpleDOIT.arts}.  
\subsection{DOIT frame}
\label{sec:scattering:frame}

As a first step for a DOIT calculation we have to calculate the
inoming field on the boundary of the cloudbox. This is done using the
workspace method \wsmindex{CloudboxGetIncoming}:
\begin{verbatim}
CloudboxGetIncoming{}
\end{verbatim}
The next step is the initialization of varibles required for a DOIT
calculation using \wsmindex{DoitInit}:
\begin{verbatim}
DoitInit{}
\end{verbatim}
The grid discretisation plays a very significant role in discrete
ordinate method. In spherical geometry the zenith angular grid is of
particular importance [cp. \secref{sec:scattering:grid_opt_interp}. 
The angular discretization is defined in the workspce method
...
\end{verbatim}
For down-looking geometries it is sufficient to define the keywords:
\begin{tabular}{ll}
  \artsstyle{N\_za\_grid}& Number of grid points in zenith angle grid,
  recommended value: 19\\
  \artsstyle{N\_aa\_grid}& Number of grid points in zenith angle grid,
...
This agenda requires the incoming
clearsky field on the cloudbox boundary as an input in gives as
...
frequency. It also performes the transformation from the scattering
...
later. The advantage is that this method needs less memory. But for 1D
...
As a test one can start with a constant radiation field using the merthod
\wsmindex{doit\_i\_fieldSetConst}. 

The iteration is peröformed in the method
...
the sensor positions are outside the cloudbox or in the variable 
\wsvindex{doit\_i\_field1D\_spectrum} if the sensor is inside the
cloudbox. This is done by the workspace method
...
the scattering integral. The methods are selected in the controlfile by
the defining several agendas.

\paragraph{Calculation of the scattering integral:}

To calculate the scattering integral (\eqnref{eq:scattering:scat-int}) the phase matrix \wsmindex{pha\_mat} is required. How
...
have to use here the corresponing method
...
which should be used for limb silumations, for which we need a fine
zenith angle grid resolution to represent the radiation field. The
scattering integral can be calculated on a coarser grid resolution, so
in \wsmindex{doit\_scat\_fieldCalcLimb}, the radiation field is
interpolated on the equidistant angular grids specified in
\wsmindex{DoitAngularGridsSet}. Alternatively, one can use
...
grid resolution about 90°. How the angular grids are defined has been
described in \secref{sec:scattering:frame}.
...
can be solved (\eqnref{eq:scattering:vrte-fs-av}). The workspace
...
sequential update which was described in
\secref{sec:scattering:sequential_update} is applied. The workspace
method \wsmindex{doit\_i\_fieldUpdate1D} does the same without
sequentioal update and is therefore much less efficient because the
number of iterations depends on the number of pressure levels in the
cloudbox.  Other options
...
calculation. This requirement is fullfilled when
\wsmindex{DoitScatteringDataPrepare} of \wsmindex{scat\_data\_monoCalc}
is executed before \wsmindex{doit\_i\_fieldIterate} (see
\secref{sec:scattering:doit_main_agenda}). The work space method
\wsmindex{ext\_matAddPart} and \wsmindex{abs\_vecAddPart} are used to
extract the absorption vector \wsvindex{abs\_vec} and extintiction
matrix \wsvindex{ext\_mat} from the workspace variable
\wsvindex{opt\_prop\_spt}. The gas absorption is added internally. 
\begin{verbatim}
...
\eqnref{eq:scattering:conv_test}. The criterion is specicified by the
keyword \artsstyle{epsilon} which specifies the convergence limit. It
must be given for each Stokes component separately. In
\wsmindex{doit\_conv\_flagAbsBT} the limit must be specified in
Rayleigh Jeans brightness temperatures whereas in
\wsmindex{doit\_conv\_flagAbs} is defined in the basic radiance unit
([W/(m$^2$Hz sr)]. Another option is to perform a least square
convergence test using the workspace method
\wsmindex{doit\_conv\_flagLsq}. Test calculation have shown that this
test is not safe, therefore a least square convergence test should
...
the sensor line of sight. This is done in the workspace method
...
to get sufficientl accurate results in reasonnable computation
time. Therefore only experieced ARTS users should use DOIT for 3D
