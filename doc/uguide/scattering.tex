...
\zlabel{sec:scattering:doit}

\subsection{Radiation field}
\zlabel{sec:scattering:rad_field}
...
\zlabel{sec:scattering:VRTE_sol}
...
\begin{figure}[t!]
  ...
  \zlabel{fig:scattering:iteration_scheme}
\end{figure}
...
\begin{equation}
  ...
  \zlabel{eq:scattering:vrte-fs-av}
\end{equation}
...
\begin{equation}
  ...
  \zlabel{eq:scattering:conv_test}
\end{equation}
is fulfilled,
...
\zlabel{sec:scattering:scattering:solution_rte_scalar}
...
\end{equation}
The \emph{\textindex{scalar radiative transfer}} equation \eqref{eq:rtetheory:SRTE} with a
fixed scattering integral is
\begin{equation}
  \zlabel{eq:scattering:SRTE-Int}
...
\zlabel{sec:scattering:ss_approx}
...
\section{\textindex{Sequential update}}\zlabel{chap:numerical_methods}
\zlabel{sec:scattering:sequential_update}
...
\centering
 ...
 \caption{Schematic of the sequential update (1D) showing the three different parts:  ``up-looking'' corresponds to zenith angles $\ZntAng_\mathrm{up}$, ``limb-looking'' corresponds to $\ZntAng_\mathrm{limb}$ ``down-looking'' corresponds to $\ZntAng_\mathrm{down}$.}
  \zlabel{fig:scattering:seq_update}  
\end{figure}
...
\zlabel{sec:scattering:implementation_DOIT}


\subsection{Grid optimization and interpolation}
\zlabel{sec:scattering:grid_opt_interp}
...
\centering
  ...
  \caption{Interpolation errors for different grid accuracies.
    ...
    directions.}
  \zlabel{fig:scattering:grid_acc}  
\end{figure}
...
oriented (p30) (cf. \secref{sec:clouds:particle_types}).
The top panels of
...
\begin{figure}[htbp]
  ...
  \zlabel{fig:scattering:interp_err}  
\end{figure}
...
particular importance (cf. \secref{sec:scattering:grid_opt_interp}). 
...
\begin{tabular}{ll}
  ...
  \artsstyle{N\_aa\_grid}& Number of grid points in azimuth angle grid,
...
\zlabel{sec:scattering:doit_main_agenda}

The agenda \wsaindex{doit\_main\_agenda} requires the incoming
...
To calculate the scattering integral (\eqnref{eq:scattering:scat-int})
...
\wsmindex{DoitAngularGridsSet} by the generic inputs \artsstyle{Nza} and
\artsstyle{Naa}. Alternatively, one can use 
...
\eqnref{eq:scattering:conv_test}. The convergence limits are specified by the
generic input \artsstyle{epsilon} which specifies the convergence limit. A limit
