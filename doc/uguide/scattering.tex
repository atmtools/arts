...
\levelb{Polarization and Stokes Parameters}
%=====================================================================
\label{sec:scattering:pol_stokes}
...
\begin{equation}
  \label{eq:scattering:intensity_}
 F(\VctStl{x}) = 
...
(Eq.~\ref{eq:scattering:e_field}).
\begin{eqnarray}
  \label{eq:scattering:intensity_from_complex}
...
\begin{eqnarray}
  \label{eq:scattering:stokesparam_I}
  I &=&   E_v E_v^\ast + E_h E_h^\ast \\
  \label{eq:scattering:stokesparam_Q}
  Q &=&   E_v E_v^\ast - E_h E_h^\ast \\
  \label{eq:scattering:stokesparam_U}
  U &=& - E_v E_h^\ast - E_h E_v^\ast \\
  \label{eq:scattering:stokesparam_V}
  V &=& i(E_h E_v^\ast - E_v E_h^\ast)
\end{eqnarray}
Using the amplitude/phase notation from
Eq.~(\ref{eq:scattering:compl_ampl}),
we can rewrite the Stokes parameters as 
\begin{eqnarray}
  \label{eq:scattering:stokesparam_alt_I}
 I &=& a_v^2 + a_h^2\\
  \label{eq:scattering:stokesparam_alt_Q}
 Q &=& a_v^2 - a_h^2\\
  \label{eq:scattering:stokesparam_alt_U}
 U &=&  - a_v a_h \cos(\delta_v-\delta_h)\\
  \label{eq:scattering:stokesparam_alt_V}
...
\begin{eqnarray}
  \label{eq:scattering:tip_of_fieldvec1}
  \tilde{E}_v (t) &=&   a_v \cos\Delta_v \cos(\omega t)
                      + a_v \sin\Delta_v \sin(\omega t)\\
  \label{eq:scattering:tip_of_fieldvec2}
...
\begin{eqnarray}
  \label{eq:scattering:ellipse_parallel}
 \tilde{E}_v (t) &=&   a_0 \sin\beta \cos(\omega t)\\
 \tilde{E}_h (t) &=&   a_0 \cos\beta \sin(\omega t)
\end{eqnarray}
Here $\beta$ must be between $-45\degree$ and $45\degree$: the tip of the
vector $\VctStl{\tilde{E}}$ describes a circle for $\beta = \pm 45\degree$
(circular polarization), oscillates along the $h$-axis for $\beta
= 0$ (linear polarization) and else describes an ellipse. The sense of
rotation is clockwise (???) for positive and counterclockwise (???)
for negative
$\beta$. Since $|\tan\beta|$ is the ratio of the semi-minor and
semi-major axes of the ellipse, $\beta$ is called the ellipticity
angle.
...
Eqs.~(\ref{eq:scattering:ellipse_rotated1})%
-(\ref{eq:scattering:ellipse_rotated2}) with the corresponding terms
in Eqs.~(\ref{eq:scattering:tip_of_fieldvec1})%
-(\ref{eq:scattering:tip_of_fieldvec2}), we get:
\begin{eqnarray}
  \label{eq:scattering:corresp1a}
 a_v \cos\Delta_v &=& a_0 \sin\beta \cos\zeta\\
  \label{eq:scattering:corresp1b}
 a_v \sin\Delta_v &=& a_0 \cos\beta \sin\zeta
\end{eqnarray}
and 
\begin{eqnarray}
  \label{eq:scattering:corresp2a}
 a_h \cos\Delta_h &=& -a_0 \sin\beta \sin\zeta\\
  \label{eq:scattering:corresp2b}
 a_h \sin\Delta_h &=&  a_0 \cos\beta \cos\zeta
\end{eqnarray}
Multiplying Eq.~(\ref{eq:scattering:corresp1a}) with
Eq.~(\ref{eq:scattering:corresp2a}), and
Eq.~(\ref{eq:scattering:corresp1b}) with
Eq.~(\ref{eq:scattering:corresp2b}) and adding up the results, we get
\begin{equation}
...
\begin{equation}
  \label{eq:scattering:sinzetacosbeta}
  \frac{a_v a_h}{a_0^2} \cos(\delta_v-\delta_h)
  = \frac{1}{2}\sin(2\zeta)\cos(2\beta)
\end{equation}
In a similar way, subtracting the product of
Eq.~(\ref{eq:scattering:corresp1b}) with
Eq.~(\ref{eq:scattering:corresp2a}) from the product of
Eq.~(\ref{eq:scattering:corresp1a}) with
Eq.~(\ref{eq:scattering:corresp2b}) and adding up the results, we get
\begin{equation}
  \label{eq:scattering:sinbeta}
...
the squares of Eq.~(\ref{eq:scattering:corresp2a}) and
Eq.~(\ref{eq:scattering:corresp2b}) from the sum of the squares of
Eq.~(\ref{eq:scattering:corresp1a}) and
Eq.~(\ref{eq:scattering:corresp1b}):
\begin{equation}
  \label{eq:scattering:cosbetazeta}
 a_v^2 - a_h^2 =  -a_0^2 \cos(2\zeta)\cos(2\beta)
\end{equation}
Finally, we use the above 3 equations
(\ref{eq:scattering:sinzetacosbeta}), 
(\ref{eq:scattering:sinbeta}) and 
(\ref{eq:scattering:cosbetazeta}) to rewrite the Stokes parameters
(Eqs.~\ref{eq:scattering:stokesparam_alt_I}%
-\ref{eq:scattering:stokesparam_alt_V}) 
as
\begin{eqnarray}
  \label{eq:scattering:stokesparam_alt2_I}
 I &=& a_0^2\\
  \label{eq:scattering:stokesparam_alt2_Q}
 Q &=&  -a_0^2 \cos(2\zeta)\cos(2\beta)\\ 
  \label{eq:scattering:stokesparam_alt2_U}
 U &=& -a_o^2 \sin(2\zeta)\cos(2\beta)\\
  \label{eq:scattering:stokesparam_alt2_V}
 V &=& -a_0^2 \sin(2\beta)
\end{eqnarray}

Thus, we can get the orientation angle $\zeta$ of the ellipse from
\begin{equation}
  \label{eq:scattering:tan2zeta}
 \tan(2\zeta) = \frac{U}{V}
\end{equation}
...
Eq.~\ref{eq:scattering:stokesparam_alt2_Q}, taking into account that
...
\begin{equation}
  \label{eq:scattering:tan2beta}
 \tan(2\beta) = - \frac{V}{(Q^2 + U^2)^{1/2}}  
\end{equation}

\texttt{FIXME: Measuring the Stokes parameters}

\texttt{FIXME: Partial Polarization}
...
\vspace{2ex}
\noindent
The methods \wsmindex{ext\_mat\_partCalc} and
...
%%% TeX-master: "uguide"
%%% End: 
% LocalWords:  Emde ext abs vec pha pnd sca lat lon za aa pt FIXME Eq Eqs scatt
% LocalWords:  mishchenko nonsp partic
