...
oriented non-spherical particles is \cite{sree02}:
\begin{eqnarray}
...
signify the state of linear polarization and the last component $V$
signifies the state of circular polarization.\\
...
\end{eqnarray}
Here $\aSAbVec{p}_i$ is the particle absorption cross section vector
...
aspherical particles as well as for spherical particles. For
aspherical particles this concept has no physical justification as
...
radiative transfer equation. A numerical solution can be obtained using for example an iterative method. The method implementes in ARTS is described in this section. 

\levelc{Basic definitions}
\begin{itemize}
\item{\bf Cloudbox}
It is not necessary to solve the VRTE in the whole atmosphere. In ARTS it is possible to define a region, in which cloud prticles can exist, this region is called the cloudbox. Only in this particular region the VRTE is solved, this saves computation time and memory. 
The cloudbox is defined by its corner points which have to correspond to atmospheric grid points (cp. Figure \ref{fig:scattering:cloudbox}). For a 3D calulation the user has to define lower and upper pressure limit (\aPrs{1} and \aPrs{2}), two latitude limits (\aLat{1} and \aLat{2}) and two longitude limits (\aLon{1} and \aLon{2}). 
...
Of course the Stokes vector also depends on the frequency of the radiation. But in orden to save computation memory, the scattering calculation is only done for one frequency at a time.
...
The starting point of the iteration method is the first guess field  \aIFld{0}. This is restricted by the boundary condition given by the radiation coming from the clear
...
\end{eqnarray}
Here ${\bf C_1}$ and ${\bf C_2}$ are constants which have to be determined. Substituting (\ref{eq:scattering:ansatz}) into (\ref{eq:scattering:vrte_fs_av}) gives the constant  ${\bf C_1}$:
\begin{eqnarray}
...
\end{eqnarray}
${\bf C_2}$ can be determined using the initial condition, which is the radiation at the intersection point ($s=0$) travelling towards the considered gridpoint:
\begin{eqnarray}
...
\end{eqnarray}
The can be resorted to obtain the following form:
\begin{eqnarray}
...
\begin{eqnarray}
{\mathcal I}^{(0)} = \left\{ \bf{I}^{(0)} \left( p_i, \alpha_i, \beta_i, \theta_i, \phi_i\right)\right\}  \qquad  
&\forall& {\rm positions } (p_i,\alpha_i, \beta_i)\\
&\forall& {\rm directions } (\theta_i,\phi_i)
\end{eqnarray}
...
\begin{eqnarray}
|\StoVec^{(m)} \DirFre -  \StoVec^{(m-1)} \DirFre| < {\bf \epsilon}, \qquad 1<m<n
\end{eqnarray}
