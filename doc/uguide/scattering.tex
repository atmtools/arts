...
\wsmindex{doit\_i\_fieldIterate}, which includes
\begin{itemize}
...
\wsaindex{opt\_prop\_part\_agenda}, \wsaindex{ppath\_step\_agenda}, and
\wsaindex{doit\_rte\_agenda}), and
...
workspace method \wsmindex{DoitCloudboxFieldPut}. If the sensor is located
outside the cloudbox, the radiation field at the boundary of the cloudbox is
stored in the interface variable \wsvindex{scat\_i\_p}. If the sensor is inside
the cloudbox, \wsvindex{doit\_i\_field1D\_spectrum} is used as interface
...
\FIXME{Oliver, Manfred: please update this paragraph to current state.}

\subsection{Agendas used in \wsmindex{doit\_i\_fieldIterate}}
