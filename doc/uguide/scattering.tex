...
scheme has been implemented (see Section \ref{sec:montecarlo}). The DOIT
...
(Chapter \ref{sec:montecarlo}) is much more appropriate for 3D
...
equation} \ref{eq:rtetheory:VRTE}
\begin{eqnarray}
  ...
  Equation \ref{eq:rtetheory:VRTE} is explained more detailed in
...
\end{equation}
is fulfilled,
 a solution to the vector radiative transfer equation \ref{eq:rtetheory:VRTE}
has been found:
\begin{equation}
  \IFld^{(N)} = \left\{ \StoVec^{(N)}_{ijklm} \right\}. 
\end{equation}
...
\end{equation}
The \emph{\textindex{scalar radiative transfer}} equation \ref{eq:rtetheory:SRTE} with a
