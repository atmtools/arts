...
\begin{figure}[htbp]
  ...
  \zlabel{fig:interpolation:types}
\end{figure}
...
\end{description}
Figure \zref{fig:interpolation:types} illustrates the different types
for a 2D example. 
...
%-------------------------------------------------------------------------
\zlabel{sec:interpolation:gridchecking}
...
%-------------------------------------------------------------------------
\zlabel{sec:interpolation:gridpos}
...
\item   Finally, note that parameter \artsstyle{extpolfac} plays the
  ...
  \zref{sec:interpolation:gridchecking}. 
\end{itemize}
...
\zref{fig:interpolation:square}).  Then the interpolated value is given
by:
\begin{eqnarray}
  y(t,u)
  &=& (1-t)*(1-u)*y_1 \nonumber \\
...
\zlabel{eq:interpolation:weights}
\end{eqnarray}
...
\begin{figure}
  ...
  \zlabel{fig:interpolation:square}
\end{figure}
...
\zref{eq:interpolation:weights}). The \funcindex{interp} family of functions
performs this step.
