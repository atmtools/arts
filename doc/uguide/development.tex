...
%====================
\zlabel{sec:development:org}
 
ARTS is written in C++ and uses the cross-platform, open-source build system
CMake (\artsstyle{http://www.cmake.org/}). It is organized in a similar
manner as most GNU packages. The top-level ARTS directory is either called
\artsstyle{arts} or \artsstyle{arts-x.y}, where x.y is the release number. It
contains various sub-directories, notably \artsstyle{doc} for documentation,
\artsstyle{src} for the C++ source code, and \artsstyle{aii} for the IDL
interface. The document that you are reading right now, the ARTS Developer
Guide, is located in \artsstyle{doc/uguide}.
...
\begin{quote}
  \footnotesize
  \artsstyle{http://www.cmake.org/cmake/project/about.html}
\end{quote}
...
is done in detail is described in Section \zref{sec:release}. Mostly,
...
\begin{description}
\item[\artsstyle{--disable-debug}:] Removes '-g' from the compiler flags and includes \artsstyle{\#define NDEBUG 1} in
...
\item[\artsstyle{--disable-maintainer-mode}:] 
Disables certain developer-centric features such as generating the user guide and the doxygen html documentation. The maintainer mode is enabled by default when using \artsstyle{autogen.sh} to configure arts.

\item[\artsstyle{--disable-more-warnings}:] 
Compile without \artsstyle{-Werror}. More warnings are enabled by default in maintainer mode except for compilers which are known to throw warnings.

\item[\artsstyle{--disable-optimize}:]
...
\item[\artsstyle{--disable-vectorize}:] 
...
\section{Conventions}
%===================
\zlabel{sec:development:conv}

Here are some general rules for ARTS programming:

\subsection{Numeric types} Never use \artsstyle{float} or
\artsstyle{double} explicitly, use the type \typeindex{Numeric} instead.
This is set by \artsstyle{configure} (to \artsstyle{double} by default).
In the same way, use \typeindex{Index} for all integers. It can take on
positive or negative values and defaults to \artsstyle{long}.
To change the default types, run \artsstyle{configure} with the options
\artsstyle{--with-index-type} or \artsstyle{--with-numeric-type}:
...
\subsection{Container types} Use \artsstyle{Vector} and
\artsstyle{Matrix} for mathematical vectors and matrices (with elements
of type \artsstyle{Numeric}). Use \artsstyle{Array<something>} to
create an array of \artsstyle{something}s.
...
%======================
 \zlabel{sec:development:extending}
...
\item Add a \artsstyle{wsv\_group\_names.push\_back("your\_type")} function to
  the function \artsstyle{define\_wsv\_group\_names()} in \fileindex{groups.cc}.
  The name must be \emph{exactly} like you use it in the source code,
  because this is used to generate interface functions.
\item XML reading/writing routines are mandatory for each workspace variable
  ...
  \begin{enumerate}
  \item Implement an \artsstyle{xml\_read\_from\_stream}
    and \artsstyle{xml\_write\_to\_stream} function. Depending
    ...
    corresponding \artsstyle{.h} file.
  \item Add an explicit instantiation for
    \artsstyle{xml\_read\_from\_file<GROUP>} and
    \artsstyle{xml\_write\_to\_file<GROUP>} to \fileindex{xml\_io\_instantiation.h}.
  \end{enumerate}
\item If your new group does not implement the output operator
  (\artsstyle{operator<<}), you have to add an explicit implementation
  of the \artsstyle{Print} function in \fileindex{m\_general.h} and
...
%---------------------------------------
\begin{enumerate}
\item Create a new subdirectory in \fileindex{tests/}. If your test is closely
...
\item Create your own test controlfile. The filename should start with
  \fileindex{Test} followed by the name of the subdirectory it is located in,
  e.g. \fileindex{tests/DOIT/TestDOIT.arts}. If the subdirectory contains more
  than one test controlfile append a short descriptive text to the end of the
  filename like \fileindex{tests/MonteCarlo/TestMonteCarloGaussian.arts}.
\item Copy all required input files into the subdirectory.
\item Add the necessary checks for your testcase to
  \fileindex{tests/testall.py}.
\item In \fileindex{tests/Makefile.am} add the name of your test to the
  variable \artsstyle{check} and the subdirectory to the variable
  \artsstyle{TESTDIRS}.
...
\item Run: \artsstyle{svn co https://www.sat.ltu.se/svn/rt/arts/trunk arts}
\end{enumerate}
...
\item If you have created a new source file, make it known to SVN by
  running the command \artsstyle{svn add <my\_file>} in the directory where
  the file resides.
  
  In general, when you run \artsstyle{svn update}, it will warn you about
  ...
  files for the current directory with \artsstyle{svn propget svn:ignore .}.
  The list can be edited with \artsstyle{svn propedit svn:ignore .}.
...
\item Make sure that you have saved all your files. Go to the top
  level ARTS directory and run: \artsstyle{svn commit}.
...
\item Tell the other developers about it. The best way to do this is
  to send an email to \artsstyle{arts-dev@www.sat.ltu.se}.
\end{enumerate}
...
the generated Makefiles. Therefore you have to run \artsstyle{autogen.sh}
working directory.


\section{Debugging (use of assert)}
%================================
\zlabel{sec:development:assert}
