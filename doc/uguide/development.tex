...
\begin{description}
\item[\artsstyle{-DCMAKE\_BUILD\_TYPE=Release}:] Removes '-g' from
the compiler flags and includes \artsstyle{\#define NDEBUG 1} in
...
This is set by CMake (to \artsstyle{double} by default).
...
To change the default types, run \artsstyle{cmake} with the options
\artsstyle{-DINDEX=long} or \artsstyle{-DNUMERIC=double}:
...
Generated by \artsstyle{doxymacs-insert-function-comment}. If
arguments are modified by the function you should add `[out]' after
the \artsstyle{\char`\\ param} command, just like for the parameter
\artsstyle{a} in the example below. If a parameter is both input and
...
\item You have to register your file in the file
  ...
  \artsstyle{.cc} file to the list of source files of the artscore
  library. Header files are not added to this list.
\item Go to \artsstyle{src} and run: \artsstyle{svn add <my\_file>} to
...
\item In \fileindex{tests/CMakeLists.txt} add an entry for your test case.
\end{enumerate}
...
have rerun \artsstyle{cmake PATHTOARTS} in that directory.
...
includes \artsstyle{arts.h} either directly or indirectly. Instead, assertions
are turned on and off with the global NDEBUG preprocessor macro, which is
set or unset automatically by the \artsstyle{cmake} build configuration.
Assertions are enabled in the default \artsstyle{cmake} build configuration
(\artsstyle{-DCMAKE\_BUILD\_TYPE=RelWithDebInfo}). They are turned off in the
release configuration (\artsstyle{-DCMAKE\_BUILD\_TYPE=Release}).
