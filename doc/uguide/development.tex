...
\item Then go to the top level arts directory and run: \artsstyle{reconf}.
...
\item In the top-level ARTS directory, run \artsstyle{reconf}.
...
%\fileindex{arts.h}. In the future there will be also a configure option to
...
under the \artsstyle{gdb} debugger. First invoke the debugger:
\begin{quote}
\artsstyle{gdb}
\end{quote}
Then load the executable and set a breakpoint at the \artsstyle{exit}
system call:
\begin{quote}
  \artsstyle{(gdb) file arts}\\
  \artsstyle{(gdb) break exit} (or \artsstyle{break \_\_assert\_fail})
\end{quote}
Now run the program: 
\begin{quote}
  \artsstyle{(gdb) run}
...
when the \artsstyle{exit} system call is invoked, and you will get back the
debugger prompt. Now type:
\begin{quote}
  \artsstyle{(gdb) where} 
\end{quote}  
to see where the crash happened. You can use the \artsstyle{print} command to
look at the contents of variables and you can use the \artsstyle{up} and \artsstyle{down}
commands to navigate the stack. For more information, see the GDB
documentation or type \artsstyle{help} at the prompt of gdb.
...
\artsstyle{up} command of GDB is particularly useful. If you give this a
