...
%, \wsmindex{abs\_coefCalc}, \wsmindex{abs\_coefCalcSaveMemory}. \newline
Methods, low level: \newline
...
\item[CO$_2$-CO$_2$:] Caveat: This is only the self continuum of
  ...
  \citet{richard:12} point out that for molecules with more than two atoms
...
Note that at present only oxygen is supported in ARTS, but that there are plans to 
add capabilities for further molecules.
...
\end{code}
The format of the above data is thusly: \builtindoc{SpeciesTag},
\builtindoc{QuantumNumberRecord}, \builtindoc{Vector}, 
...
property data (\builtindoc{scat\_data\_raw}) and particle concentrations
...
frequencies, wherethe scattering contribution is negligible) in a consistent
...
\wsvindex{scat\_data\_raw}. This can be done each-by-each using
...
use \wsvindex{scat\_data\_raw} to store the particle single scattering data.
