...
After the version tag (ARTSCAT-x), ARTS outputs the number of lines when catalog files are written. This number is not used by reading routines, though.\\
...
The isotopic ratio of the species implemented in ARTS was in general
taken from HITRAN96, even for such ones given by JPL00 and
HITRAN96. For species not present in  HITRAN96 the values were
extracted from JPL00. The isotopic ratios are normalized for some
species in JPL00 to 1, for these cases, the HITRAN96  values of the
major isotope is used to scale the JPL00 isotopic ratio to relative number.


\levelc{ARTS Workspace Variables and Methods}

Both expressions (3.1) and (3.3) are used for the line by line
calculations of the absorption. In order to calculate it certain {\it
  workspace variables} and {\it methods} are used. It has to be
...
coefficients in ARTS.\\
The first one is one is through the workspace method 
\begin{verbatim}
absCalc{}
\end{verbatim}
...
absCalcFromXsec{}
\end{verbatim}
The first one calculates the the cross section {\it x} per tag group,
the second one - the absorption coefficients $\alpha$ from the cross
sections and the volume mixing ratios {\it VMR} at altitude $i$ through
the expression $\alpha_i~=~x_i\times {\it VMR_i}$.\\
The second alternative way of computing the absorption coefficients is
by using the afore mentioned methods explicitly in the control file in
combination with one another
\begin{verbatim}
xsec_per_tgCalc{}
absCalcFromXsec{}
\end{verbatim}
That's why we'll pay special attention to the functioning, the input
...
coefficients will be calculated later on.\\
The monochromatic grid is created through the method
\begin{verbatim}
...
a file through {\verb VectorWriteAscii } {\verb (f_mono) }
 {\verb {""} } .\\
The  pressure grid  is created through the method 
\begin{verbatim}
...
a file - {\verb VectorWriteAscii } {\verb (p_abs) } {\verb {""} } .\\
...
\end{verbatim}
where the profiles are read from the available in ARTS atmospheric
...
workspace variables, it's better to treat them at this place. In our
case of line by line calculations we are not interested in the
continuum description, that's why we should just call a workspace
method to assign empty arrays to the variables:
\begin{verbatim}
...
one accounts for the different lineshape models now in ARTS
available. It's done by setting {\verb shape } to a given model name
:
\begin{description} 
...
\item{{\verb Voigt_Kuntz6 }~:} Kuntz approximation to the Voigt line
  shape with relative accuracy better than $ 2 \times 10^{-2} $;
...
\item{{\verb quadratic }~:} $(\nu / \nu_0)^2$
\end{description}
The {\verb cutoff } can  be set to -1 for no cutoff or to a positive
number for a cutoff at a given frequency in~ [Hz], e.g. 650e9.\\
Thus we have all input workspace variables for {\verb absCalc }
generated. Therefore this part of the subsection is a little bit short
