...
of the calculation methods within ARTS of these quantities is the
second goal.
...
analytically just from the theoretical expressions of quantum dynamics,
statistical physics, and theoretical mechanics. Certain approximations
have to be made to account for different physical phenomena related to
...
negligible compared to the other two. Neverthereless, we will pay a
...
transform theorem from the time to the freqeuncy domain. If we
...
\end{equation}
where $ \tau$ is the mean lifetima of the excited state. Thus the
number of spontayneously emitted photons and in this way the flux of
...
$\gamma_L$ is much greater and because of that, of experimental significance
...
fact. Regardless of this flaw, htis is the only way up to date to model
...
requirements, while this was foundto be not true for some other
algorithms, commonly used for Voight-shape computation. In particular,
...
gas, which generaly speaking describes the energy states distribution
...
a~$-1$ means that no value exist. Fields 22-29 are strings inside quotes
