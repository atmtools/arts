...
\starthistory
  2012-08-28 & Updated to abs\_mat\_per\_species - by Richard Larsson. \\
...
\startsymbolswithunits
  ...
  \aAbsCoef{i} & m$^{-1}$          & abs\_mat\_per\_species  & Absorption matrix of
                                                               absorbing species $i$\\
...
\wsaindex{abs\_mat\_per\_species\_agenda}.  RT functions execute this agenda
...
\wsvindex{abs\_mat\_per\_species}, a tensor with dimensions of absorption species,
...
conditions that are given.  \wsaindex{abs\_mat\_per\_speciesAddOnTheFly} calculates
absorption for all non-Zeeman species which is why \wsaindex{abs\_mat\_per\_speciesAddZeeman}
follows.
...
Each of \wsmindex{abs\_mat\_per\_speciesAddOnTheFly}, 
\wsmindex{abs\_mat\_per\_speciesAddZeemanLBL} and
\wsmindex{abs\_mat\_per\_speciesAddFromLookup} take into account a
possible Doppler shift. However, there is an important difference in
the implementation: For \wsmindex{abs\_mat\_per\_speciesAddOnTheFly} and \wsmindex{abs\_mat\_per\_speciesAddZeemanLBL} the shift
is implemented by simply making the line-by-line calculation on a
shifted frequency grid, so the treatment is exact. For
\wsmindex{abs\_mat\_per\_speciesAddFromLookup}, on the other hand, the
...
Agenda: \wsaindex{abs\_mat\_per\_species\_agenda}. \newline
Variable: \wsvindex{abs\_mat\_per\_species}, which is nitialized in \wsmindex{abs\_mat\_per\_speciesInit}.\newline
Methods: \wsmindex{abs\_mat\_per\_speciesAddOnTheFly} and \wsmindex{abs\_mat\_per\_speciesAddZeemanLBL}. Alternative:
\wsmindex{abs\_mat\_per\_speciesAddFromLookup} (extract absorption from
...
Methods: \wsvindex{abs\_lookupCalc}. Alternative: Load lookup table
...
\wsmindex{abs\_mat\_per\_speciesAddFromLookup}. This will extract
...
The \builtindoc{abs\_mat\_per\_speciesAddFromLookup} method is meant to
be used inside the agenda \wsaindex{abs\_mat\_per\_species\_agenda},
