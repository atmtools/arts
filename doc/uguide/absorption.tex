...
\artsstyle{abs\_cont\_names} identifies the model,
\artsstyle{abs\_cont\_models} contains switches that select different
behavior (for example taking only the lines, or only the continuum
part of a complete model), and \artsstyle{abs\_cont\_parameters} can
contain numerical parameters. 

Yes, the nomenclature for these additional continuum parameters,
particularly \artsstyle{abs\_cont\_models}, is confusing. However,
