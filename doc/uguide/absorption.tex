...
\begin{figure}
 \begin{center}
  ...
  % FIXME: abs_xsec_per_speciesAddLines has friend abs_xsec_per_speciesAddLines2.
  % abs_xsec_per_speciesAddLines does not depend on:
  % line_mixing_data line_mixing_data_lut.
  % these 2 are there from legacy.
  % in ADDITION abs_xsec_per_speciesAddLines2 does not depend on:
  % abs_lineshape
  }
...
\wsmindex{abs\_xsec\_per\_speciesAddLines} (one core method for the
actual line-by-line calculation that requires \wsvindex{abs\_lineshape} and \wsmindex{abs\_lines\_per\_speciesAddMirrorLines}),\newline
\wsmindex{abs\_xsec\_per\_speciesAddLines2} (another core method for the
actual line-by-line calculation that assumes line shape data from the catalogue itself),\newline
...
This requires preparing a \wsvindex{zeeman\_linerecord\_precalc} first to speed up the calculations.
