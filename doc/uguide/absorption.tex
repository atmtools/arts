...
\wsaindex{propmat\_clearsky\_agenda} is called by RT methods when they
...
ARTS is the agenda \wsaindex{propmat\_clearsky\_agenda} (see Figure
...
some way or another compute \wsvindex{propmat\_clearsky}. See the
built-in documentation of the individual methods to learn more. File
\fileindex{agendas.arts}, one of the standard include-controlfiles,
predefines some typical alternatives how
\wsvindex{propmat\_clearsky\_agenda} can be set for different
purposes.

The method \wsmindex{propmat\_clearsky\_agendaSetAutomatic} constructs
a \wsvindex{propmat\_clearsky\_agenda} that contains all required
absorption methods. The list of methods is determined automatically
from the species present in \wsvindex{abs\_species}. This agenda can
then be used for RT calculation with one-the-fly absorption or the
preparation of an absorption lookup table. The method
\wsmindex{propmat\_clearsky\_agendaSetAutomaticForLookup} constructs a
\wsvindex{propmat\_clearsky\_agenda} that uses a lookup table in RT
calculations.
...
absorption: \wsvindex{propmat\_clearsky\_agenda}.
...
Variable: \wsvindex{propmat\_clearsky}, which is initialized in \wsmindex{propmat\_clearskyInit}.\newline
Methods: 
\wsmindex{propmat\_clearskyAddCIA} computes collision-induced absorption,\newline
\wsmindex{propmat\_clearskyAddConts} computes legacy continua or full absorption models,\newline
...
Recommendation:
\wsmindex{propmat\_clearsky\_agendaSetAutomatic} can be used to setup lookup table free calculations, and\newline
\wsmindex{propmat\_clearsky\_agendaSetAutomaticForLookup} can be used to setup calculations using the lookup table\\
%---------------------------------------------------------------------
\arabic{rownum}b &
...
is \wsmindex{propmat\_clearskyAddConts}. Users normally do not have to
call this method explicitly, since it is used implicitly by higher
level methods, such as \wsmindex{propmat\_clearsky\_fieldCalc} or the
RT methods via \wsvindex{propmat\_clearsky\_agenda}.
...
read. Finally, use WSM \wsmindex{propmat\_clearskyAddCIA} in
...
The empirical absorption cross sections are calculated in the method \wsmindex{propmat\_clearskyAddXsecFit}.
...
field into the model. This can be done either by calling \wsmindex{MagFieldsCalcIGRF} (earth only model),
or by manually supplying raw magnetic field measurements on a gridded field and use
the combination of \wsmindex{MagRawRead} and one of \wsmindex{MagFieldsCalc}, \wsmindex{MagFieldsCalcExpand1D},
or \wsmindex{MagFieldsFromAltitudeRawCalc}.  Since altitude and not pressure is the natural coordinate for magnetic fields,
it is required that \wsvindex{z\_field} and/or the \wsvindex{refellipsoid} are defined.  Note that it is possible but absolutely
...
As an example, \wsmindex{abs\_linesSetLinemixingLimit} sets the line mixing limit for all lines in \wsvindex{abs\_lines}.
...
used by storing all of the data to a directory using \wsmindex{WriteBuiltinPartitionFunctionsXML}
