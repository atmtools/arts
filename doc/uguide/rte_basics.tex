...
\item[Particles] This second class treats all matter causing significant
  ...
  type are stored in \wsvindex{scat\_data\_raw}.
\end{description}
...
internally from \builtindoc{scat\_data\_raw}.

% If propmat_particles_agenda is introduced:
%As workspace variables, \aAbsMat{a}\ and \aExtMat{p}\
...
and \wsaindex{blackbody\_radiation\_agenda}. The vector \aSrcVec{e}\ is
...
propagation matrix covers also scattering and \wsvindex{scat\_data\_raw}
must also contain such data.
%As a consequence, the workspace
