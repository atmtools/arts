...
\starthistory
 130218 & A first version (Patrick Eriksson).\\
\stophistory
...
general unpolarised, and if not scattering has to be considered it is
sufficient to only include the first Stokes components in the simulations (ie.\
\builtindoc{stokes\_dim} set to 1). In this case, to include higher order
Stokes components results only in slower calculations. Simulations where
\builtindoc{stokes\_dim} is two or higher are denoted as \textindex{vector
  radiative transfer}, while \textindex{scalar radiative transfer} refers to
the case when only the first Stokes component is considered.
...
%As Equation~\ref{eq:VRTE0} indicates, source mechanisms are emission and
%scattering.
...
\item[Absorbing species] This class covers atmospheric matter for which
  ...
  atmospheric fields are gathered into \wsvindex{abs\_species\_field}. The unit
...
\item[Particles] This second class treats all matter causing significant
  ...
  \wsvindex{pnd\_field}. The radiative properties of each particle type are
  stored in \wsvindex{scat\_data\_raw}.
\end{description}
...
magneto-optical effects). As workspace variables, \aAbsMat{a}\ and \aExtMat{p}\
are denoted as \wsvindex{propmat\_clearsky} and \wsvindex{propmat\_particles},
respectively. These variables are obtained by two agendas:
\wsaindex{propmat\_clearsky\_agenda} and \wsaindex{propmat\_particles\_agenda}.
...
\end{equation}
where $B$ is the Planck function (a scalar value), describing blackbody
radiation. The related workspace variables are \wsvindex{blackbody\_radiation}
and \wsaindex{blackbody\_radiation\_agenda}. The vector $\VctStl{j}_e$ is
denoted as \wsvindex{emission\_vector}, and is calculated by
\wsaindex{emission\_vector\_agenda}.
...
\end{equation}
where \VctStl{s} can be seen as the emission source vector, defined as
\begin{equation}
  \VctStl{s} = [B,0,0,0]^T.
\end{equation}
Hence, as only the first element of \VctStl{s}\ is non-zero, the absorption
...
variable \wsvindex{absvec\_particles} must be introduced, that is returned by
\builtindoc{propmat\_particles\_agenda} together with \aExtMat{p}.
...
absorption vector due to particles (\builtindoc{absvec\_particles}).
...
%\section{Including the sensor}
%%==============================================================================
%\label{sec:ppp}
%
%The equations above treat a single frequency and a single direction,
...
%
%The Stokes vector is as workspace variable denoted as \wsvindex{iy}. The
%variable can hold \StoVec\ for a range of frequencies, but covers only
%radiative transfer along a single propagation path. Please note the distinction
%to \wsvindex{y} (see Section~\ref{sec:rte}), that holds radiances from several
%frequencies and pencil beams, possibly weighted with some instrument responses.
