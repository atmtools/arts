...
\leveld{Transpose:}
The function \artsstyle{transpose} works a bit different for Sparse than
for Vector and Matrix. This is due to the fact that we don't have any
views for Sparse. Therefor the transposed Sparse with the right dimensions
has to be initialized before calling the function.
\begin{verbatim}
...
mult(f,a,g);        // f = a * g
\end{verbatim}
Tthe result is put in the first argument, consistent with the Matrix class.
