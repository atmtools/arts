...
If this is not realized for some use-cases, this is considered a deficiency and can freely be fixed.
Note that neither of these C++ standards are actually available as of yet (2023-02-28), so the
...
Again, it is our intent that we should expose as much of these types behavior in the interface to our
own classes as possible.  However, a lot of this is missing, so do feel free to add it.  However, do
consult with others because it is as of yet not clear to us if \shortcode{std::vector::push_back}
should push back a single element and only work for rank 1 types, or an entire left-most dimensional
column so that it works for arbitrary ranks.

\subsection{Defining the template classes}
The data owning object for the type that knows its size at compile time is called
\begin{code}
...
The `reduce\_rank' example requires that `I' and `J' are less than the rank of the tensor,
and that the dimensions not kept are all 1-long in the higher ranked type.  Note that the
higher ranked type will be left in a valid to destroy, but not valid to access state.
