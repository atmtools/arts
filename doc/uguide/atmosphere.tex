...
atmospheric species (\wsvindex{abs\_species\_field}). The content and units of
\builtindoc{abs\_species\_field} are discussed in
...
the \textindex{cloud box}. The cloud box acts as an additional atmospheric
...
\wsvindex{pnd\_field}, see further Section~\ref{sec:rteq:absspecies}.
...
\wsvindex{wind\_w\_field}. As described above, one, two or all of these
...
(\theory, Sec.~\ref{T-sec:at:zeeman}), also the \textindex{magnetic field} must
...
specific point is handled by \wsvindex{rte\_mag}. For this variable the three
