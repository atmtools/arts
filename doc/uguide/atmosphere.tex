...
functions \wsmindex{AtmosphereSet1D}, \wsmindex{AtmosphereSet2D} and
...
\item[\textindex{3D}\,\,\,] In this, the most general, case, the
  ...
  true atmosphere (Figures~\zref{fig:fm_defs:3d} and
  \zref{fig:fm_defs:3dcross}). A \textindex{spherical coordinate
  ...
  in Section~\zref{sec:fm_defs:altitudes}. The valid range for
...
\item[\textindex{1D}\,\,\,] A 1D atmosphere can be described as being
  spherically symmetric (Figure~\zref{fig:fm_defs:1d}). The term 1D is
  ...
  interest (see also Section~\zref{sec:ppath:Ppath}). This latter
  ...
  atmosphere is shown in Figure~\zref{fig:fm_defs:1d}.
  
\item[\textindex{2D}\,\,\,] In contrast to the 1D and 3D cases, a 2D
  atmosphere extends only inside a plane
  (Figure~\zref{fig:fm_defs:2d}). A spherical coordinate system is
...
\begin{figure}[!p]
 \begin{center}
  ...
  \caption{Schematic of a 3D atmosphere. Plotting symbols as in 
    Figure~\zref{fig:fm_defs:2d}. Radii and fields are here defined to
...
\begin{figure}[!p]
 \begin{center}
  ...
  \caption{A latitudinal, or longitudinal, cross section of a 3D atmosphere. 
    Plotting symbols as in Figure~\zref{fig:fm_defs:1d}. Radii and
...
\item[Geometrical altitude\index{geometrical altitude}] The term
  ...
  a point and the geoid (Section~\zref{sec:fm_defs:geoid}) along the
  vector to the centre of the coordinate system
  (Equation~\zref{eq:fm_defs:zsurface}). Hence, the geometrical altitude
...
Section~\zref{sec:fm_defs:atmdim}.  The latitude angle is throughout
...
latitudes (see Section~\zref{sec:ppath:geoid}). This is in accordance
...
Section~\zref{sec:fm_defs:atmdim}.
...
Section~\zref{sec:fm_defs:ppaths}. The volume (or area for 2D) covered
...
Section~\zref{sec:fm_defs:geoid}) has for 1D the size $[1,1]$.
...
Figures~\zref{fig:fm_defs:1d}~-~\zref{fig:fm_defs:3dcross}), or for 1D
at each pressure level (thick dots in Figure~\zref{fig:fm_defs:1d}).
...
coordinate, Section~\zref{sec:fm_defs:altitudes}), and along the
...
Section~\zref{sec:fm_defs:rte}). Cloud box calculations can be
...
longitude direction (Figure~\zref{fig:fm_defs:3dcross}). The latitude
