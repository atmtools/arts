...
\item[Geometrical altitude\index{geometrical altitude}] The term geometrical
  ...
  reference ellispoid (Section \ref{sec:fm_defs:geoid}) along the vector to the
...
\item[\textindex{1D}\,\,\,] A 1D atmosphere can be described as being
  ...
  sensor is considered to be placed at $(\Lat,\Lon)=(0,0)$. Further, the sensor
  is assumed to by directed towards the North pole, corresponding to an azimuth
  angle of 0\degree. A 1D atmosphere is shown in Figure \ref{fig:fm_defs:1d}.
...
\item[\textindex{2D}\,\,\,] In contrast to the 1D and 3D cases, a 2D atmosphere
  ...
  in detail, this case be seen as observations restricted to the plane passing
  the North and South poles and $(\Lat,\Lon)=(0,0)$. A spherical coordinate
  system is not needed here and a polar system\index{polar coordinate system},
  consisting of a radial and an angular coordinate, is used. The angular
  coordinate is denoted as latitude, and matches the 3D latitude in the range
...
definition of geometric altitudes above. For 3D, a \textindex{longitude} grid
(\wsvindex{lon\_grid}) must also be specified. Valid ranges for latitude and
longitude values are given in Section \ref{sec:fm_defs:atmdim}. If the
longitude and latitude grids are not used for the selected atmospheric
dimensionality, then the longitude grid (for 1D and 2D) and the latitude grid
(for 1D) must be set to be empty.
...
method \wsmindex{z\_fieldFromHSE}. This method is presently valid only for the
...
ellipsoid, \wsvindex{refellispoid}). Reference ellipsoids are discussed further
in Section \ref{sec:fm_defs:geoid}. The lower boundary of the atmosphere is
