...
In contrast to above, mass conent is here affecting both $\no$ and $\la$ and
the derivative expressions become more lengthy:
\begin{eqnarray}
  \frac{dn}{dw} &=& \left( \frac{dn}{d\no}\frac{d\no}{d\la} + 
                        \frac{dn}{d\la} \right) \frac{d\la}{dw} \nonumber\\
 &=& \left( \frac{dn}{d\no} \frac{w\ga\eterm\la^{\eterm-1}}{a\Gamma(\eterm)} + 
                        \frac{dn}{d\la} \right) 
   \left(a\ntot\frac{\Gamma(\eterm)}{\Gamma(\rterm)}\right)^{\ga/b} \frac{(-\ga)}{b}w^{-(\ga/b+1)}.
\end{eqnarray}
and
\begin{eqnarray}
  \frac{dn}{d\ntot} &=& \left( \frac{dn}{d\no}\frac{d\no}{d\la} + 
