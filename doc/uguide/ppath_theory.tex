...
replaced with the three first terms of their Taylor expansions maintaining a
high accuracy. That is,
\begin{eqnarray}
  \cos \Lat & \approx & 1 - \Lat^2 / 2 + \Lat^4 / 24  \nonumber \\
  \sin \Lat & \approx & \Lat - \Lat^3 / 6 + \Lat^5 / 120 \nonumber
\end{eqnarray}
Equation \ref{eq:ppath:psurf1} becomes with these replacements a
polynomial equation of order 6:
\begin{eqnarray}
...
It was first tested to use the analytical expressions above to calculate the
length between a point and the crossing of some radius, latitude or longitude.
However, the expressions include the trigonometric functions and the
squaring of radii, which results in a high sensitivity to numerical
inaccuracy. A second, less advanced, algorithm was then implemented and tested,
that is described below. It turned out that the two algorithms gave similar
calculation speed for non-spherical cases and the second, more stable,
algorithm was the selected.
