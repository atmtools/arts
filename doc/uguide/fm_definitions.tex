...
\starthistory
  030306 & Sections on atmosphere, sensor and RT written by Patrick Eriksson.\\
\stophistory
...
here denoted as \textindex{vector radiative transfer}, while
\textindex{scalar radiative transfer} refers to the case when only
...
\subsection{Sensor polarization\index{sensor polarization}}
%===================
...
\item[Interior of cloud box] If the sensor is situated inside the
  ...
  by \artsstyle{iy\_cloudbox\_agenda} and can requiere some special
...
If there is an interception of the propagation path, the agenda
\wsaindex{iy\_surface\_agenda} is supposed to provide the upwelling
radiation at the interception point along the propagation path. The
upwelling radiation is provided by setting \artsstyle{iy}. There exist
a standard method for \artsstyle{iy\_surface\_agenda}, named as
\wsmindex{surfaceCalc}. The primary input to this method are the
workspace variables \wsvindex{surface\_emission},
\wsvindex{surface\_los} and \wsvindex{surface\_rmatrix}. These
