...
%
\startsymbols
  \Ind           & \verb|i|          & vector/matrix/tensor index           \\
  \aInd{\Lat}    & \verb|i_alpha|    & the \Ind:th latitude                 \\
  \VctLng        & \verb|n|          & vector length or size of matrix/tensor for a dimension \\
  \aVctLng{\Lat} & \verb|n_alpha|    & length of the latitude grid          \\
  ...
  \Planck        & \verb|source|     & Planck function                      \\
  \Lat           & \verb|alpha|      & latitude                             \\
  \Lon           & \verb|beta|       & longitude                            \\
  \ZntAng        & \verb|psi|        & zenith angle                         \\
  \AzmAng        & \verb|omega|      & azimuthal angle                      \\
  \MpiVct        & \verb|mpi|        & vector with monochromatic pencil beam values \\
  \MsrVct        & \verb|y|          & measurement vector                   \\
  \SnsMtr        & \verb|h|          & sensor and data reduction transfer matrix \\
...
by the grids is denoted as the \qindex{model atmosphere}.
...
are rank-3 tensors with dimension
...
The \qindex{geoid} is an imaginary surface used as a
...
(\verb|z_ground|) is a matrix of the same size as the geoid
...
variable \verb|e_ground|, holding the ground emission factor
...
\item[Blackbody ground\index{blackbody ground}] If the ground as a
  ...
  respectively), all values of \verb|e_ground| must be 1.
\end{description}
...
scattering shall be considered is denoted as the \qindex{cloud box},
...
box is activated by setting the variable \verb|cloudbox_on| to 1. The
limits of the cloud box are stored in \verb|cloudbox_limits|.
...
\verb|antenna_dim|, \verb|psi_block_grid|, \verb|omega_block_grid| and
\verb|Hb|. These variables are presented separately below in this
...
(see Section~\ref{sec:fm_defs:ppaths}). An obviously incorrect choice is to
...
is applied. This is the case as the sensor can for 2D and 3D be placed
outside the covered latitude and longitude ranges, thus outside the
defined geoid and the geometrical altitude is undefined. See
Section~[**] for hints on how to simply set the sensor position.
...
line-of-sight of \qindex{propagation path}s is defined in the
...
\qindex{meridian plane}, that is, the plane going through the north
...
\verb|antenna_dim| is 1, and the variable \verb|omega_block_grid| has
yet no importance (but it must be initiated, to be empty, before
calling \verb|retCalc|). The presentation below assumes that the 2D
...
concern and $\Delta\aZntAng{i}$ is value $i$ of \verb|psi_block_grid|.
With other words, \verb|psi_block_grid| and \verb|omega_block_grid|
give the grid (relative to the sensor line-of-sight) for the
calculation of the intensity field that will be weighted with the
antenna response. For cases with 1D antenna patterns,
\verb|omega_block_grid| must be set to be an empty vector.
...
the \qindex{measurement sequence}. That is, a measurement sequence
covers all spectra recorded at all considered sensor positions
(\verb|sensor_pos|). A measurement sequence consists of one or several
\qindex{measurement block}s. The observations inside the various
...
\end{equation}
where \aFrq{i} is element $i$ of \verb|f_grid| and $n_\Frq$ the length of
...
lowermost spectrum, and the zenith angles in \verb|psi_block_grid|
...
workspace variable \verb|emission_on|. In some cases it is of interest
...
option selected by \verb|emission_on|.
...
\qindex{propagation path}. The forward direction of the propagation
...
Propagation paths are described by a set of points on the
path, and the distance along the path between the points. These
quantities, and a number of auxiliary variables, are stored together in
...
sensor and following the path backwards by some \qindex{ray tracing}
...
Algorithm~\ref{alg:fm_defs:rteCalc}) equals \verb|i_space|
