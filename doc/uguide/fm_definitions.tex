...
degree of complexity, the \textindex{atmospheric dimensionality}. There
...
set of layers with equal pressure, the pressure grid (\wsvindex{p\_grid}).
...
A \textindex{latitude} grid (\wsvindex{alpha\_grid}) must be specified
...
\textindex{longitude} grid (\wsvindex{beta\_grid}) must also be specified.
...
pressure surfaces (\wsvindex{z\_field}), the temperature (\wsvindex{t\_field})
and considered atmospheric species (\artsstyle{????}). The atmospheric
...
valid for the field of geometrical altitudes (\artsstyle{z\_field}).
...
of the ground. The \textindex{ground emission} is described by the
variable \wsvindex{e\_ground}, holding the ground emission factor
...
\item[Blackbody ground\index{blackbody ground}] If the ground as a
  ...
  respectively), all values of \artsstyle{e\_ground} must be 1.
\end{description}
...
\artsstyle{alpha\_grid} and \artsstyle{beta\_grid}). A path point is
