...
points are also included if such exist. More points can also be added
to the propagation path, for example, by setting an upper limit for
the distance along the path between the points.
...
\item[Space] When the propagation path starts at the top of the
  ...
  directed towards the sun, when the radiative background shall be set
  to mimic the solar radiation. The radiative background at the top of
...
\item[Surface of cloud box] 
\item[Interior of cloud box] 
\end{description}


\levelc{The agenda for radiative transfer}
%===================
...
\begin{algorithm}[!b]
 ...
 \caption{Typical structure of a workspace method to 
   ...
   method considering emission. [* Complete the algorith when
   absorption is ready. *]}
