\graphicspath{{Figs/fm_definitions/}}

\chapter{The forward model: concepts, definitions and overview}
 \label{sec:fm_defs}
...
\subsection{Atmospheric grids and fields}
%===================
...
operational ARTS version (1.0.x) can be used to calculate the needed
...
\item[Surface of cloud box] For cases when the propagation path enters
  ...
  \wsaindex{iy\_cloudbox\_agenda}. How to perform calculations
...
\item[Interior of cloud box] If the sensor is situated inside the
  ...
  by \artsstyle{iy\_cloudbox\_agenda} and can require some special
...
The scattering inside the cloud box can be handled by several methods.
The different modules are here only quickly outlined, details are
given in seperate chapters.


\subsection{DOIT -- the discrete ordinate iterative module}
%==============================================================================
\label{sec:fm_defs:doit}

In the DOIT module, the complete intensity field inside the cloud box
is determined. This means that the intensity from a discrete set of
directions is calculated at all positions of the grid mesh inside the
cloud box. The intensity field is determined in an iterative manner,
and this calculation must be performed before calling \artsstyle{RteCalc}.
The DOIT module is described in Chapter \ref{sec:scattering}.


\subsection{MC -- reversed Monte Carlo scattering module}
%==============================================================================
\label{sec:fm_defs:mc}

The approach in the MC module is to follow propagation paths
backwards, with scattering angles and path lengths randomly chosen
from probability density functions determined by the scattering phase
function, and a scalar extinction coefficient, respectively. The phase
matrices for every scattering event and scalar extinction are then
sequentially applied to the source Stokes vector to give the Stokes
vector contribution for each photon. The tracking of the propagation
paths starts at the cloud box boundary and continues backwards to the
point of emission or the boundary of the cloud box (whatever comes
first). See further Chapter \ref{sec:montecarlo}.
...
% LocalWords:  cloudbox mblock mblock binning Hotelling lat
