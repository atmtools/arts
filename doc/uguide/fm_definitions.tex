...
The temperature of the ground\index{ground temperature} is treated to
be the same as for the temperature field for the altitude of the
ground. The \qindex{ground emission} is described by the variable
...
turned off when determing the radiation field going into the cloud
box.


\levelb{Compulsary sensor and data reduction variables}
%==============================================================================
...
imaginery sensor with infinite frequency and angular resolution. The
...
with 2D antenna patterns (a feature not yet implemented, see below),
the workspace variable \verb|omega_block_grid| is also compulsary.
These variables are presented below, where they are first discussed
individually and then later together when it is described how
different measurement sequences are modelled in the most practical way
(section~\ref{sec:fm_defs:measseqs}). Remaining sensor workspace
variables are described last in Section~\ref{sec:fm_defs:sensor2}.
...
comprehensive term for the response, beside polarisation effects, of
all sensor parts that affect how the field of monochromatic pencil
beam intensities are translated to the recorded spectrum. For example,
the antenna pattern, the side-band filtering and response of the
spectrometer channels are normally the most important charcteristics
for a microwave heterodyne radiometer. Any processing of the spectral
data that takes place before the retrieval is denoted as \qindex{data
  reduction}. The most common processing is to represent the original
spectra with a smaller set of values, that is, a reduction of the data
size. The most common data reduction techniques is binning and
Hotelling transformation by an eigenvector expansion (Section~[**]).
...
(\verb|Hb|), where the subscript $b$ stands for block (see
Section~\ref{sec:fm_defs:measseqs}). There are several workspace
...
options are 1 and 2, standing for 1D and 2D, respectivly. A 1D antenna
...
\verb|antenna_dim| is 1. The presentation below assumes that 2D option
is implemented.
...
$\Delta\aZntAng{i}$ is value $i$ of \verb|psi_block_grid|.
With other words, the values of \verb|psi_block_grid| and
\verb|omega_block_grid| give the calculation grid to use when the
intensity field is weighted with the antenna response. For 1D antenna
patterns, \verb|omega_block_grid| is assumed to be a vector of length
1 with the value 0 (any existing values are ignored).  The transfer
matrix \verb|Hb| is applied on the monochromatic pencil beam spectra
for each sensor position as described in
Section~\ref{sec:fm_defs:measseqs}.


\levelc{How to model a different measurement sequences?} 
%===================
\label{sec:fm_defs:measseqs}

\begin{equation}
  \MsrVct = \left[ \begin{array}{c} \aSnsMtr{b}\MpiVct_1 \\ 
                                    \aSnsMtr{b}\MpiVct_2 \\
                                    \vdots                  \\
                                    \aSnsMtr{b}\MpiVct_n \end{array} \right]
...
% seperated. Note the macro \MpiVct in symbol_defs.
%
