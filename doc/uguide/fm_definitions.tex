...
ARTS, as the way to use a forward model programme normally differs
...
cloud box are stored in \wsvindex{cloudbox\_limits}. These are the only
...
\artsstyle{za\_block\_grid}, \artsstyle{aa\_block\_grid} and
...
\wsvindex{antenna\_dim}, \wsvindex{za\_block\_grid} and
\wsvindex{aa\_block\_grid} are the compulsory ones.
...
integrating the azimuthal response for each zenith angle. ARTS does
not handle 2D antenna patterns yet, therefore, the only allowed choice
for \artsstyle{antenna\_dim} is 1, and the variable
\artsstyle{aa\_block\_grid} has yet no importance (but it must be
...
\artsstyle{za\_block\_grid} and \artsstyle{aa\_block\_grid}. For
...
\artsstyle{za\_block\_grid}.  With other words,
\artsstyle{za\_block\_grid} and \artsstyle{aa\_block\_grid} give
...
For cases with 1D antenna patterns, \artsstyle{aa\_block\_grid}
...
\wsvindex{za\_block\_grid} (and possibly
\wsvindex{aa\_block\_grid}), as described in
...
the same vector. The order of the angles inside \artsstyle{za\_block\_grid}
and \artsstyle{aa\_block\_grid} is followed when looping the pencil beam
...
lowermost spectrum, and the zenith angles in \artsstyle{za\_block\_grid}
...
{\bf THIS IS JUST A FIRST TRY. THis PART WILL BE MODIFIED.}
...
Propagation paths are determined by the function \wsfindex{ppathCalc}.
...
include \wsfindex{ppath\_step\_geometric} and
\wsfindex{ppath\_step\_refraction}.
