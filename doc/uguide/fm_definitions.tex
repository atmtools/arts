...
\begin{figure}[!t]
 \begin{center}
  ...
  \caption{Schematic of a 1D atmosphere. The atmosphere is 
    ...
    black dots indicate how the atmsopheric fields are defined. The
    fields are specified by a value for each pressure surface. The
    extension of the cloud box is either from the ground up to a
    pressure surface, or between two pressure surfaces (which is the
...
\begin{figure}[!t]
 \begin{center}
  ...
  \caption{Schematic of a 2D atmosphere. Plotting symbols as in 
    Figure~\ref{fig:fm_defs:1d}. The radii (for the geoid, the ground
    ...
    variation. The vertical extension of the cloud box is as for 1D.
    The horisontal extension is between two latitude grid points.  }
...
\begin{figure}[!t]
 \begin{center}
  ...
  \caption{Schematic of a 3D atmosphere. Plotting symbols as in 
    Figure~\ref{fig:fm_defs:1d}, but the cloud box is here not
    included. Radii and fields are here defined to vary linearly along
    the latitude and longitude grid points. This means that surfaces
    (such as the radius of the ground) have a bi-linear variation
    inside the area limited by two latitude and longitude grid values,
    while the atmospheric fields have a tri-linear variation inside
    the grid cells. A 3D cloud box is defined as for 2D, but has also
...
box is activated by setting the variable \wsvindex{cloudbox\_on} to 1.
The limits of the cloud box are stored in \wsvindex{cloudbox\_limits}.
It is recommnded to use the method \wsmindex{CloudboxOff} when no
...
Section~\ref{sec:fm_defs:sensor2}. Section~\ref{sec:fm_defs:rte}
...
angles are counted clockwise and 0\degree means that the viewing or
propagation direction is north-wise.  Hence, +90\degree means that the
...
by other means, as described in
Section~\ref{sec:fm_defs:howtomeasseq}.
...
for such observations. However, simulations of transmission
...
Conversion to brightness temperature is treated in Section~[**]. 
...
A pencil beam path through the atmosphere to reach a position from a
...
intersection with the cloud box or the ground. The propagtaion path
...
\begin{figure}[!b]
 \begin{center}
  ...
  \caption{Propagation path examples for a 2D atmosphere. The atmosphere 
    ...
    emphasized. No value for the maximum path step length was applied.
    Note the tangent point inserted for the uppermost propagation
    path, plotted as $\oplus$.}
...
\item[Interior of cloud box] If the sensor is situated inside the
  ...
  intensity field of the cloud box at the position of the sensor.
  This case requieres some special considerations, as described in
...
radiative transfer calculation along the given propagation path
(Algorithm~\ref{alg:fm_defs:rte_agenda}). The determination of the
radiative background is not part of the task of
...
\artsstyle{RteCalc}, as long as \wsvindex{i\_rte} and \artsstyle{ppath}
are set correctly.

\begin{algorithm}[!b]
 \begin{algorithmic}
   \STATE{determine the temperature along the propagation path by
     interpolating \wsvindex{t\_field}}
   \FORALL{propagation path steps}
   \STATE{absorption ??}
    \FORALL{frequencies}
    \STATE{solve the raditaive transfer problem by an expression such 
      as Eq.~\ref{eq:fm_defs:rte_step}}
    \ENDFOR
   \ENDFOR
 \end{algorithmic}
 \caption{Typical structure of a workspace method to 
   be part of \artsstyle{rte\_agenda}. The example is valid for a
   method considering emission.}
...
next. The simplest expression that can be used in this way is
\begin{equation}
...
\wsvindex{ground\_refl\_agenda}. The details of this agenda and the
associated variables are described in Section~[**].

Three main assumptions regarding the ground properties can be made:
1. The ground acts as a blackbody. 2. The ground reflects radiation,
but only in the specular direction. 3. The ground is a reflecting and
scattering surface. Figure~\ref{fig:fm_defs:ground_refl} gives a
schematic description of case 3, the most complex situation. The slope
of the ground is considered when calculating the specular direction of
ground reflections, as shown in Figure~\ref{fig:fm_defs:ppath_cases2}.

\begin{figure}[!t]
 \begin{center}
  \includegraphics*[width=0.7\hsize]{Figs/fm_definitions/ground_refl}
  \caption{}
...
%The transmission is provided as the optical thickness (Equation~[**]).
