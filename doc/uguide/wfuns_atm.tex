...
\section{Calculation approaches}
 \zlabel{sec:wfuns:approaches}
  ...
  calculations. By Equation \zref{eq:formalism:kx2},
  \begin{eqnarray*}
  ...
  can be expressed as (see Section \zref{sec:wfuns:species} for
  ...
  The absorption and source function in Equation \zref{eq:wfuns:taylor}
  ...
  problem, Equation \zref{eq:wfuns:taylor} is expanded one step further
  \begin{equation}
  ...
  in Section \zref{sec:wfuns:sourceloswfs}. The term $\partial\iv/
  \partial \kappa$ is denoted as absorption LOS WFs and is discussed
  in Sections \zref{sec:wfuns:absloswfs} and
  \zref{sec:wfuns:absloswfs2}. These terms are treated separately as
  ...
  is the temperature.  See further Section \zref{sec:wfuns:temp}. For
...
\section{Absorption LOS WFs with emission}
 ...
 used in Section \zref{sec:rte}. The calculation approach used for the
 LOS WFs is ``inspired'' by the corresponding work in
 \citet{master00}.


 \subsection{Single pass}
 \zlabel{sec:wfuns:single}
 ...
 instead discussed in Section \zref{sec:wfuns:limb} and
 \zref{sec:wfuns:down}, respectively.
 ...
 \begin{figure}[t]
  \begin{center}
   ...
   \caption{The terms used for the derivation of line of sight weighting
            ...
            \zref{fig:rte:los}.}
   \zlabel{fig:wfuns:single}  
  \end{center}
 \end{figure}

 By rewriting Equation \zref{eq:rte:rteprod}, the monochromatic pencil beam
 intensity can be expressed in the following ways (see Fig. 
 \zref{fig:wfuns:single})\footnote{The indexing used here is 1-based 
 ...
 \zref{eq:wfuns:mpbi} as they are the only terms including the absorption
 ...
 be of interest to note that Equation \zref{eq:wfuns:loswfsxx} can be
 written
 \begin{equation}
 ...
 the LOS, and the correspondence to Equation \zref{eq:wfuns:mpbi} for a
 ground point is (c.f. Equations \zref{eq:rte:ground} and
 \zref{eq:rte:tground})
 \begin{eqnarray}
   I &=&\Big[I_{q+1}\zeta_q(1-e)\zeta_{q-1}+\psi_q(1-\zeta_q)(1-e)\zeta_{q-1}
          +eB\zeta_{q-1}+ \nonumber \\
     & & + \psi_{q-1}(1-\zeta_{q-1}) \Big] \Theta^{q-1}_1, \quad 1<q<n 
    \zlabel{eq:wfuns:mpbi_ground}
 \end{eqnarray}
 and the corresponding absorption LOS WF for this point is (cf. Eq.
 \zref{eq:wfuns:loswfsxx})
 \begin{eqnarray}
 ...
 \zref{fig:wfuns:limb}) and both crossings of an atmospheric layer are
 ...
 \zref{eq:wfuns:mpbi}, the intensity of a limb sounding
 observations can be expressed as (Fig. \zref{fig:wfuns:limb})
 \begin{eqnarray}
 ...
 Equation \zref{eq:wfuns:Theta}. The equations defining $I_q$, $I_q^q$
 ...
 included for $q=1$ to make Equation \zref{eq:wfuns:limb1} valid for
 ...
 \begin{figure}[tb]
  \begin{center}
   ...
   \zlabel{fig:wfuns:limb}  
  \end{center}
 \end{figure}
 
 If the different combinations of $\zeta_{q-1}$ and $\zeta_q$ are 
 grouped, for example, Equation \zref{eq:wfuns:limb1} becomes
 \begin{eqnarray}
 ...
 $\zeta_{q-1}$ and $\zeta_q$ than Equation \zref{eq:wfuns:mpbi}, and
 ...
 \begin{equation}
  ...
  \zlabel{eq:wfuns:dzeta3}
 \end{equation}
 \begin{equation}
   \frac{\partial \zeta_{q-1}^2\zeta_q^2}{\partial \kappa_q} = 
          -2\Delta l \zeta_{q-1}^2\zeta_q^2
  \zlabel{eq:wfuns:dzeta4}
 \end{equation}
 Using Equations \zref{eq:wfuns:dzeta1}, \zref{eq:wfuns:dzeta2},
 \zref{eq:wfuns:dzeta3} and \zref{eq:wfuns:dzeta4}, the LOS WFs for 1D
...
\section{Source line of sight weighting functions}
 ...
 section follows closely Section \zref{sec:wfuns:absloswfs}.


 \subsection{Single pass}
  \zlabel{sec:wfuns:single2}
  As, for example,
  \begin{equation}
  ...
  \begin{equation}
   ...
   \zlabel{eq:wfuns:dpsi}
  \end{equation}
  This derivate for other $\psi$ terms is zero.
 
  Using \zref{eq:wfuns:mpbi}, the source LOS WFs for upward looking
 ...
 \subsection{1D downward looking observations}
  ...
  as for the absorption LOS WFs (Sec. \zref{sec:wfuns:down}).

  The LOS WF for the index corresponding to the platform altitude is
  (cf. Eq. \zref{eq:wfuns:idown})
  observations can be determined to be
  \begin{equation}
   \K_{\sigma}^q = \frac{1}{2}\Big[ (1-\zeta_{q-1}\zeta_q) 
           \Big(\Theta^{q-1}_{1}\Big)^2
...
\section{Transformation from vertical altitudes to distances along LOS}
 ...
 as tenth functions (Fig. \zref{fig:wfuns:zbasis}).
 
 
 \subsection{Transformation from $z$ to $l$} 
 The forward model uses
 internally a grid along the line of sight (Sec. \zref{sec:los}), while
 ...
 \begin{figure}[t]
  \begin{center}
   ...
   \zlabel{fig:wfuns:zbasis}  
  \end{center}
 \end{figure}
 \begin{figure}[t]
  \begin{center}
   \includegraphics*[width=0.7\hsize]{fig_absbasis_l}
   \caption{The basis functions of Figure \zref{fig:wfuns:zbasis} shown
            as a function of the distance from the tangent point, where
            $z_{tan}=30$ km.}
   \zlabel{fig:wfuns:lbasis}  
  \end{center}
 \end{figure}
 ...
 absorption at one altitude.  Figure \zref{fig:wfuns:lbasis}
 ...
 Equation \zref{eq:wfuns:bases} only for the grid points along the LOS:
 \begin{equation}
...
\section{Species WFs}
 ...
 absorption for other species. Equation \zref{eq:wfuns:kspecies}
 ...
 \zref{sec:wfuns:approaches:anal}.

 If Equation \zref{eq:wfuns:kspecies} is valid, we have then that
 \begin{equation}
   \frac{\partial \mat{k}^p}{\partial \xt^p} = \mat{\bar{k}}^p_s
  \zlabel{eq:wfuns:dkspecies}
 \end{equation}
 ...
 addition, Equation \zref{eq:wfuns:dkspecies} is especially simple
 for this case:
 \begin{equation}
...
\section{Continuum absorption WFs}
 ...
 (Fig. \zref{fig:wfuns:cont}). This approach was selected as it gives
 ...
 Figure \zref{fig:wfuns:cont} exemplifies this for $n_{cont}=2$.  The
 ...
 \begin{eqnarray}
  ...
  \zlabel{eq:wfuns:lagrange}
 \end{eqnarray}
 where $x_i$ is the absorption at the selected frequency points, $\f_i$,
 that are given by Equation \zref{eq:wfuns:cont:f}, and $N=n_{cont}+1$.
 
 The frequency dependency of the continuum WFs can be obtained by
 differentiating Equation \zref{eq:wfuns:lagrange}:
 \begin{equation}
...
\section{Temperature profile WFs}
 ...
 derived in Section \zref{sec:wfuns:sourceloswfs}, while the absorption
 LOS WFs ($\partial \iv/\partial \kappa$) are found in Section
 \zref{sec:wfuns:absloswfs}. As a single grid is here of concern,
 Equation \zref{eq:wfuns:sandk} is valid, that is, $\partial\kappa/
 \partial \mat{k}^p$ equals $\partial \sigma / \partial S^\mat{p}$.
 These two terms are discussed in Section \zref{sec:wfuns:bases}.
 ...
 (Equation \zref{eq:rte:planck}), and the derivative of the source
 ...
 \begin{equation}
   ...
   \zlabel{eq:wfuns:dsdt}
 \end{equation}
 The term $\partial \mat{S}^p / \partial \mat{t}^p$ is calculated
 using Equation \zref{eq:wfuns:dsdt} where $T$ is replaced by $\mat{t}^p$.
