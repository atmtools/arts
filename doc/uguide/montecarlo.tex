...
Eq. \zref{antInt} and sample $\mathbf{n'}$ according to a probability
density function (PDF) equal to $\psi(\mathbf{n'})$ then an unbiased
estimate of Eq. \zref{antInt} is given by (e.g. \cite{press:1992:numerical})
...
Eq. \zref{antIntMC}, which is given by Eq. \zref{intVRTE}.  First, we
re-express \zref{intVRTE} as a single integral, for simplicity dropping
...
The integrand Eq. \zref{inorouteq} is a piecewise function of the
...
The reader
could easily verify that evaluating Eq. \zref{inorouteq} gives
Eq. \zref{intVRTE}. The aim in importance sampling is to choose probability density functions
