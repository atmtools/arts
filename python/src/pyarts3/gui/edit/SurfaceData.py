"""Editor for SurfaceData struct type."""
...
def edit(value, parent=None):
    """
    Edit a SurfaceData object.

    Behavior mirrors AtmData/SubsurfaceData:
    - "data" row shows current type and value
    - Double-click Type on the 'data' row to change among {GeodeticField2, Numeric, NumericBinaryOperator}
    - Double-click Value to edit current value with the appropriate editor
    - lat/lon extrapolation limits editable via double-click on Value
    """
    from pyarts3 import arts
    from . import edit as dispatch_edit

    dialog = QDialog(parent)
    dialog.setWindowTitle("Edit SurfaceData")
    dialog.setMinimumWidth(700)

    layout = QVBoxLayout(dialog)

    help_label = QLabel(
        "Double-click Type (data row) to change type, or Value to edit it. Others: double-click Value."
    )
    ...
    # Track the variant data in a mutable cell
    ...
    # Cache for non-data fields while editing
    current_values = {}
    ...
    def on_cell_double_clicked(row, col):
        ...
        if field_name == "data":
            if col == 1:
                # Change between GeodeticField2, Numeric, NumericBinaryOperator
                ...
                type_layout.addWidget(
                    QLabel(f"Current type: {type(current_data[0]).__name__}")
                )
                type_combo = QComboBox()
                type_combo.addItems(["GeodeticField2", "Numeric", "NumericBinaryOperator"])
                ...
                if type_dialog.exec_() == QDialog.Accepted:
                    ...
                    try:
                        ...
                        elif new_type == "GeodeticField2":
                            import numpy as np

                            new_data = arts.GeodeticField2(
                                name="Empty",
                                data=np.zeros((1, 1)),
                                grid_names=["Latitude", "Longitude"],
                                grids=[np.array([0.0]), np.array([0.0])],
                            )
                        elif new_type == "NumericBinaryOperator":
                            new_data = arts.NumericBinaryOperator()
                        else:
                            return
                        ...
                        QMessageBox.warning(
                            dialog, "Error", f"Failed to create {new_type}: {e}"
                        )
                return
            elif col == 2:
                # Edit current value using dispatcher
    ...
    if dialog.exec_() == QDialog.Accepted:
        result = arts.SurfaceData(current_data[0])
