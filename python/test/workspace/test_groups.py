...
class TestGroups:
    ...
    def test_if_group_fulfill_basic_contracts(self):
        ...
        for g in list_of_groups:
            if g in special_groups: continue
            ...
            assert hasattr(
                x, "readxml"), f"Workspace group {g} lacks an xml reading routine!"
            assert hasattr(
                x, "savexml"), f"Workspace group {g} lacks an xml saving routine!"
            assert x.__doc__ != "Missing description" and len(
                x.__doc__), f"{g} lacks documentation"
