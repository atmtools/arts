...
class CIARecord:
    ...
    def specs(self):
        """ The species (tuple of 2 Index) """
        return [Species(c.c_void_p(lib.getSpecies1CIARecord(self.__data__)), delete=True),
                Species(c.c_void_p(lib.getSpecies2CIARecord(self.__data__)), delete=True)]

    @specs.setter
    def specs(self, x):
        if isinstance(x, Sized) and len(x) == 2:
            s1 = x[0] if isinstance(x[0], Species) else Species(x[0])
            s2 = x[1] if isinstance(x[1], Species) else Species(x[1])
            lib.setSpeciesCIARecord(self.__data__, s1.__data__, s2.__data__)
        else:
...
lib.getSpecies1CIARecord.restype = c.c_void_p
lib.getSpecies1CIARecord.argtypes = [c.c_void_p]

lib.getSpecies2CIARecord.restype = c.c_void_p
lib.getSpecies2CIARecord.argtypes = [c.c_void_p]

lib.setSpeciesCIARecord.restype = None
lib.setSpeciesCIARecord.argtypes = [c.c_void_p, c.c_void_p, c.c_void_p]
