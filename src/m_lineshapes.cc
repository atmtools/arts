...
void elsVoigt_Drayson(//WS Output:
                         Vector&   els,
                         //WS Input:
                         const Numeric& ls_sigma,
                         const Numeric& ls_gamma,
...
// constant normalization factor for voigt
  Numeric fac = 1.0 / ls_sigma * sqrt_invPI;
...
// Resize will do nothing if the size is already correct.
  els.resize(nf);
      ...
      for (i1=0; i1<nf; i1++)
        { 
          Numeric deltaf=els_f_grid[i1];
          x[i1] = fabs(deltaf)/ls_sigma;
        }
      cout << x;
      xml_write_to_file ("x.xml",x );
...
L104: for (K=0; K<(int) nf; K++)
  ...
  L108: J = (int) (x[K]+x[K]+1.85);
      ...
      els[K] = UU/(UU*UU+VV*VV)/1.772454*fac;
      continue;
  L110: Y2 = Y*Y;
  ...
  L112: Y2 = Y*Y;
      ...
      U = x[K]-XX[3];
      V = x[K]+XX[3];
      els[K] = Y*(HH[3]/(Y2+U*U)+HH[3]/(Y2+V*V))*fac;
      continue;
  ...
  L113: U = x[K]-XX[1];
      ...
      UU = x[K]-XX[2];
      VV = x[K]+XX[2];
      els[K] = Y*(HH[1]/(Y2+U*U)+HH[1]/(Y2+V*V)+HH[2]/(Y2+UU*UU)+HH[2]/(Y2+
                                                                      VV*VV))*fac;
      continue;
  }
}
...
void elsVoigt_Kuntz6(//WS Output:
                         Vector&   els,
                         //WS Input:
                         const Numeric& ls_sigma,
                         const Numeric& ls_gamma,
  ...
  if (lauf[7] >= lauf[3] || lauf[15] >= lauf[11]) {
    ...
    for (i2 = 1; i2 <= 3; i2 += 2) {
      ...
      for (i1 = lauf[(i2 << 2) - 1]; i1 <= i__1; ++i1) {
        x2 = x[i1-1] * x[i1-1];
        els[i1-1] = fac * (exp(y2 - x2) * cos(x[i1-1] * ym2) - (a7 + x2 *
  ...
  if (lauf[6] >= lauf[2] || lauf[14] >= lauf[10]) {
    ...
    for (i2 = 1; i2 <= 3; i2 += 2) {
      ...
      for (i1 = lauf[(i2 << 2) - 2]; i1 <= i__1; ++i1) {
        x2 = x[i1-1] * x[i1-1];
        els[i1-1] = fac * (a5 + x2 * (b5 + x2 * (c5 + x2 * (d5 + x2 * 
  ...
  if (lauf[5] >= lauf[1] || lauf[13] >= lauf[9]) {
    ...
    for (i2 = 1; i2 <= 3; i2 += 2) {
      ...
      for (i1 = lauf[(i2 << 2) - 3]; i1 <= i__1; ++i1) {
        x2 = x[i1-1] * x[i1-1];
        els[i1-1] = fac * (a3 + x2 * (b3 + x2 * (c3 + x2 * d3))) / (a4 
