...
/**
   ...
   \retval   os           the stream to write to
   \param    tgs          the tag groups to write
      ...
      if ( tgs[i].nelem() > 0 )
        {
          ...
          const  SpeciesRecord& spr = species_data[tgs[i][0].Species()];
          os << spr.Name() << '\n';
        }
    }
}
        ...
        const TagGroups&   tgs )
{
  ...
  // Write the array of String to the stream:
  write_tag_groups_species_to_stream(of,tgs);
}
