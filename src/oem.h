...
using LM_CG_S =
    invlib::LevenbergMarquardt<Numeric, OEMCovarianceMatrix, Normed<CG>>;
...
class ArtsLog {
 public:
  ArtsLog(unsigned int v, Vector &g, bool l = false)
      : verbosity(v), gamma_history(g), linear(l), finalized(false) {}
  ...
  void step(const Params &... params) {
    if (verbosity >= 1) {
      ...
      std::cout << OptimizerLog<OptimizationType>::log(
          std::get<5>(tuple), gamma_history, std::get<0>(tuple));
      std::cout << std::endl;
    }
  }
