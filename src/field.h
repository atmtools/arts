...
public:
  ...
  Field3D(Field3D&& g) : 
  mpages(std::move(g.mpages)), mrows(std::move(g.mrows)),
  mcols(std::move(g.mcols)), data(std::move(g.data)) {}
  
  Field3D(size_t pages=0, size_t rows=0, size_t cols=0, const base& init=base()) : 
  mpages(pages), mrows(rows), mcols(cols), data(cols*rows*pages, init) {}
  
  base& operator()(size_t page=0, size_t row=0, size_t col=0) {return data[col + row * mcols + page * mrows * mcols];}
