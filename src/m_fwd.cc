...
void spectral_rad_operatorClearsky1D(const Workspace&          ws,
  ...
  auto lines  = ws.wsv_and_contains(lines_str) ? ws.share(lines_str).share<lines_t>() : std::shared_ptr<lines_t>{};
  auto cia    = ws.wsv_and_contains(cia_str) ? ws.share(cia_str).share<cia_t>() : std::shared_ptr<cia_t>{};
  auto xsec   = ws.wsv_and_contains(xsec_str) ? ws.share(xsec_str).share<xsec_t>() : std::shared_ptr<xsec_t>{};
  auto predef = ws.wsv_and_contains(predef_str) ? ws.share(predef_str).share<predef_t>() : std::shared_ptr<predef_t>{};
...
void spectral_rad_fieldFromOperatorPlanarGeometric(GriddedSpectralField6&          spectral_rad_field,
  ...
  spectral_rad_field = GriddedSpectralField6{
      ...
      .data       = StokvecTensor6(nalt, nlat, nlon, nza, naa, nfreq, Stokvec{0.0, 0.0, 0.0, 0.0}),
      .grid_names = {"Altitude", "Latitude", "Longitude", "Zenith angle", "Azimuth angle", "Frequency"},
  ...
  ARTS_USER_ERROR_IF(stdr::binary_search(zen_grid, 90.0), "Zenith angle must not be 90 degrees")
  ...
  const std::vector<fwd::path> upwards   = spectral_rad_operator.geometric_planar({alt_low, lat, lon}, {180, 0});
  const std::vector<fwd::path> downwards = spectral_rad_operator.geometric_planar({alt_high, lat, lon}, {0, 0});

  const auto pathstep = [&upwards, &downwards](const Numeric za, const Numeric az) {
  ...
  const auto freqstep = [&spectral_rad_operator](
                            const Numeric freq, const Numeric za, const std::vector<fwd::path>& path) {
    StokvecVector srad;
    if (za < 90.0) {
      srad = spectral_rad_operator(freq, path, SpectralRadianceOperator::as_vector{});
    } else {
      srad = spectral_rad_operator(freq, path, SpectralRadianceOperator::as_vector{});
      stdr::reverse(srad);
    }
    ...
    for (Index i = 0; i < nza; ++i) {
      for (Index j = 0; j < naa; ++j) {
        ...
        for (Index n = 0; n < nfreq; ++n) {
          spectral_rad_field.data[joker, 0, 0, i, j, n] = freqstep(freq_grid[n], zen_grid[i], path);
        }
      }
    }
  } else {
...
void spectral_rad_fieldFromOperatorPath(const Workspace&                ws,
        ...
        for (Index ilon = 0; ilon < nlon; ++ilon) {
          for (Index iza = 0; iza < nza; ++iza) {
            for (Index iaa = 0; iaa < naa; ++iaa) {
              ...
              stdr::transform(freq_grid,
                              spectral_rad_field[ialt, ilat, ilon, iza, iaa, joker].begin(),
                              [path = spectral_rad_operator.from_path(ray_path), &spectral_rad_operator](Numeric f) {
...
void measurement_vecFromOperatorPath(const Workspace&                ws,
  ...
  const SensorSimulations simulations = collect_simulations(measurement_vec_sensor);
  ...
  for (auto& [freq_grid, poslos_grid, iposlos] : simulations) {
    ...
    ray_path_observer_agendaExecute(ws, ray_path, poslos.pos, poslos.los, ray_path_observer_agenda);

    const StokvecVector spectral_rad{
        std::from_range,
        freq_grid | stdv::transform([path = spectral_rad_operator.from_path(ray_path),
                                     &spectral_rad_operator](Numeric f) { return spectral_rad_operator(f, path); })};

    for (Size iv = 0; iv < measurement_vec_sensor.size(); ++iv) {
      const SensorObsel& obsel = measurement_vec_sensor[iv];
      if (obsel.same_freqs(freq_grid)) { measurement_vec[iv] += obsel.sumup(spectral_rad, iposlos); }
    }
  }
}
