...
void subsurf_profileFromPath(ArrayOfSubsurfacePoint&            subsurf_profile,
  ...
  const auto extract = stdv::transform([&](Vector3 pos) { return subsurf_field.at(pos[0], pos[1], pos[2]); });

  subsurf_profile = ray_path | to_vec3 | extract | stdr::to<ArrayOfSubsurfacePoint>();
}
...
void spectral_radSubsurfaceDisortEmissionWithJacobian(const Workspace&            ws,
  ...
  spectral_radSubsurfaceDisortEmission(ws,
                                       spectral_rad,
                                       disort_settings,
                                       ray_path,
...
#pragma omp parallel for if (not arts_omp_in_parallel() and                                               \
                                 static_cast<Size>(disort_quadrature_dimension) < model_state_vec.size()) \
  ...
  for (Size i = 0; i < model_state_vec.size(); i++) {
    try {
      ...
      std::transform(
          ...
          [d = model_state_vec_perturbation[i]](const Stokvec& a, const Stokvec& b) -> Stokvec { return (a - b) / d; });
...
#pragma omp critical
  ...
  ARTS_USER_ERROR_IF(not error.empty(), "Error occurred in disort-spectral with jacobian:\n{}", error)
}
