...
void RadarOnionPeelingTableCalc(
  ...
  ARTS_USER_ERROR_IF(nss != 2,
                     ...
                     "scattering species.");
  ARTS_USER_ERROR_IF(
      scat_species.size() != static_cast<Size>(nss),
  ...
  // Allocate
  if (invtable.empty()) invtable.resize(2);
  invtable[iss].data_name =
  ...
  Matrix b(nt, nse), e(nt, nse);
  {
    ...
    for (Index i = 0; i < nse; i++) {
      ...
      ARTS_USER_ERROR_IF(scat_data[iss][i].za_grid[ib] < 179.999,
                         ...
                         "This is not the case for scat_data[{}][{}] (0-based)",
                         iss,
                         i)
      ...
      interp(e(joker, i),
             itw,
             scat_data[iss][i].ext_mat_data(0, joker, 0, 0, 0),
             gp);
      interp(b(joker, i),
             itw,
             scat_data[iss][i].pha_mat_data(0, joker, ib, 0, 0, 0, 0),
             gp);
    }
  }
  ...
  for (Index w = 0; w < nwc; w++) {
    ...
    for (Index t = 0; t < nt; t++) {
      for (Index i = 0; i < nse; i++) {
        ARTS_USER_ERROR_IF(
            b(t, i) < 0,
            "A negative back-scattering found for scat_species {}"
            ",\ntemperature {}"
            "K and element ",
            iss,
            t_grid[t],
        ...
        ARTS_USER_ERROR_IF(e(t, i) < 0,
                           "A negative extinction found for scat_species {}"
                           ",\ntemperature {}"
                           "K and element {}",
                           iss,
                           t_grid[t],
        ...
        ARTS_USER_ERROR_IF((pnd_data(t, i) < 0),
                           "A negative PSD value found for scat_species {}"
                           ",\ntemperature {}"
                           "K and {}"
                           " kg/m3",
                           iss,
