...
struct Workspace {
  ...
  Workspace(WorkspaceInitialization how_to_initialize = WorkspaceInitialization::FromGlobalDefaults)
      : wsv{WorkspaceInitialization::FromGlobalDefaults == how_to_initialize ? global_wsv_defaults()
  ...
  template <WorkspaceGroup T> void set(const std::string& name, const T& data) { set(name, Wsv{data}); }

  //! Move the workspace variable with the given name to the given value.
  template <WorkspaceGroup T> void set(const std::string& name, T&& data) { set(name, Wsv{std::move(data)}); }

  //! Borrows the workspace variable with the given name to the given value.
  template <WorkspaceGroup T> void set(const std::string& name, T* data) { set(name, Wsv{data}); }

  //! Overwrites a variable with another variable of the same type
  void overwrite(const std::string& name, const Wsv& data);

  //! Copy the workspace variable with the given name to the given value.
  template <WorkspaceGroup T> void overwrite(const std::string& name, const T& data) { overwrite(name, Wsv{data}); }

  //! Move the workspace variable with the given name to the given value.
  template <WorkspaceGroup T> void overwrite(const std::string& name, T&& data) {
    overwrite(name, Wsv{std::move(data)});
  }

  //! Borrows the workspace variable with the given name to the given value.
  template <WorkspaceGroup T> void overwrite(const std::string& name, T* data) { overwrite(name, Wsv{data}); }

  //! Returns a type directly based on the name of the workspace variable.
  template <WorkspaceGroup T> [[nodiscard]] T& get(const std::string& name) const;

  //! Returns a type directly based on the name of the workspace variable, creating it in-place if it is not there
  template <WorkspaceGroup T> [[nodiscard]] std::shared_ptr<T> share_or(const std::string& name);

  //! Returns a type directly based on the name of the workspace variable, creating it in-place if it is not there
  template <WorkspaceGroup T> [[nodiscard]] T& get_or(const std::string& name);
...
template <> struct std::formatter<Workspace> {
  format_tags tags;
  ...
  template <class FmtContext> FmtContext::iterator format(const Workspace& v, FmtContext& ctx) const {
    const std::string_view sep   = tags.sep();
    const std::string_view quote = tags.quote();

    tags.add_if_bracket(ctx, "{"sv);
    for (auto& [name, wsv] : v) { tags.format(ctx, quote, name, quote, ": "sv, wsv, sep); }
    tags.add_if_bracket(ctx, "}"sv);
...
template <> struct xml_io_stream_name<Workspace> {
  static constexpr std::string_view name = "Workspace";
};

template <> struct xml_io_stream<Workspace> {
  constexpr static std::string_view type_name = xml_io_stream_name_v<Workspace>;

  static void write(std::ostream& os, const Workspace& x, bofstream* pbofs = nullptr, std::string_view name = ""sv);
