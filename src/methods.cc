...
*/
  ...
  md_data_raw.push_back(create_mdrecord(
      ...
      DESCRIPTION(
          ...
          "(if 2D, the GIN *option_2d* must be set, the default results in an error):\n"
          "\n"
          "\"interp_response\""
          ...
          "and the antenna pattern is interpolated to these directions. This means\n"
          "that the antenna response must be broader than the angular domain\n"
          "covered by *mblock_dlos_grid* (i.e. the opposite to 1D). There is no\n"
          "check on how well *mblock_dlos_grid* covers the antenna response.\n"
          "\n"
          "\"gridded_dlos\""
          "This option can be seen as parallel to the 1D case, except that both\n"
          "antenna response and radiances are treated as bi-linear functions.\n"
          "For this option *mblock_dlos_grid* must match a combination of zenith\n"
          "and azimuth grids, and this for a particular order. If the zenith and\n"
          "azimuth grids have 3 and 2 values, respectively, the order shall be:\n"
