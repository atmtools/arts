        ...
        const Index&          agenda_verb )
{
  ...
  // Resize iy to have the correct the size
    ...
    case 2:   //--- The surface -----------------------------------------------
      {
        ...
        if( surface_los.nrows() == 0 )
        ...
        else
          {
            ...
            // Copy surface data to local variables the surface variables can 
            // be changed below by call of *rte_calc*. There is no need 
            // to copy *surface_los*.
            Index     nlos = surface_los.nrows();
            Matrix    surface_emission_local(nf,stokes_dim);
            Tensor4   surface_refl_coeffs_local(nlos,nf,stokes_dim,stokes_dim);
            ...
            for( Index ilos=0; ilos<nlos; ilos++ )
              {
                ...
                rte_calc( y_local, ppath, ppath_step, iy,
                          rte_pos, rte_los,
                   surface_emission, surface_los, surface_refl_coeffs, 
                   ppath_step_agenda, rte_agenda, iy_space_agenda, 
                   surface_agenda, iy_space_agenda, atmosphere_dim, p_grid, lat_grid, 
                   lon_grid, z_field, t_field, r_geoid, z_surface,
                   cloudbox_on, cloudbox_limits,
                   sensor_response, sensor_pos, sensor_los,
                          f_grid, stokes_dim, antenna_dim,
                ...
                // Add the calculated spectrum (*iy*) to *i_sum*,
                // considering the reflection coeff. matrix.
                //
                for( Index iv=0; iv<nf; iv++ )
                  {
                    if( stokes_dim == 1 )
                      {
                        i_sum(iv,0) += 
                                 surface_refl_coeffs(ilos,iv,0,0) * iy(iv,0);
                      }
                    else
                      {
                        for( Index is0=0; is0<stokes_dim; is0++ )
            ...
            for( Index iv=0; iv<nf; iv++ )
              {
                for( Index is=0; is<stokes_dim; is++ )
                  { 
                    iy(iv,is) = i_sum(iv,is) + surface_emission_local(iv,is);
                  }
              }
        ...
        const bool&           agenda_verb )
{
  ...
  get_radiative_background( iy, ppath_step, rte_pos, rte_los, 
          surface_emission, surface_los, surface_refl_coeffs, 
                            ppath, ppath_step_agenda, rte_agenda, 
          iy_space_agenda, surface_agenda, iy_cloudbox_agenda, 
                            atmosphere_dim, 
          ...
          for( iaa=0; iaa<naa; iaa++ )
            {
              ...
              iy_calc( iy, ppath, ppath_step, rte_pos, rte_los, 
                 surface_emission,  surface_los,  surface_refl_coeffs, 
                 ppath_step_agenda, rte_agenda, iy_space_agenda, 
                 surface_agenda, iy_cloudbox_agenda, atmosphere_dim, p_grid, 
              ...
              // If the sensor should be applied *iy* is reshaped
              // to a column vector to match *sensor_response*
              if( apply_sensor ) {
                for( ip=0; ip<stokes_dim; ip++ )
