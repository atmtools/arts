...
void mc_IWP_cloud_opt_pathCalc(
  ...
  if (mc_antenna.get_type() == ANTENNA_TYPE_PENCIL_BEAM) {
    ...
    mc_IWP_error =
        sqrt((iwp_squared / (Numeric)mc_iteration_count - mc_IWP * mc_IWP) /
...

{
  ...
  Matrix opt_depth_mat(stokes_dim, stokes_dim),
      incT(stokes_dim, stokes_dim, 0.0);
  ...
  while ((evol_op(0, 0) > r) & (!termination_flag)) {
    ...
    ppath_step_geom_3d(
        ppath_step, lat_grid, lon_grid, z_field, refellipsoid, z_surface, -1);
    ...
    else if (fractional_gp(ppath_step.gp_p[np - 1]) >=
  ...
  if (termination_flag) {  //we must have reached the cloudbox boundary
    ...
    k = -opt_depth_mat(0, 0) /
        cum_l_step[np - 1];  //K=K11 only for diagonal ext_mat
...
void MCIPA(Workspace& ws,
  ...
  if (!(atmosphere_dim == 3)) {
    throw runtime_error("For montecarlo, atmosphere_dim must be 3.");
  }
  ...
  Vector pnd_vec(
      N_pt);            //Vector of particle number densities used at each point
  ...
  rng.seed(mc_seed, verbosity);
  bool keepgoing,
      inside_cloud;  // flag indicating whether to stop tracing a photons path
  Numeric g, temperature, albedo, g_los_csc_theta;
  Matrix A(stokes_dim, stokes_dim), Q(stokes_dim, stokes_dim),
      evol_op(stokes_dim, stokes_dim), ext_mat_mono(stokes_dim, stokes_dim),
  ...
  Vector vector1(stokes_dim), abs_vec_mono(stokes_dim), I_i(stokes_dim),
      Isum(stokes_dim), Isquaredsum(stokes_dim);
  y.resize(stokes_dim);
  y = 0;
  ...
  if (y_unit == "RJBT") {
    std_err_i = f_grid[0] * f_grid[0] * 2 * BOLTZMAN_CONST / SPEED_OF_LIGHT /
  ...
  while (true) {
    mc_iteration_count += 1;
    keepgoing =
        true;  //flag indicating whether to continue tracing a photon path
    ...
    while (keepgoing) {
      ...
      np = ppath.np;
      mc_points(ppath.gp_p[np - 1].idx,
                ppath.gp_lat[np - 1].idx,
    ...
    for (Index j = 0; j < stokes_dim; j++) {
      mc_error[j] =
          sqrt((Isquaredsum[j] / (Numeric)mc_iteration_count - y[j] * y[j]) /
               (Numeric)mc_iteration_count);
    }
...
md_data_raw.push_back(MdRecord(
    ...
    GOUT("pha_mat_single"),
    GOUT_TYPE("Matrix"),
    GOUT_DESC("Phase matrix for a single frequency and combination of angles"),
    IN("f_grid", "f_index", "stokes_dim", "scat_data_array", "rtp_temperature"),
    GIN("za_out", "aa_out", "za_in", "aa_in"),
    GIN_TYPE("Numeric", "Numeric", "Numeric", "Numeric"),
    GIN_DEFAULT(NODEF, NODEF, NODEF, NODEF),
    GIN_DESC("Outgoing zenith angle",
...
void pha_matExtractManually(Matrix& pha_mat,
  ...
  scat_data_array_monoCalc(
      scat_data, scat_data_array, f_grid, f_index, verbosity);

  Vector pnd(1, 1.0);

  pha_mat.resize(stokes_dim, stokes_dim);

  pha_mat_singleCalc(pha_mat,
                     za_out,
                     aa_out,
                     za_in,
                     aa_in,
...
void mcPathTraceIPA(Workspace& ws,
  ...
  while ((evol_op(0, 0) > r) & (!termination_flag)) {
    ...
    Numeric Dx = (lon_grid[gp_lon.idx + 1] - lon_grid[gp_lon.idx]) * DEG2RAD *
                 refell2r(refellipsoid, lat_grid[gp_lat.idx]) *
    ...
    if (Dxy / abs(tan(rte_los[0] * DEG2RAD)) > Dz) {
      dz = Dz;
    } else {
      dz = Dxy / abs(tan(rte_los[0] * DEG2RAD));
    }
    Numeric dxy;
    if (dz * abs(tan(rte_los[0] * DEG2RAD)) > Dxy) {
      dxy = Dxy;
    } else {
      dxy = dz * abs(tan(rte_los[0] * DEG2RAD));
    }
    ...
    rte_pos[0] =
        rv_ellips + interp_atmfield_by_gp(3, z_field, gp_p, gp_lat, gp_lon);
    rte_pos[1] = interp(lat_iw, lat_grid, gp_lat);
    rte_pos[2] = interp(lon_iw, lon_grid, gp_lon);

    //calculate evolution operator
    inside_cloud =
        is_gp_inside_cloudbox(gp_p, gp_lat, gp_lon, cloudbox_limits, false);
...
void iwp_cloud_opt_pathCalc(
  ...
  if (ppath_what_background(ppath) > 2) {
    ...
    for (Index i = 0; i < ppath.np; ++i) {
      ...
      iwc_vec[i] = pnd_vec * particle_masses(
                                 joker, 0);  //hopefully this is the dot product
    }
