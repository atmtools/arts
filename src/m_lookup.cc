...
namespace {
template <bool calc>
std::conditional_t<calc, Vector, void> _spectral_propmatAddLookup(PropmatVector& spectral_propmat [[maybe_unused]],
  ...
  if constexpr (calc) {
    ...
    const Vector absorption = _spectral_propmatAddLookup<not calc>(spectral_propmat,
    ...
    for (Size i = 0; i < freq_grid.size(); i++) {
      if (no_negative_absorption == 0 or absorption[i] > 0.0) { spectral_propmat[i].A() += absorption[i]; }
    }
    ...
    if (jac_targets.atm.size()) {
      ...
      for (auto& jacobian_target : jac_targets.atm) {
        ARTS_USER_ERROR_IF(not std::isnormal(jacobian_target.d),
                           "The target {} is not good, it lacks a perturbation value.",
                           jacobian_target);

        if (is_wind(jacobian_target)) {
          const AscendingGrid freq_grid2(
              freq_grid.begin(), freq_grid.end(), [d = jacobian_target.d](Numeric x) { return x + d; });
          d_absorption = _spectral_propmatAddLookup<not calc>(spectral_propmat,
                                                              spectral_propmat_jac,
                                                              freq_grid2,
                                                              {},
                                                              spectral_propmat_select_species,
        ...
        for (Size i = 0; i < freq_grid.size(); i++) {
          if (no_negative_absorption == 0 or d_absorption[i] > 0.0) {
            spectral_propmat_jac[jacobian_target.target_pos, i].A() = (d_absorption[i] - absorption[i]) * d_inv;
          }
        }
      }
    }
...
void spectral_propmatAddLookup(PropmatVector&                spectral_propmat,
...
void abs_lookup_dataPrecompute(AbsorptionLookupTables&  abs_lookup_data,
  ...
  abs_lookup_data[select_species] = {
      ...
      abs_bands,
      abs_ecs_data,
      temperature_perturbation.empty() ? nullptr : std::make_shared<const AscendingGrid>(temperature_perturbation),
      water_perturbation.empty() ? nullptr : std::make_shared<const AscendingGrid>(water_perturbation)};
}

void abs_lookup_dataPrecomputeAll(AbsorptionLookupTables&   abs_lookup_data,
                                  const ArrayOfAtmPoint&    atm_profile,
                                  const AscendingGrid&      freq_grid,
                                  const AbsorptionBands&    abs_bands,
                                  const LinemixingEcsData&  abs_ecs_data,
  ...
  const auto         species_not_in_lut =
      ...
      stdv::filter([&lut_species](const SpeciesEnum& s) { return lut_species.end() == stdr::find(lut_species, s); });
  ...
  for (auto spec : water_affected_species) {
    ARTS_USER_ERROR_IF(not stdr::binary_search(lut_species, spec),
  ...
  const auto t = std::make_shared<const AscendingGrid>(temperature_perturbation);
  const auto w = water_affected_species.empty() ? nullptr : std::make_shared<const AscendingGrid>(water_perturbation);

  for (SpeciesEnum s : lut_species) {
    const bool do_water_perturb = stdr::any_of(water_affected_species, Cmp::eq(s));
...
void abs_lookup_dataFromProfiles(AbsorptionLookupTables&   abs_lookup_data,
                                 const AscendingGrid&      freq_grid,
                                 const AbsorptionBands&    abs_bands,
                                 const LinemixingEcsData&  abs_ecs_data,
  ...
  ArrayOfAtmPoint atm_profile(pressure_profile.size(), AtmPoint{to<IsoRatioOption>(isoratio_option)});

  ARTS_USER_ERROR_IF(not same_shape(pressure_profile.vec(), temperature_profile),
  ...
  for (auto& [spec, prof] : vmr_profiles) {
    ARTS_USER_ERROR_IF(not same_shape(pressure_profile.vec(), prof),
                       "Pressure and VMR profiles must agree in size, fails for species {}",
                       spec);

    for (Size i = 0; i < prof.size(); i++) { atm_profile[i][spec] = prof[i]; }
  }
...
void abs_lookup_dataSimpleWide(AbsorptionLookupTables&   abs_lookup_data,
  ...
  ARTS_USER_ERROR_IF(
      pressure_range[1] <= pressure_range[0], "Pressure range must be increasing, got {:B,}", pressure_range);
  ARTS_USER_ERROR_IF(temperature_range[1] <= temperature_range[0],
                     "Temperature range must be increasing, got {:B,}",
                     temperature_range);
  ARTS_USER_ERROR_IF(
      water_vmr_range[1] <= water_vmr_range[0], "Water VMR range must be increasing, got {:B,}", water_vmr_range);
  ARTS_USER_ERROR_IF(vmr_value <= 0.0, "VMR value must be positive, got {}", vmr_value);
  ARTS_USER_ERROR_IF(atmospheric_steps < 2, "Must have at least two atmospheric levels, got {}", atmospheric_steps);
  ARTS_USER_ERROR_IF(temperature_perturbation_steps < 0,
                     "Must have zero or positive temperature perturbations steps, got {}",
                     temperature_perturbation_steps);
  ARTS_USER_ERROR_IF(water_vmr_perturbation_steps < 0,
  ...
  const DescendingGrid pressure_profile = nlogspace(pressure_range[1], pressure_range[0], atmospheric_steps);

  const AscendingGrid water_perturbation = [vmr_value, water_vmr_range, water_vmr_perturbation_steps]() {
    Vector out  = nlinspace(water_vmr_range[0], water_vmr_range[1], water_vmr_perturbation_steps);
    out        /= vmr_value;
    return out;
  }();
  const AscendingGrid temperature_perturbation = nlinspace(temperature_range[0] - temperature_profile[0],
                                                           temperature_range[1] - temperature_profile[0],
                                                           temperature_perturbation_steps);

  const SpeciesEnumVectors vmr_profiles = [species = lbl::species_in_bands(abs_bands), vmr_value, atmospheric_steps]() {
    SpeciesEnumVectors out;
    for (SpeciesEnum s : species) { out[s] = Vector(atmospheric_steps, vmr_value); }
    return out;
  }();
