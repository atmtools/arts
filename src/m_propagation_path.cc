...
los:      {:B,}
  ...
  ray_path.resize(1);
  ray_path[0] = path::init(pos, los, atm_field, surf_field, static_cast<bool>(as_sensor));
}
...
void ray_pathSetGeometricExtremes(ArrayOfPropagationPathPoint& ray_path,
  ...
  path::set_geometric_extremes(
      ray_path, atm_field, surf_field, surf_search_accuracy, static_cast<bool>(surf_safe_search));
}

void ray_pathRemoveNonGeometricGridCrossings(ArrayOfPropagationPathPoint& ray_path,
                                             const AtmField&              atm_field,
  ...
  ARTS_USER_ERROR_IF(
      not std::holds_alternative<GeodeticField3>(data.data), "The data for key {} is not a GeodeticField3", atm_key);
  ...
  std::erase_if(ray_path, [&alt, &lat, &lon](auto& p) {
    return not(stdr::contains(alt, p.altitude()) or stdr::contains(lat, p.latitude()) or
...
void ray_pathAddGeometricAltitudeGridCrossings(ArrayOfPropagationPathPoint& ray_path,
  ...
  ARTS_USER_ERROR_IF(
      stdr::any_of(ray_path, Cmp::not_in_sorted(alt_grid), [](const PropagationPathPoint& p) { return p.altitude(); }),
...
void ray_pathAddLimbPoint(ArrayOfPropagationPathPoint& ray_path, const SurfaceField& surf_field) {
  ARTS_TIME_REPORT
...
void ray_path_observer_agendaSetGeometric(Agenda&        ray_path_observer_agenda,
  ...
  creator.add("ray_pathInit", SetWsv{"pos", "obs_pos"}, SetWsv{"los", "obs_los"}, SetWsv("as_sensor", Index{1}));
  ...
  if (add_crossings) { creator.add("ray_pathAddGeometricGridCrossings", SetWsv("atm_key", atm_key)); }

  switch (to<ray_path_observer_agendaSetGeometricMaxStep>(max_step_option)) {
    case ray_path_observer_agendaSetGeometricMaxStep::half: creator.add("ray_pathFillGeometricMaxStep"); break;
    case ray_path_observer_agendaSetGeometricMaxStep::step: creator.add("ray_pathFillGeometricStepwise"); break;
    case ray_path_observer_agendaSetGeometricMaxStep::None: break;
  }

  if (remove_non_crossings) { creator.add("ray_pathRemoveNonGeometricGridCrossings", SetWsv("atm_key", atm_key)); }
  ...
  if (remove_nearby > 0.0) {
    creator.add("ray_pathRemoveNearby", SetWsv("min_dist", remove_nearby), SetWsv("first", remove_nearby_first));
  }
...
void ray_pathGeometric(ArrayOfPropagationPathPoint& ray_path,
  ...
  ray_pathSetGeometricExtremes(ray_path, atm_field, surf_field, surf_search_accuracy, surf_safe_search);
...
void ray_pointBackground(PropagationPathPoint& ray_point, const ArrayOfPropagationPathPoint& ray_path) {
  ARTS_TIME_REPORT
...
void ray_pointForeground(PropagationPathPoint& ray_point, const ArrayOfPropagationPathPoint& ray_path) {
  ARTS_TIME_REPORT
...
void ray_pointLowestFromPath(PropagationPathPoint& ray_point, const ArrayOfPropagationPathPoint& ray_path) {
  ARTS_TIME_REPORT

  ARTS_USER_ERROR_IF(ray_path.size() == 0, "Empty propagation path.")

  ray_point = *stdr::min_element(ray_path, [](const PropagationPathPoint& a, const PropagationPathPoint& b) {
    return a.altitude() < b.altitude();
  });
}

void ray_pointHighestFromPath(PropagationPathPoint& ray_point, const ArrayOfPropagationPathPoint& ray_path) {
  ARTS_TIME_REPORT

  ARTS_USER_ERROR_IF(ray_path.size() == 0, "Empty propagation path.")

  ray_point = *stdr::max_element(ray_path, [](const PropagationPathPoint& a, const PropagationPathPoint& b) {
    return a.altitude() < b.altitude();
  });
}
...
void ray_pathGeometricUplooking(ArrayOfPropagationPathPoint& ray_path,
  ...
  ray_pathGeometric(ray_path,
                    ...
                    surf_field,
                    max_step,
                    {surf_field.single_value(SurfaceKey::h, latitude, longitude), latitude, longitude},
                    {0, 0},
                    1.0,
                    true,
                    false,
...
void ray_pointPastGeometric(PropagationPathPoint&              ray_point,
  ...
  ray_point = past_geometric(ray_path.back(), atm_field, surf_field, max_stepsize, safe_search_accuracy, search_safe);
}
...
void ray_pointPastRefractive(PropagationPathPoint&              ray_point,
  ...
  future.zenith()             = acosd((future.nreal / (1.0 + single_dispersion)) * cosd(future.zenith()));

  future.nreal = 1 + single_dispersion;

  ray_point = past_geometric(future, atm_field, surf_field, max_stepsize, safe_search_accuracy, search_safe);
}
