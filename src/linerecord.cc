...
bool LineRecord::ReadFromHitranModifiedStream(istream& is, const Verbosity& verbosity)
{
    CREATE_OUT3;
    ...
    if ( !hinit )
    {
        ...
        for ( Index i=0; i<hspec.nelem(); ++i )
        {
            if ( missing != hspec[i] )
            {
                ...
                // stl Strings.
                out3 << "  mo = " << i << "   Species = "
                << std::setw(10) << setiosflags(ios::left)
                << species_data[hspec[i]].Name().c_str()
                << "iso = ";
                for ( Index j=1; j<hiso[i].nelem(); ++j )
                {
    ...
    while (comment)
    {
        ...
        if ( 0 != mo )
        {
            ...
            if ( missing != hspec[mo] )
            {
                ...
                {
                    ostringstream os;
                    os << "Invalid modified HITRAN 2004 line data record with " << nChar <<
                    " characters (expected: 202).";
                    throw std::runtime_error(os.str());
                }
    ...
    if ( iso < hiso[mo].nelem() )
        ...
        mpressurebroadeningdata.SetAirAndWaterBroadeningFromCatalog(sgam,nself,0.0,agam,nair,psf,wgam,nw,0.0);
