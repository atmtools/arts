...
void abs_linesReadFromMytran2(  // WS Output:
  ...
  while (go_on) {
    ...
    if (lr.ReadFromMytran2Stream(is, verbosity)) {
      ...
      if (fmin <= lr.F())
        if (lr.F() <= fmax) abs_lines.push_back(std::move(lr));
    }
  }
  out2 << "  Read " << abs_lines.nelem() << " lines.\n";
}
...
void abs_lines_per_speciesWriteToSplitArtscat(  // WS Input:
  ...
  for (ArrayOfArrayOfLineRecord::const_iterator it =
    ...
    if (it->nelem()) {
      ...
      WriteXML(
          output_file_format, *it, species_filename, 0, "", "", "", verbosity);
    }
  }
}
...
void abs_linesReadFromSplitArtscat(  // WS Output:
  ...
  for (ArrayOfArrayOfSpeciesTag::const_iterator asp = abs_species.begin();
       asp != abs_species.end();
       asp++)
    for (ArrayOfSpeciesTag::const_iterator sp = asp->begin(); sp != asp->end();
         sp++) {
  ...
  for (set<Index>::const_iterator it = unique_species.begin();
    ...
    abs_linesReadFromArts(more_abs_lines,
                          tmpbasename + (species_data[*it].Name()) + ".xml",
                          fmin,
                          fmax,
                          verbosity);
    abs_lines.insert(
        abs_lines.end(), more_abs_lines.begin(), more_abs_lines.end());
  }
...
void abs_lines_per_speciesReadFromCatalogues(  // WS Output:
  ...
  if (n_cat != formats.nelem() || n_cat != fmin.nelem() ||
      n_cat != fmax.nelem()) {
    ostringstream os;
  ...
  for (Index i = 1; i < n_tg; ++i) {
    ...
    if (n_cat > i) {
      ...
      const Index that_cat =
          find(real_filenames.begin(), real_filenames.end(), filenames[i]) -
          real_filenames.begin();
      if (that_cat < real_filenames.nelem()) {
        ...
        if (formats[i] != real_formats[that_cat] ||
            fmin[i] != real_fmin[that_cat] || fmax[i] != real_fmax[that_cat]) {
          ostringstream os;
  ...
  for (Index i = 0; i < n_real_cat; ++i) {
    ...
    if ("HITRAN96" == real_formats[i]) {
      abs_linesReadFromHitranPre2004(
          abs_lines, real_filenames[i], real_fmin[i], real_fmax[i], verbosity);
    ...
    abs_lines_per_speciesCreateFromLines(
        these_abs_lines_per_species, abs_lines, these_tgs, verbosity);

    // Put these lines in the right place in abs_lines_per_species:
    for (Index s = 0; s < real_tgs[i].nelem(); ++s) {
      abs_lines_per_species[real_tgs[i][s]] =
          std::move(these_abs_lines_per_species[s]);
    }
  }
}
...
void abs_lines_per_speciesCreateFromLines(  // WS Output:
  ...
  for (Index i = 0; i < abs_lines.nelem(); ++i) {
    ...
    for (j = 0; j < tgs.nelem() && !found; ++j) {
      ...
      for (Index k = 0; k < tgs[j].nelem() && !found; ++k) {
        ...
        // used.
        if (this_tag.Isotopologue() !=
            this_line.SpeciesData().Isotopologue().nelem())
          if (this_tag.Isotopologue() != this_line.Isotopologue()) continue;
        ...
        if (this_tag.Lf() >= 0)
          if (this_tag.Lf() > this_line.F()) continue;

        // Then the upper limit:
        if (this_tag.Uf() >= 0)
          if (this_tag.Uf() < this_line.F()) continue;
    ...
    if (found) {
      ...
      if (species_used[this_line.Species()]) {
        ...
        out2 << "  Your tags include other lines of species "
             ...
             << "  Why do you not include line " << i << " (at "
             << this_line.F() << " Hz)?\n";
      }
    }
  }
  ...
  for (Index i = 0; i < tgs.nelem(); ++i) {
    ...
    for (Index s = 0; s < tgs[i].nelem(); ++s) out3 << " " << tgs[i][s].Name();
      ...
      for (Index j = 0; j < n; ++j) {
        ...
        if (ll[j].GetMirroringType() == MirroringType::None)
          ...
          throw std::runtime_error(
              "Cannot mirror line manually since it is already mirrored by LineRecord");
      }
    }
    //
    ll.reserve(n + nnew);
  ...
  for (Index i = 0; i < abs_lines_per_species.nelem(); ++i) {
    ...
    if (cutoff != -1) {
      ...
      if (keep.nelem() != ll.nelem()) {
        ...
        for (Array<ArrayOfLineRecord::iterator>::iterator j = keep.begin();
             j != keep.end();
             j++) {
...
void abs_lineshapeDefine(  // WS Output:
  ...
  if (found0 == -1) throw runtime_error("Selected lineshape not available.");
  if (found1 == -1)
...
void abs_lineshape_per_tgDefine(  // WS Output:
  ...
  if ((tg_sz != shape.nelem()) || (tg_sz != normalizationfactor.nelem()) ||
      (tg_sz != cutoff.nelem())) {
    ostringstream os;
...
void abs_xsec_per_speciesAddLines(  // WS Output:
  ...
  // groups as a dimension are consistent.
  {
    ...
    if (n_tgs != n_xsec || n_tgs != n_vmrs || n_tgs != n_lines ||
        (n_tgs != n_shapes && 1 != n_shapes)) {
      ostringstream os;
  ...
  for (Index ii = 0; ii < abs_species_active.nelem(); ++ii) {
    ...
    if (0 < ll.nelem() && tgs[i].nelem() && !is_zeeman(tgs[i])) {
      ...
      if ("Rosenkranz_Voigt_Drayson" == lineshape_name ||
        ...
        if ("O2" != species_name) {
          ostringstream os;
          os << "You are trying to use one of the special Rosenkranz lineshapes with\n"
      ...
      if (tgs[i][0].LineMixing() == SpeciesTag::LINE_MIXING_OFF and
          not do_jac) {
        Matrix dummy_phase;
    ...
    if (out3.sufficient_priority()) {
      ostringstream os;
      os << "  Tag group " << i << " (" << get_tag_group_name(tgs[i])
...
void propmat_clearskyAddParticles(
  ...
  Matrix pnd_ext_mat(stokes_dim, stokes_dim),
      pnd_ext_mat_df(stokes_dim, stokes_dim),
      pnd_ext_mat_dt(stokes_dim, stokes_dim), tmp(stokes_dim, stokes_dim),
      tmp2(stokes_dim, stokes_dim);
  Vector pnd_abs_vec(stokes_dim), pnd_abs_vec_df(stokes_dim),
      pnd_abs_vec_dt(stokes_dim);
  Vector f_grid_df;
      ...
      for (Index i_se = 0; i_se < scat_data[i_ss].nelem(); i_se++) {
        ...
        if (rtp_vmr[sp] > 0.) {
          ...
          if (do_jac_temperature) {
            opt_propExtract(pnd_ext_mat_dt,
                            pnd_abs_vec_dt,
                            scat_data_mono[i_ss][i_se],
                            rtp_los_back[0],
                            rtp_los_back[1],
          ...
          if (do_jac && first_loop) rtp_vmr_sum += rtp_vmr[sp];
