...
//! Reads ArrayOfAtmPoint from XML input stream
/*!
 * \param is_xml     XML Input stream
 * \param at         ArrayOfAtmPoint return value
 * \param pbifs      Pointer to binary input stream. NULL in case of ASCII file.
 */
void xml_read_from_stream(istream& is_xml,
                          ArrayOfAtmPoint& at,
  ...
  tag.read_from_stream(is_xml);
  tag.check_name("Array");
  tag.check_attribute("type", "AtmPoint");
  ...
  try {
    ...
    ostringstream os;
    os << "Error reading ArrayOfAtmPoint: "
       << "\n Element: " << n << "\n"
       << e.what();
    throw runtime_error(os.str());
  }
...
//! Writes ArrayOfAtmPoint to XML output stream
/*!
 *  \param os_xml  XML Output stream
 *  \param at      ArrayOfAtmPoint
 *  \param pbofs   Pointer to binary file stream. NULL for ASCII output.
 *  \param name    Optional name attribute
 */
void xml_write_to_stream(ostream& os_xml,
                         const ArrayOfAtmPoint& at,
                         bofstream* pbofs,
                         const String& name,
                         const Verbosity& verbosity) {
  ArtsXMLTag open_tag(verbosity);
  ...
  open_tag.add_attribute("type", "AtmPoint");
  open_tag.add_attribute("nelem", at.nelem());
