...
void spectral_propmat_pathFromPath(const Workspace                   &ws,
...
void spectral_propmat_pathAddAdaptiveHalfPath(const Workspace             &ws,
                                              ArrayOfPropmatVector        &spectral_propmat_path,
                                              ArrayOfStokvecVector        &spectral_nlte_srcvec_path,
                                              ArrayOfPropmatMatrix        &spectral_propmat_jac_path,
                                              ArrayOfStokvecMatrix        &spectral_nlte_srcvec_jac_path,
  ...
  for (Size ip = 0; ip < ray_path.size() - 1; ip++) {
    ...
    for (Size f = 0; f < nf; f++) {
      ...
      if (sum_tau[f] < cutoff_tau and dtau > max_tau and dtau > max_dtau) { max_dtau = dtau; }
      sum_tau[f] += dtau;
    }
    ...
    if (max_dtau > 0.0) {
      ...
      nrp.front()         = rp0;
      nrp.back()          = rp1;
      const Numeric new_r = std::min(std::nextafter(0.5 * r, 0.0),  // at least 3 points
      ...
      if (not nrp.empty()) {
        ...
        spectral_propmat_pathFromPath(
            ws, nspp, nsnsp, nspjp, nsnsjp, spectral_propmat_agenda, nfgp, nfwsjp, jac_targets, nrp, nap);
        ...
        freq_wind_shift_jac_path.insert_range(freq_wind_shift_jac_path.begin() + ip + 1, nfwsjp);
        spectral_propmat_path.insert_range(spectral_propmat_path.begin() + ip + 1, nspp);
        spectral_nlte_srcvec_path.insert_range(spectral_nlte_srcvec_path.begin() + ip + 1, nsnsp);
        spectral_propmat_jac_path.insert_range(spectral_propmat_jac_path.begin() + ip + 1, nspjp);
        spectral_nlte_srcvec_jac_path.insert_range(spectral_nlte_srcvec_jac_path.begin() + ip + 1, nsnsjp);
        ip += nrp.size();
      }
    }
  }
...
void spectral_propmat_and_atm_path_agendaSetAdaptive(Agenda        &spectral_propmat_and_atm_path_agenda,
                                                     const Numeric &max_tau,
                                                     const Numeric &cutoff_tau) {
  assert(stdr::contains(internal_workspace_methods().at("spectral_propmat_pathAddAdaptiveHalfPath").gin, "max_tau"));
  assert(stdr::contains(internal_workspace_methods().at("spectral_propmat_pathAddAdaptiveHalfPath").gin, "cutoff_tau"));
  assert(internal_workspace_methods().at("spectral_propmat_pathAddAdaptiveHalfPath").gin.size() == 2);

  spectral_propmat_and_atm_path_agenda = get_spectral_propmat_and_atm_path_agenda("AdaptiveHalfPath");
  spectral_propmat_and_atm_path_agenda.change_default("max_tau"sv, max_tau);
  spectral_propmat_and_atm_path_agenda.change_default("cutoff_tau"sv, cutoff_tau);
}