                                             ...
                                             const Index& n)
{
  ...
  for(Index iline = 0; iline < n; iline++)
  {
    ...
    const LineShape::Output X({pressure_broadening[iline], psf[iline], 0., 0., 0., 0., Y[iline], G[iline], DV[iline]});
    
    if(do_temperature_jacobian(derivatives_data))
    {

      const LineShape::Output dT({dpressure_broadening_dT[iline], dpsf_dT[iline], 0., 0., 0., 0., dY_dT[iline], dG_dT[iline], dDV_dT[iline]});
