...
struct WorkspaceMethodInternalMetaRecord {
  ...
  [[nodiscard]] WorkspaceMethodInternalRecord create(
      const std::unordered_map<std::string, WorkspaceMethodInternalRecord>& wsms) const;
  [[nodiscard]] std::string call(const std::unordered_map<std::string, WorkspaceMethodInternalRecord>& wsms) const;
};
