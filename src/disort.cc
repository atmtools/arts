...
void dtauc_ssalbCalc(Workspace& ws,
  ...
  for (Index scat_p_index_local = 0; scat_p_index_local < Np_cloud;
    ...
    ext_vector[scat_p_index_local + cloudbox_limits[0]] =
        ext_mat_local.Kjj()[0];
    abs_vector[scat_p_index_local + cloudbox_limits[0]] =
        abs_vec_local.Kjj()[0];
  }
  ...
  for (Index i = 0; i < Np; i++) {
    ...
    for (auto& pm : propmat_clearsky_local) gas_vector[i] += pm.Kjj()[0];
  }
...
*/
    ...
    dtauc[Np - 2 - i] =
        (ext_part + abs_gas) * (z_field(i + 1, 0, 0) - z_field(i, 0, 0));
  }
}
  ...
  for (Index f_index = 0; f_index < f_grid.nelem(); f_index++) {
    ...
    if (pfct_method == "interpolate") {
      phase_functionCalc2(phase_function,
                          ...
                          t_field,
                          cloudbox_limits,
                          scat_angle_grid.nelem(),
                          verbosity);
      for (Index l = 0; l < nlyr; l++)
        if (phase_function(l, 0) == 0.) assert(ssalb[l] == 0.);
      ...
      phase_functionCalc(phase_function,
                         scat_data,
                         f_index,
                         pnd_field,
                         cloudbox_limits,
...
#pragma omp critical(fortran_disort_qgausn)
  { qgausn_(&nn, gmu.get_c_array(), gwt.get_c_array()); }

  // Calc polar angles za from their cosines mu
  for (Index i = 0; i < nn; i++) za_grid[i] = acos(gmu[i]) * 180. / PI;
