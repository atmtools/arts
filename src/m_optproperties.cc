    ...
    for (Index p_index = 0; p_index <= cloudbox_limits[1] - cloudbox_limits[0];
      ...
      for (Index scat_za_index_local = 0;
        ...
        pha_matCalc(pha_mat_local,
                    ...
                    verbosity);
        pha_mat_doit(
            p_index, scat_za_index_local, 0, joker, joker, joker, joker) =
            pha_mat_local;
      }
    }
...
void TestScatDataInterp(
  ...
  //
  Vector pnd_vec(N_se, 0);
  pnd_vec[scat_elem_index] = 1;  // 1-pt pnd as needed for MC

  Tensor4 pnd(
      N_se, 1, 1, 1);  // 3D field (containing 1pt only) as used in DO methods)
  ...
  if (printinfo) {
    ...
    cout << "phase matrix (" << za_printinfo_index << "," << aa_printinfo_index
         << "):\n"
         << pha_mat_bulk(
                f_index,
                0,
                0,
                printinfo_index[za_printinfo_index][aa_printinfo_index],
                joker,
                joker)
         << endl;
    cout << endl;
...
*/
  ...
  //spt_calc_agenda
  opt_prop_sptFromMonoData(ext_mat_spt,
                           abs_vec_spt,
                           scat_data_mono,
                           sza_grid,
                           saa_grid,
        ...
        if (ssd.ptype == PTYPE_TOTAL_RND)
          for (Index iza = 0; iza < n_za; iza++)
            for (Index iaa = 0; iaa < n_aa; iaa++)
              pha_matTransform(
                  ...
                  ssd.pha_mat_data(
                      0, i_pfct, joker, joker, joker, joker, joker),
                  ssd.za_grid,
                  ssd.aa_grid,
                  ssd.ptype,
                  0,
          ...
          for (Index iaa = 0; iaa < n_aa; iaa++) {
              ...
              for (Index sza = 0; sza < nza_se; sza++)
                for (Index ist1 = 0; ist1 < stokes_dim; ist1++)
                  for (Index ist2 = 0; ist2 < stokes_dim; ist2++)
                    pha_mat_interp(sza, iza, ist1, ist2) =
                        interp(itw_aa,
                               ssd.pha_mat_data(0,
                                                i_pfct,
                                                sza,
            ...
            for (Index iza = 0; iza < n_za; iza++) {
              ...
              for (Index ist1 = 0; ist1 < stokes_dim; ist1++)
                for (Index ist2 = 0; ist2 < stokes_dim; ist2++)
                  pha_mat_rt4(iza, iaa, ist1, ist2) = interp(
                      itw_za,
  ...
  if (printinfo) {
    ...
    for (Index ist1 = 0; ist1 < stokes_dim; ist1++)
      for (Index ist2 = 0; ist2 < stokes_dim; ist2++)
        dext_mat(ist1, ist2) =
            tmp1(ist1, ist2) - ext_mat_bulk(f_index, 0, 0, ist1, ist2);
    for (Index ist1 = 0; ist1 < stokes_dim; ist1++)
      for (Index ist2 = 0; ist2 < stokes_dim; ist2++)
        dpha_mat(ist1, ist2) =
...
*/
  ...
  for (Index ic = 0; ic < compare.nelem(); ic++) {
    if (compare[ic] == 1) {
      ...
      Compare(abs_vec_bulk(f_index, 0, 0, joker),
              abs_vec_mc,
              dmax,
              "Deviation in abs_vec",
              "US",
              "MC",
              "",
              "",
              verbosity);
      dmax = 0.5 * tolerance *
             (ext_mat_bulk(f_index, 0, 0, 0, 0) + ext_mat_mc(0, 0));
      Compare(ext_mat_bulk(f_index, 0, 0, joker, joker),
              ext_mat_mc,
              dmax,
              "Deviation in ext_mat",
              "US",
              "MC",
              "",
              "",
      ...
      for (Index iza = 0; iza < n_za; iza++)
        for (Index iaa = 0; iaa < n_aa; iaa++) {
          ...
          dmax = 0.5 * tolerance *
                 (pha_mat_bulk(f_index, 0, 0, printinfo_index[iza][iaa], 0, 0) +
                  pha_mat_mc(iza, iaa, 0, 0));
          Compare(pha_mat_bulk(
                      f_index, 0, 0, printinfo_index[iza][iaa], joker, joker),
      ...
      for (Index iza = 0; iza < n_za; iza++)
        for (Index iaa = 0; iaa < n_aa; iaa++) {
          ...
          os << "Deviation in pha_mat at za[" << iza << "]=" << pha_mat_za[iza]
             << "deg and aa[" << iaa << "]=" << pha_mat_aa[iaa] << "deg.";
          dmax = 0.5 * tolerance *
                 (pha_mat_rt4(iza, iaa, 0, 0) + pha_mat_mc(iza, iaa, 0, 0));
          Compare(pha_mat_rt4(iza, iaa, joker, joker),
                  pha_mat_mc(iza, iaa, joker, joker),
                  dmax,
                  os.str(),
                  "RT4",
