...
void rte_step_doit(  //Output and Input:
  ...
  for (Index i = 1; i < stokes_dim; i++)
    if (abs_vec_av[i] != 0) unpol_abs_vec = false;
...
void interp_cloud_coeff1D(  //Output
  ...
  for (Index i = 0; i < stokes_dim; i++) {
    ...
    } else if (scat_za_interp == 1)  //polynomial interpolation
    {
      ...
      Tensor3 doit_i_field_mono_int_za(
          stokes_dim, ppath_step.np, scat_za_grid.nelem(), 0.);
      for (Index za = 0; za < scat_za_grid.nelem(); za++) {
        interp(sca_vec_int_za(i, joker, za),
               ...
               cloud_gp_p);
        out3 << "Interpolate doit_i_field_mono:\n";
        interp(doit_i_field_mono_int_za(i, joker, za),
               itw,
      ...
      for (Index ip = 0; ip < ppath_step.np; ip++) {
        ...
        doit_i_field_mono_int(i, ip) =
            interp_poly(scat_za_grid,
                        doit_i_field_mono_int_za(i, ip, joker),
                        los_grid[ip],
                        gp_za[ip]);
      }
    }
  ...
  while (max_diff > acc) {
    ...
    for (Index i_p = 0; i_p < N_p; i_p++) {
      for (Index i_za_red = 0; i_za_red < idx.nelem(); i_za_red++) {
        ...
        doit_i_field_opt(i_p, i_za_red) =
            doit_i_field_mono(i_p, 0, 0, idx[i_za_red], 0, 0);
      }
    ...
    //Transform in %
    max_diff =
        max_diff_p / doit_i_field_mono(ind_p, 0, 0, ind_za[ind_p], 0, 0) * 100.;
...
void iy_interp_cloudbox_field(Matrix& iy,
  ...
  if (border == 99) {
    if (atmosphere_dim == 3) {
      ...
      out3 << " Sensor inside cloudbox at position:  " << rte_gp_p << "\n";
      ...
      if (interpmeth == "linear") {
        for (Index is = 0; is < stokes_dim; is++) {
          for (Index iv = 0; iv < nf; iv++) {
            for (Index i_za = 0; i_za < nza; i_za++) {
              iy_p[i_za] = interp(
                  itw_p, doit_i_field(iv, joker, 0, 0, i_za, 0, is), gp_p);
            }
        ...
        for (Index is = 0; is < stokes_dim; is++) {
          for (Index iv = 0; iv < nf; iv++) {
            ...
            iy(iv, is) = interp_poly(scat_za_grid, iy_p, rte_los[0], gp_za);
          }
        }
      }
    }
        ...
        if (is == 0 && rigorous) {
          for (Index iv = 0; iv < nf; iv++) {
            if (doit_i_field(iv, border_index, 0, 0, gp.idx, 0, is) /
                        doit_i_field(
                            iv, border_index, 0, 0, gp.idx + 1, 0, is) >
                    1 / maxratio &&
                doit_i_field(iv, border_index, 0, 0, gp.idx, 0, is) /
                    ...
                    maxratio) {
              iy(iv, is) = interp(
                  itw, doit_i_field(iv, border_index, 0, 0, joker, 0, is), gp);
            } else {
              ostringstream os;
              ...
              os << "Happens at boundary " << border << " between zenith\n"
                 ...
                 << " and "
                 << doit_i_field(iv, border_index, 0, 0, gp.idx + 1, 0, 0)
                 << " W/(sr m2 Hz).";
      ...
      for (Index is = 0; is < stokes_dim; is++) {
        for (Index iv = 0; iv < nf; iv++) {
          iy(iv, is) =
              interp(itw,
                     doit_i_field(
                         iv, joker, border_index - 2, joker, joker, joker, is),
                     cb_gp_p,
                     cb_gp_lon,
                     gp_za,
                     gp_aa);
      ...
      for (Index is = 0; is < stokes_dim; is++) {
        for (Index iv = 0; iv < nf; iv++) {
          iy(iv, is) =
              interp(itw,
                     doit_i_field(
                         iv, joker, joker, border_index - 4, joker, joker, is),
                     cb_gp_p,
                     cb_gp_lat,
                     gp_za,
                     gp_aa);
