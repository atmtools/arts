...
Tensor3 to_tensor3(const muelmat_vector_const_view &m) {
  ...
  std::transform(m.begin(), m.end(), out.begin(),
                 [](const muelmat &v) { return Matrix{v}; });
...
stokvec_vector to_stokvec_vector(const ExhaustiveConstMatrixView &v) {
  ...
  std::transform(v.begin(), v.end(), out.begin(),
                 [](const ExhaustiveConstVectorView &a) { return stokvec{a}; });
  return out;
}
...
Matrix to_matrix(const stokvec_vector_const_view &v) {
  Matrix out(v.size(), 4);
  std::transform(v.begin(), v.end(), out.begin(),
                 [](const stokvec &s) { return Vector{s}; });
  return out;
}
...
Matrix to_matrix(const propmat& v) {
    ...
    v.B(), v.A(), v.U(), v.V(),
    v.C(), -v.U(), v.A(), v.W(),
    v.D(), -v.V(), -v.W(), v.A()}.reshape(4, 4);
}

Vector to_vector(const stokvec &v) {
    return Vector{v.I(), v.Q(), v.U(), v.V()};
}
} // namespace rtepack
