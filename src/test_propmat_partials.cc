...
void test_pressurebroadening() {
  ...
  const LineRecord line(6,
                        0,
                        61150556350.7454,
                        0.0,
                        4.03935532732085e-19,
                        296.0,
                        2.5505950629926e-21,
                        13255.072408981,
                        13047.9619025907,
  ...
  for (Numeric p = -2; p < 6; p++) {
    ...
    for (Numeric T = 150; T < 350; T += 20) {
      line.PressureBroadening().GetPressureBroadeningParams(g_0,
                                                            tmp1,
                                                            tmp2,
                                                            df_0,
                                                            tmp3,
      ...
      line.PressureBroadening().GetPressureBroadeningParams_dT(
          dg, ddf, T, line.Ti0(), P, vmr * P, 0, -1, empty1, vmrs, none);
...
void test_linefunctionsdata() {
  ...
  const LineFunctionData test(istringstream(
...
Numeric test2(Numeric x) { return x * x; }
void test_partitionfunction() {
  const ArrayOfNumeric empty_aon;
  const LineRecord line(species_index_from_species_name("O2"),
                        0,
                        61150556350.7454,
                        0.0,
                        4.03935532732085e-19,
  ...
  Vector part_data;
  part_data = {4.016432e-01,
               7.315888e-01,
               -3.313678e-05,
               6.642877e-08};  //O2 66 partition function

  for (Numeric T = 150; T < 350; T += 20) {
    CalculatePartitionFctFromCoeff(
        qref_cache, qt_cache, line.Ti0(), T, part_data);
    CalculatePartitionFctFromCoeff(
        qref_cache_d, qt_cache_d, line.Ti0(), T + dT, part_data);
...
void test_K1_and_K2() {
  ...
  for (Numeric T = 150; T < 350; T += 20) {
    ...
    const Numeric gamma_ref =
        exp(-PLANCK_CONST * line.F() / (BOLTZMAN_CONST * line.Ti0()));
    // Stimulated emission
    const Numeric K2 = (1. - gamma) / (1. - gamma_ref);
    // Boltzmann level
    const Numeric K1 =
        exp(line.Elow() / BOLTZMAN_CONST * (T - line.Ti0()) / (T * line.Ti0()));

    // Following Futbolin's division into two parts for the Boltzmann ratio because
    // gamma is also used for the NLTE part later on
    const Numeric gamma_d =
        exp(-PLANCK_CONST * line.F() / (BOLTZMAN_CONST * (T + dT)));
    // Stimulated emission
    const Numeric K2_d = (1. - gamma_d) / (1. - gamma_ref);
    // Boltzmann level
    const Numeric K1_d = exp(line.Elow() / BOLTZMAN_CONST *
                             ((T + dT) - line.Ti0()) / ((T + dT) * line.Ti0()));

    const Numeric dK1 = line.Elow() / BOLTZMAN_CONST / T / T * K1;
    const Numeric dK2 = -line.F() * PLANCK_CONST / BOLTZMAN_CONST / T / T *
                        (gamma / (1.0 - gamma_ref));
...
void test_lineshape() {
  ...
  Vector ls(5, 0.0), ls_d(5, 0.0), ls_phase(5, 0.0), dls_dx(5, 0.0),
      dls_dy(5, 0.0), dx_dT(5, 0.0);

  for (Numeric p = -2; p < 6; p++) {
    const Numeric P = pow(10., p);
    //std::cout<<"\nPressure: "<<P<<" Pa. dT: "<<dT<<" K\n";
    for (Numeric T = 150; T < 350; T += 20) {
      line.PressureBroadening().GetPressureBroadeningParams(g,
                                                            tmp1,
                                                            tmp2,
                                                            df,
                                                            tmp3,
      ...
      const Numeric sigma =
          line.F() * doppler_const * sqrt(T / 31.989830);  //mass is O2-66

      const Numeric dsigma_dT = line.F() * doppler_const * sqrt(T / 31.989830) /
                                T / 2.0;  //mass is O2-66
...
void test_matrixexp() {
  ...
  tmp(3, 2) = 4.55607e-11;
  tmp(3, 3) = 2.08609e-10;
  const Matrix ppath_ext_0 = tmp;  //dppath_ext_dx for ip
  tmp(0, 0) = 2.08609e-10;
  tmp(0, 1) = 7.33668e-12;
  tmp(0, 2) = 1.39302e-11;
  tmp(0, 3) = -1.11706e-10;
  ...
  tmp(3, 2) = 0;
  tmp(3, 3) = 2.08609e-10;
  const Matrix ppath_ext_0_test = tmp;  //dppath_ext_dx for ip
  tmp(0, 0) = -2.53045e-12;
  tmp(0, 1) = -9.07729e-14;
  tmp(0, 2) = -1.72351e-13;
  tmp(0, 3) = 1.36037e-12;
