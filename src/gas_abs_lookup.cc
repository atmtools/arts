...
void GasAbsLookup::Adapt(const ArrayOfArrayOfSpeciesTag& current_species,
  ...
  *this = new_table;
  ...
  flag_default = Interpolation::LagrangeVector(f_grid, f_grid, 0, 0.5, false, Interpolation::LagrangeType::Linear);
}
...
void GasAbsLookup::Extract(Matrix& sga,
  ...
  if (f_interp_order == 0) {
    ...
    if (n_new_f_grid == n_f_grid) {
      ...
      flag = &flag_local;
      flag_local = Interpolation::LagrangeVector(new_f_grid, f_grid, 0, 0.5, false, Interpolation::LagrangeType::Linear);
    ...
    flag = &flag_local;
    flag_local = Interpolation::LagrangeVector(new_f_grid, f_grid, f_interp_order, 0.5, false, Interpolation::LagrangeType::Linear);
  }
  ...
  const auto plag = Interpolation::LagrangeVector(log(p), log_p_grid, p_interp_order, 0.5, false, Interpolation::LagrangeType::Linear);
