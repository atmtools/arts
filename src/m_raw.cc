...
void yColdAtmHot(Vector& y,
                 ...
                 const Index& calib) {
  ARTS_USER_ERROR_IF(
      cold.size() not_eq atm.size() or atm.size() not_eq hot.size(),
...
void ybatchColdAtmHotAtmCycle(ArrayOfVector& ybatch,
  ...
  ARTS_USER_ERROR_IF(
      level0_data.size() not_eq static_cast<Size>(cold_temp.size()) or
          level0_data.size() not_eq static_cast<Size>(hot_temp.size()),
      "Length of vectors must be correct");
  ARTS_USER_ERROR_IF(
      level0_time.size() not_eq level0_data.size() and
          level0_time.size() not_eq 0,
      "Bad level0_time length, must be empty of same as level0_data");

  ybatch =
      Raw::Calibration::caha(level0_data, cold_temp, hot_temp, first_c_index);
...
void ybatchTimeAveraging(ArrayOfVector& ybatch,
  ...
  ARTS_USER_ERROR_IF(
      not std::is_sorted(sensor_time.cbegin(), sensor_time.cend()),
  ...
  if (static_cast<Size>(lims.front()) == n) {
    ...
    ARTS_USER_ERROR_IF(
        m < 0, "Must include last if time step covers all of the range");
...
void ybatchTroposphericCorrectionNaiveMedianForward(ArrayOfVector& ybatch_corr,
  ...
  ARTS_USER_ERROR_IF(std::any_of(ybatch.begin(),
                                 ybatch.end(),
                                 [m](auto& y) { return y.size() not_eq m; }),
                     "Bad input size, all of ybatch must match itself");

  ARTS_USER_ERROR_IF(
      trop_temp.size() not_eq n and trop_temp.size() not_eq 1,
  ...
  for (Index i = 0; i < n; i++) {
    ...
    ybatch_corr[i][1] =
        std::exp(-std::log((ybatch_corr[i][2] - ybatch_corr[i][0]) /
                           (ybatch_corr[i][2] - targ_temp)));
  }
...
void ybatchTroposphericCorrectionNaiveMedianInverse(
    ArrayOfVector& ybatch, const ArrayOfVector& ybatch_corr) {
  // Size of problem
  const Size n = ybatch.size();
  ARTS_USER_ERROR_IF(
      (std::any_of(ybatch_corr.begin(),
                   ybatch_corr.end(),
                   [](auto& corr) { return corr.size() not_eq 3; })) or
          ybatch_corr.size() not_eq n,
...
void yMaskOutsideMedianRange(Vector& y, const Numeric& dx) {
...
void ybatchMaskOutsideMedianRange(ArrayOfVector& ybatch, const Numeric& dx) {
