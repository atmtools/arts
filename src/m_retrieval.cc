...
void RetrievalAddSubsurface(
  ...
  covmat_diagonal_blocks[JacobianTargetType{jac_targets.subsurf.back().type}] =
      {.first = matrix, .second = inverse};
}
...
void RetrievalAddSurface(
  ...
  covmat_diagonal_blocks[JacobianTargetType{jac_targets.surf.back().type}] = {
      .first = matrix, .second = inverse};
}
...
void RetrievalAddAtmosphere(
  ...
  covmat_diagonal_blocks[JacobianTargetType{jac_targets.atm.back().type}] = {
      .first = matrix, .second = inverse};
}
...
void RetrievalAddMagneticField(
    JacobianTargets& jac_targets,
    JacobianTargetsDiagonalCovarianceMatrixMap& covmat_diagonal_blocks,
...
void RetrievalAddOverlappingMagneticField(
  ...
  covmat_diagonal_blocks[JacobianTargetType{jac_targets.atm[N - 2].type}] = {
      .first = matrix, .second = inverse};

  covmat_diagonal_blocks[JacobianTargetType{jac_targets.atm[N - 1].type}] = {
      .first = matrix, .second = inverse};
}
