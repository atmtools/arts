...
os << R"--( throw std::runtime_error(var_string("Unknown workspace group ", std::quoted(type)));
  ...
  for (auto& group : groups()) {
    ...
    os << "template <> const std::shared_ptr<" << group << ">& Wsv::share_unsafe<"
       << group << ">() const { return *std::get_if<std::shared_ptr<" << group
       << ">>(&value); }\n\n";

    os << "template <> const std::shared_ptr<" << group << ">& Wsv::share<" << group
       << ">() const {\n"
          "  if (not holds<"
       << group
       << ">()) {\n"
...
os << R"--(//! auto-generated by make_auto_wsg.cpp
  ...
  for (auto& group : groups()) {
    os << "template<> const std::shared_ptr<" << group << ">& Workspace::share_or<"
       << group
