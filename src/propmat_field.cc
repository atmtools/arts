                          ...
                          const Agenda&                     propmat_clearsky_agenda)
{
  ...
  #pragma omp parallel for if(not arts_omp_in_parallel()) schedule(guided) firstprivate(l_ws, l_propmat_clearsky_agenda)
  for(Index i=0; i<nalt; i++) {
    for(Index j=0; j<nlat; j++) {
      for(Index k=0; k<nlon; k++) {
        thread_local Index itmp;
        get_stepwise_clearsky_propmat(l_ws, propmat_field(i, j, k),
  ...
  const Verbosity&                  verbosity)
{
  ...
  for(Index ip=0; ip<np; ip++) {
    get_stepwise_blackbody_radiation(B, vtmp, f_grid, interp_atmfield_by_gp(1, t_field, ppath.gp_p[ip]), 0);
    ...
    if(ip)
      stepwise_transmission(lyr_tra[ip], tmtmp, tmtmp, K_past, K_this, pmtmp, pmtmp, ppath.lstep[ip-1], 0, 0, -1);
    
    stepwise_source(src_rad[ip], rvtmp, K_this, a, S, pmtmp, svtmp, svtmp, B, vtmp, rqtmp, 0);
  ...
  Matrix iy;
  get_iy_of_background(ws, iy, t3tmp, 
                       iy_trans_new, 0, 0, rqtmp,
                       ppath, {0}, 1, t_field, z_field, vmr_field, 
                       0, 1, f_grid, "1", surface_props_data,
                       iy_main_agenda, iy_space_agenda, iy_surface_agenda, 
                       iy_cloudbox_agenda, 1, verbosity);
  lvl_rad[np-1] = iy;
  
  // Radiative transfer calculations
  for( Index ip=np-2; ip>=0; ip-- )
    update_radiation_vector(lvl_rad[ip]=lvl_rad[ip+1],  rvtmp, rvtmp,
                            src_rad[ip], src_rad[ip+1], rvtmp, rvtmp,
                            lyr_tra[ip+1], tot_tra[ip], tmtmp, tmtmp,
