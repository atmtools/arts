    ...
    const Verbosity& )
{
  ...
  if( surface_type >= iy_surface_agenda_array.nelem()  )
    {
      ostringstream os;
      os << "*iy_surface_agenda_array* has only " << iy_surface_agenda_array.nelem()
         << " elements,\n while you have selected element " << surface_type;
      throw runtime_error( os.str() );
    }

  iy_surface_agenda_arrayExecute( ws, iy, diy_dx,
                                  surface_type,
    ...
    const Verbosity&  verbosity )
{
  ...
  chk_rte_pos( atmosphere_dim, rtp_pos );
  chk_rte_los( atmosphere_dim, rtp_los );
  chk_if_in_range_exclude( "surface skin temperature", surface_skin_t, 260.0, 373.0 );
  chk_if_in_range_exclude_high( "salinity", salinity, 0, 1 );
  chk_if_in_range_exclude_high( "wind speed", wind_speed, 0, 100 );
...
/* Workspace method: Doxygen documentation will be auto-generated */
void surface_rtpropCallAgendaX(
          Workspace&        ws,
          Numeric&          surface_skin_t,
          Matrix&           surface_los,
          Tensor4&          surface_rmatrix,
    ...
    const Verbosity& )
{
  ...
  surface_rtprop_agenda_arrayExecute( ws, surface_skin_t, surface_emission,
                                      surface_los, surface_rmatrix,
                                      surface_type,
                                      f_grid, rtp_pos, rtp_los,
                                      surface_type_aux, surface_rtprop_agenda_array );
}
    ...
    const Verbosity&       verbosity )
{
  ...
  Matrix         itw;
  rte_pos2gridpos( gp_lat[0], gp_lon[0], atmosphere_dim, lat_grid, lon_grid, rtp_pos );
  interp_atmsurface_gp2itw( itw, atmosphere_dim, gp_lat, gp_lon );

  // Skin temperature
  Vector skin_t(1);
  surface_props_interp( skin_t, "Water skin temperature",
                        atmosphere_dim, gp_lat, gp_lon, itw,
                        surface_props_data, surface_props_names );  

  // Wind speed
  Vector wind_speed(1);
  surface_props_interp( wind_speed, "Wind speed",
                        atmosphere_dim, gp_lat, gp_lon, itw,
                        surface_props_data, surface_props_names );  

  // Salinity
  Vector salinity(1);
  surface_props_interp( salinity, "Salinity",
                        atmosphere_dim, gp_lat, gp_lon, itw,
                        surface_props_data, surface_props_names );  

  // Call TESSEM
  surfaceTessem( surface_los, surface_rmatrix, surface_emission,
                 atmosphere_dim, stokes_dim, f_grid, rtp_pos, rtp_los,
                 skin_t[0], net_h, net_v, salinity[0], wind_speed[0], verbosity );
      ...
      if( irq >= 0 )
        {
          ...
          Matrix surface_los2;
          surfaceTessem( surface_los2, dsurface_rmatrix_dx[irq],
                         dsurface_emission_dx[irq], atmosphere_dim,
                         stokes_dim, f_grid, rtp_pos, rtp_los, skin_t[0]+dd,
                         net_h, net_v, salinity[0], wind_speed[0], verbosity );
          //
      ...
      if( irq >= 0 )
        {
          ...
          surfaceTessem( surface_los2, dsurface_rmatrix_dx[irq],
                         dsurface_emission_dx[irq], atmosphere_dim,
                         stokes_dim, f_grid, rtp_pos, rtp_los, skin_t[0],
                         net_h, net_v, salinity[0], wind_speed[0]+dd, verbosity );
          //
          dsurface_rmatrix_dx[irq] -= surface_rmatrix;
          dsurface_rmatrix_dx[irq] /= dd;
          dsurface_emission_dx[irq] -= surface_emission;
      ...
      if( irq >= 0 )
        {
          ...
          surfaceTessem( surface_los2, dsurface_rmatrix_dx[irq],
                         dsurface_emission_dx[irq], atmosphere_dim,
                         stokes_dim, f_grid, rtp_pos, rtp_los, skin_t[0],
                         net_h, net_v, salinity[0]+dd, wind_speed[0], verbosity );
          //
          dsurface_rmatrix_dx[irq] -= surface_rmatrix;
          dsurface_rmatrix_dx[irq] /= dd;
          dsurface_emission_dx[irq] -= surface_emission;
    ...
    const Verbosity&       verbosity )
{
  ...
  surface_props_interp( wind_direction, "Wind direction",
                        atmosphere_dim, gp_lat, gp_lon, itw,
                        surface_props_data, surface_props_names );  
  ...
  surfaceFastem( surface_los, surface_rmatrix, surface_emission,
                 atmosphere_dim, stokes_dim, f_grid, rtp_pos, rtp_los,
                 skin_t[0], salinity[0], wind_speed[0], wind_direction[0],
                 transmittance, fastem_version, verbosity );
      ...
      if( irq >= 0 )
        {
          const Numeric dd = 0.1;
          Matrix surface_los2;
          surfaceFastem( surface_los2, dsurface_rmatrix_dx[irq],
                         dsurface_emission_dx[irq], atmosphere_dim,
                         stokes_dim, f_grid, rtp_pos, rtp_los, skin_t[0]+dd,
                         salinity[0], wind_speed[0], wind_direction[0],
                         transmittance, fastem_version, verbosity );
          //
          dsurface_rmatrix_dx[irq] -= surface_rmatrix;
          dsurface_rmatrix_dx[irq] /= dd;
      ...
      if( irq >= 0 )
        {
          ...
          surfaceFastem( surface_los2, dsurface_rmatrix_dx[irq],
                         dsurface_emission_dx[irq], atmosphere_dim,
                         stokes_dim, f_grid, rtp_pos, rtp_los, skin_t[0],
                         salinity[0], wind_speed[0]+dd, wind_direction[0],
                         transmittance, fastem_version, verbosity );
          //
          dsurface_rmatrix_dx[irq] -= surface_rmatrix;
          dsurface_rmatrix_dx[irq] /= dd;
      ...
      if( irq >= 0 )
        {
          ...
          surfaceFastem( surface_los2, dsurface_rmatrix_dx[irq],
                         dsurface_emission_dx[irq], atmosphere_dim,
                         stokes_dim, f_grid, rtp_pos, rtp_los, skin_t[0],
                         salinity[0], wind_speed[0], wind_direction[0]+dd,
                         transmittance, fastem_version, verbosity );
          //
          dsurface_rmatrix_dx[irq] -= surface_rmatrix;
          dsurface_rmatrix_dx[irq] /= dd;
      ...
      if( irq >= 0 )
        {
          ...
          surfaceFastem( surface_los2, dsurface_rmatrix_dx[irq],
                         dsurface_emission_dx[irq], atmosphere_dim,
                         stokes_dim, f_grid, rtp_pos, rtp_los, skin_t[0],
                         salinity[0]+dd, wind_speed[0], wind_direction[0],
                         transmittance, fastem_version, verbosity );
          //
          dsurface_rmatrix_dx[irq] -= surface_rmatrix;
          dsurface_rmatrix_dx[irq] /= dd;
