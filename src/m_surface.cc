...
void surface_reflectivityFromGriddedField6(Tensor3& surface_reflectivity,
  ...
  }
  {
    ...
    const LagrangeInterpolation lag(0, 180 - rtp_los[0], r_field.get_numeric_grid(3), order, 0.5, false, Interpolation::LagrangeType::Linear);
...
void surface_scalar_reflectivityFromGriddedField4(
  ...
  }
  {
    ...
    const LagrangeInterpolation lag(0, 180 - rtp_los[0], r_field.get_numeric_grid(1), order, 0.5, false, Interpolation::LagrangeType::Linear);
    const auto itw=interpweights(lag);
    for (Index i = 0; i < nf_in; i++) {
