    ...
    const Verbosity&                  verbosity)
{
  ...
  ArrayOfRadiationVector lvl_rad(np, RadiationVector(nf, ns));
  ArrayOfArrayOfRadiationVector dlvl_rad(np, ArrayOfRadiationVector(nq,
                                                                    RadiationVector(
                                                                        nf,
                                                                        ns)));

  ArrayOfRadiationVector src_rad(np, RadiationVector(nf, ns));
  ArrayOfArrayOfRadiationVector dsrc_rad(np, ArrayOfRadiationVector(nq,
                                                                    RadiationVector(
                                                                        nf,
                                                                        ns)));
  ...
  if (np == 1 && rbi == 1)
  {  // i.e. ppath is totally outside the atmosphere:
    ppvar_p.resize(0);
    ppvar_t.resize(0);
    ppvar_vmr.resize(0, 0);
    ppvar_nlte.resize(0, 0);
  ...
  else
  {
    ...
    if (j_analytical_do)
    {
      ...
      for (Index ip = 0; ip < np; ip++)
      {
        dK_dx[ip].resize(nq);
        FOR_ANALYTICAL_JACOBIANS_DO (
            dK_dx[ip][iq] = PropagationMatrix(nf, ns);
        )
      }
      FOR_ANALYTICAL_JACOBIANS_DO (
          da_dx[iq] = StokesVector(nf, ns);
...
#pragma omp parallel for \
    ...
    {
      const Numeric dr_dT_past = do_hse ? ppath.lstep[ip - 1] /
                                          (2.0 * ppvar_t[ip - 1]) : 0;
      const Numeric dr_dT_this = do_hse ? ppath.lstep[ip - 1] /
                                          (2.0 * ppvar_t[ip]) : 0;
      stepwise_transmission(lyr_tra[ip], dlyr_tra_above[ip],
                            dlyr_tra_below[ip],
                            K[ip - 1], K[ip], dK_dx[ip - 1], dK_dx[ip],
                            ppath.lstep[ip - 1],
                            dr_dT_past, dr_dT_this,
                            temperature_derivative_position);
    }

  }

  const ArrayOfTransmissionMatrix tot_tra = cumulative_transmission(lyr_tra,
  ...
  get_iy_of_background(ws, iy, diy_dx,
                       iy_trans_new, iy_id, jacobian_do, jacobian_quantities,
                       ppath, rte_pos2, atmosphere_dim, t_field, z_field,
                       vmr_field,
  ...
  for (Index ip = np - 2; ip >= 0; ip--)
  {
    lvl_rad[ip] = lvl_rad[ip + 1];
    update_radiation_vector(lvl_rad[ip], dlvl_rad[ip], dlvl_rad[ip + 1],
                            src_rad[ip], src_rad[ip + 1], dsrc_rad[ip],
                            dsrc_rad[ip + 1],
                            lyr_tra[ip + 1], tot_tra[ip],
                            dlyr_tra_above[ip + 1], dlyr_tra_below[ip + 1],
                            RadiativeTransferSolver::Emission);
  }
  ...
  for (Index ip = 0; ip < lvl_rad.nelem(); ip++)
  {
    ...
    if (j_analytical_do)
      FOR_ANALYTICAL_JACOBIANS_DO (
          diy_dpath[iq](ip, joker, joker) = dlvl_rad[ip][iq];);
  }
