    ...
    for (Index j = 0; j < pfr_lon_grid.nelem(); j++) {
      if (pnd_field_raw.data(i_p, i, j) != 0.) {
        ...
        os << "Warning: \n"
           ...
           << "following position:\n"
           << "pressure = " << pfr_p_grid[i_p] << ", p_index = " << i_p << "\n"
           << "latitude = " << pfr_lat_grid[i] << ", lat_index = " << i << "\n"
           << "longitude = " << pfr_lon_grid[j] << ", lon_index = " << j << "\n"
           << "\n";
    ...
    for (Index j = 0; j < pfr_lon_grid.nelem(); j++) {
      if (pnd_field_raw.data(i, i_lat, j) != 0.) {
        ...
        os << "Warning: \n"
           ...
           << "or close the cloudbox boundary at the \n"
           << "following position:\n"
           << "pressure = " << pfr_p_grid[i] << ", p_index = " << i << "\n"
           << "latitude = " << pfr_lat_grid[i_lat] << ", lat_index = " << i_lat
           << "\n"
           << "longitude = " << pfr_lon_grid[j] << ", lon_index = " << j << "\n"
           << "\n";
        //throw runtime_error( os.str() );
    ...
    for (Index j = 0; j < pfr_lat_grid.nelem(); j++) {
      if (pnd_field_raw.data(i, j, i_lon) != 0.) {
        ...
        os << "Warning: \n"
           ...
           << "following position:\n"
           << "pressure = " << pfr_p_grid[i] << ", p_index = " << i << "\n"
           << "latitude = " << pfr_lat_grid[j] << ", lat_index = " << j << "\n"
           << "longitude = " << pfr_lon_grid[i_lon] << ", lon_index = " << i_lon
           << "\n"
           << "\n";
        //throw runtime_error( os.str() );
        out1 << os.str();
...
void bin_integral(Vector& w, const Vector& x, const Vector& y) {
  ...
  {
    bin_quadweights(w, x);
    for (Index i = 0; i < x.nelem(); i++) w[i] *= y[i];
...

{
  ...
  if (x.sum() ==
      0.0) {  // when x.sum()==0, but xwc!=0, obviously something went wrong in pnd calc
    ostringstream os;
    ...
    if (error > 1.10 || error < 0.90) {
      //CREATE_OUT1;
      out3
          << "WARNING: in WSM chk_pndsum in pnd_fieldCalcFromscat_speciesFields!\n"
