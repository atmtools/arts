...
void disort_settingsInit(DisortSettings&                    disort_settings,
  ...
  ARTS_USER_ERROR_IF(quadrature_dimension % 2, "Quadrature dimension ({}) must be even", quadrature_dimension);

  ARTS_USER_ERROR_IF(ray_path.size() < 2, "Must have at least one layer (two levels) to use disort solver");

  disort_settings.resize(quadrature_dimension,
                         legendre_polynomial_dimension,
                         fourier_mode_dimension,
                         freq_grid,
                         DescendingGrid{ray_path.begin(), ray_path.end(), [](auto& v) { return v.altitude(); }});
}
...
void disort_settingsSetSun(DisortSettings&             disort_settings,
  ...
  const Vector2 los = geometric_los(ray_point.pos, sun_pos, surf_field.ellipsoid);

  const Numeric sin2_alpha = sun.sin_alpha_squared(ray_point.pos, surf_field.ellipsoid);

  const Size nv = freq_grid.size();

  ARTS_USER_ERROR_IF(disort_settings.solar_source.size() != nv or static_cast<Size>(sun.spectrum.nrows()) != nv,
...
sun.spectrum.nrows():                {}
  ...
  for (Size iv = 0; iv < nv; iv++) { disort_settings.solar_source[iv] = sun.spectrum[iv, 0] * sin2_alpha; }
...
void disort_settingsNoLayerThermalEmission(DisortSettings& disort_settings) {
  ...
  disort_settings.source_polynomial.resize(disort_settings.frequency_count(), disort_settings.layer_count(), 0);
}

namespace {
template <typename T> void disort_settingsLayerThermalEmissionLinearInTauImpl(DisortSettings&      disort_settings,
                                                                              const T&             ray_points,
                                                                              const AscendingGrid& freq_grid) {
  ARTS_TIME_REPORT
  ...
  ARTS_USER_ERROR_IF(not same_shape({nv, N - 1}, disort_settings.optical_thicknesses),
...
#pragma omp parallel for if (not arts_omp_in_parallel())
  for (Size iv = 0; iv < nv; iv++) {
    ...
    for (Size i = 0; i < N - 1; i++) {
      ...
      const Numeric x0 = i == 0 ? 0.0 : disort_settings.optical_thicknesses[iv, i - 1];
...
void disort_settingsLayerThermalEmissionLinearInTau(DisortSettings&        disort_settings,
                                                    const ArrayOfAtmPoint& atm_path,
                                                    const AscendingGrid&   freq_grid) {
  disort_settingsLayerThermalEmissionLinearInTauImpl(disort_settings, atm_path, freq_grid);
}

void disort_settingsSubsurfaceLayerThermalEmissionLinearInTau(DisortSettings&               disort_settings,
                                                              const ArrayOfSubsurfacePoint& subsurf_profile,
                                                              const AscendingGrid&          freq_grid) {
  disort_settingsLayerThermalEmissionLinearInTauImpl(disort_settings, subsurf_profile, freq_grid);
}

void disort_settingsLayerNonThermalEmissionLinearInTau(DisortSettings&             disort_settings,
                                                       const ArrayOfAtmPoint&      atm_path,
...
void disort_settingsSurfaceEmissionByTemperature(DisortSettings&             disort_settings,
                                                 const AscendingGrid&        freq_grid,
                                                 const PropagationPathPoint& ray_point,
                                                 const SurfaceField&         surf_field) {
  ARTS_TIME_REPORT
  ...
  const Numeric T = surf_field.single_value(SurfaceKey::t, ray_point.latitude(), ray_point.longitude());

  auto& limit = disort_settings.upward_boundary_condition = 0.0;

  ARTS_USER_ERROR_IF(static_cast<Index>(nv) != limit.npages(),
                     "Frequency grid size does not match the upward boundary condition size: {} vs {}",
                     nv,
                     limit.npages())

  ARTS_USER_ERROR_IF(limit.nrows() < 1, "Must have at least one fourier mode to use the upward boundary condition.")

  for (Size iv = 0; iv < nv; iv++) { limit[iv, 0, joker] = planck(freq_grid[iv], T); }
}

void disort_settingsSubsurfaceDownwardEmissionBySurfaceTemperature(DisortSettings&             disort_settings,
                                                                   const AscendingGrid&        freq_grid,
                                                                   const PropagationPathPoint& ray_point,
                                                                   const SurfaceField&         surf_field) {
  ARTS_TIME_REPORT
  ...
  ARTS_USER_ERROR_IF(limit.nrows() < 1, "Must have at least one fourier mode to use the downward boundary condition.")

  for (Size iv = 0; iv < nv; iv++) { limit[iv, 0, joker] = planck(freq_grid[iv], T); }
}

void disort_settingsSubsurfaceEmissionByTemperature(DisortSettings&               disort_settings,
                                                    const AscendingGrid&          freq_grid,
                                                    const ArrayOfSubsurfacePoint& subsurf_profile) {
  ARTS_TIME_REPORT
  ...
  for (Size iv = 0; iv < nv; iv++) { limit[iv, 0, joker] = planck(freq_grid[iv], Tbot); }
}
...
void disort_settingsCosmicMicrowaveBackgroundRadiation(DisortSettings&      disort_settings,
                                                       const AscendingGrid& freq_grid) {
  ARTS_TIME_REPORT
  ...
  ARTS_USER_ERROR_IF(nv != disort_settings.downward_boundary_condition.npages(),
                     "Frequency grid size does not match the downward boundary condition size: {} vs {}",
                     nv,
                     disort_settings.downward_boundary_condition.npages())

  ARTS_USER_ERROR_IF(disort_settings.downward_boundary_condition.nrows() < 1,
                     "Must have at least one fourier mode to use the downward boundary condition.")

  for (Index iv = 0; iv < nv; iv++) {
    disort_settings.downward_boundary_condition[iv, 0, joker] =
        planck(freq_grid[iv], Constant::cosmic_microwave_background_temperature);
  }
}

void disort_settingsDownwellingObserver(const Workspace&                   ws,
  ...
  ARTS_USER_ERROR_IF(nv != limit.npages(),
  ...
  ARTS_USER_ERROR_IF(limit.ncols() != N, "Must have at least one quadrature dimension.")
...
#pragma omp parallel for if (not arts_omp_in_parallel()) firstprivate(spectral_rad, spectral_rad_jac, ray_path_up)
  for (Index i = 0; i < N; i++) {
    try {
      ...
      for (Index iv = 0; iv < nv; iv++) { limit[iv, 0, i] = dot(spectral_rad[iv], pol); }
...
void disort_settingsNoLegendre(DisortSettings& disort_settings) {
  ...
  ARTS_USER_ERROR_IF(disort_settings.legendre_coefficients.nrows() < 1,
...
void disort_settingsOpticalThicknessFromPath(DisortSettings&                    disort_settings,
                                             const ArrayOfPropagationPathPoint& ray_path,
                                             const ArrayOfPropmatVector&        spectral_propmat_path,
  ...
  ARTS_USER_ERROR_IF(not all_same_shape({nv}, spectral_propmat_path),
  ...
  ARTS_USER_ERROR_IF(
      stdr::any_of(spectral_propmat_path,
                   [](const PropmatVector& pms) { return stdr::any_of(pms, Cmp::eq<true>(), &Propmat::is_polarized); }),
      "No implementation for polarized propagation matrices.");

  const Vector r = [n = N, &ray_path]() {
    Vector out(n);
    for (Index i = 0; i < n; i++) { out[i] = ray_path[i].altitude() - ray_path[i + 1].altitude(); }
  ...
  for (Index iv = 0; iv < nv; iv++) {
    for (Index i = 0; i < N; i++) {
      disort_settings.optical_thicknesses[iv, i] =
          std::max(r[i] * std::midpoint(spectral_propmat_path[i + 1][iv].A(), spectral_propmat_path[i + 0][iv].A()),
                   min_optical_depth);
      if (i > 0) {
        disort_settings.optical_thicknesses[iv, i] += disort_settings.optical_thicknesses[iv, i - 1];
...
void disort_settingsSubsurfaceScalarAbsorption(DisortSettings&                    disort_settings,
                                               const ArrayOfPropagationPathPoint& ray_path,
                                               const ArrayOfSubsurfacePoint&      subsurf_profile,
                                               const Numeric&                     min_optical_depth) {
  ARTS_TIME_REPORT
  ...
  ARTS_USER_ERROR_IF(ray_path.size() != subsurf_profile.size() or subsurf_profile.size() != static_cast<Size>(N + 1),
                     "Wrong path size.")
...
ray_path: {:B,}
  ...
  ARTS_USER_ERROR_IF(not stdr::all_of(subsurf_profile, Cmp::contains(absorption_tag)),
                     R"(Missing '{}' in some or all of the subsurface profile)",
                     absorption_tag);

  for (Index i = 0; i < N; i++) {
    disort_settings.optical_thicknesses[joker, i] =
        std::max(r[i] * std::midpoint(subsurf_profile[i + 1][absorption_tag], subsurf_profile[i + 0][absorption_tag]),
                 min_optical_depth);
    if (i > 0) { disort_settings.optical_thicknesses[joker, i] += disort_settings.optical_thicknesses[joker, i - 1]; }
  }
}
...
void disort_settingsSurfaceLambertian(DisortSettings& disort_settings, const Vector& vec) {
  ...
  for (Index iv = 0; iv < disort_settings.frequency_count(); iv++) {
    disort_settings.bidirectional_reflectance_distribution_functions[iv, 0] =
        DisortBDRF{[value = vec[iv]](MatrixView x, const ConstVectorView&, const ConstVectorView&) { x = value; }};
  }
}
...
void disort_settingsSubsurfaceScalarSingleScatteringAlbedo(DisortSettings&               disort_settings,
                                                           const ArrayOfSubsurfacePoint& subsurf_profile) {
  ARTS_TIME_REPORT
  ...
  for (Index i = 0; i < N; i++) {
    disort_settings.single_scattering_albedo[joker, i] =
        std::midpoint(subsurf_profile[i + 1][ssa_tag], subsurf_profile[i + 0][ssa_tag]);
  }
}
...
void disort_settingsLegendreCoefficientsFromPath(DisortSettings&             disort_settings,
    ...
    for (Index iv = 0; iv < F; iv++) {
      for (Index j = 0; j < L; j++) {
        disort_settings.legendre_coefficients[iv, i, j] =
            invfac[j] * std::midpoint(spectral_phamat_spectral_path[i][iv, j][0, 0].real(),
  ...
  for (Size i = 0; i < N; i++) {
    for (Index iv = 0; iv < F; iv++) {
      ...
      if (std::isnormal(disort_settings.legendre_coefficients[iv, i, 0])) {
        disort_settings.legendre_coefficients[iv, i, joker] /= disort_settings.legendre_coefficients[iv, i, 0];

        //! WARNING: Numerical instabiliy occurs for large j-values in invfac cf what scattering_species does
        for (auto& v : disort_settings.legendre_coefficients[iv, i, joker]) { v = std::clamp(v, -1.0, 1.0); }
      } else {
...
void disort_settingsSingleScatteringAlbedoFromPath(DisortSettings&             disort_settings,
                                                   const ArrayOfPropmatVector& spectral_propmat_path,
  ...
  ARTS_USER_ERROR_IF(
      not all_same_shape({F}, spectral_absvec_scat_path, spectral_propmat_scat_path),
      "The shape of spectral_propmat_scat_path and spectral_absvec_scat_path must be {:B,}, at least one is not",
      std::array{F});

  ARTS_USER_ERROR_IF(not same_shape({F, N}, disort_settings.single_scattering_albedo),
...
#pragma omp parallel for if (not arts_omp_in_parallel()) collapse(2)
  for (Size i = 0; i < N; i++) {
    for (Index iv = 0; iv < F; iv++) {
      ...
      disort_settings.single_scattering_albedo[iv, i] = std::isnormal(x) ? x : 0.0;
    }
  }
}
ARTS_METHOD_ERROR_CATCH
...
namespace {
Agenda disort_settings_agendaSetup(const disort_settings_agenda_setup_layer_emission_type& layer_emission_setting,
  ...
  agenda.add("disort_settingsOpticalThicknessFromPath", SetWsv("min_optical_depth", min_optical_depth));
  ...
  switch (layer_emission_setting) {
    ...
    case LinearInTau:       agenda.add("disort_settingsLayerThermalEmissionLinearInTau"); break;
    case LinearInTauNonLTE: agenda.add("disort_settingsLayerNonThermalEmissionLinearInTau"); break;
  ...
  switch (space_setting) {
    ...
    case CosmicMicrowaveBackgroundRadiation: agenda.add("disort_settingsCosmicMicrowaveBackgroundRadiation"); break;
  }
  ...
  switch (surf_setting) {
    ...
    case ThermalLambertian:
      agenda.add("ray_pointLowestFromPath");
      agenda.add("disort_settingsSurfaceEmissionByTemperature");
      agenda.add("disort_settingsSurfaceLambertian", SetWsv{"value", surf_lambertian_value});
      break;
    case Lambertian:
      agenda.add("disort_settingsNoSurfaceEmission");
      agenda.add("disort_settingsSurfaceLambertian", SetWsv{"value", surf_lambertian_value});
...
Agenda disort_settings_agendaSubsurfaceSetup(const disort_settings_agenda_setup_sun_type& sun_setting,
                                             const Numeric&                               min_optical_depth) {
  ARTS_TIME_REPORT
  ...
  agenda.add("disort_settingsInit");
  agenda.add("disort_settingsSubsurfaceScalarAbsorption", SetWsv("min_optical_depth", min_optical_depth));
...
void disort_settings_agendaSetup(Agenda&        disort_settings_agenda,
  ...
  disort_settings_agenda =
      disort_settings_agendaSetup(to<disort_settings_agenda_setup_layer_emission_type>(layer_emission_setting),
...
void disort_settings_agendaSubsurfaceSetup(Agenda&        disort_settings_agenda,
  ...
  disort_settings_agenda =
      disort_settings_agendaSubsurfaceSetup(to<disort_settings_agenda_setup_sun_type>(sun_setting), min_optical_depth);
}

void CoupledAtmosphereAndSubsurfaceDisortSettings(const Workspace&     ws,
  ...
  ARTS_USER_ERROR_IF(alt_grid.front() > surf_elev or alt_grid.back() < surf_elev,
...
alt_grid:          {:B,}
  ...
  ArrayOfPropagationPathPoint subsurf_ray_path{
      std::from_range, std::span{alt_grid.begin(), surf_elev_iter} | stdv::transform([to_ppp](Numeric z) {
  ...
  ArrayOfPropagationPathPoint atm_ray_path{to_ppp(surf_elev, PathPositionType::atm)};
  atm_ray_path.append_range(std::span{surf_elev_iter + 1, alt_grid.end()} |
                            stdv::transform([to_ppp](Numeric z) { return to_ppp(z, PathPositionType::atm); }));
