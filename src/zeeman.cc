...
try
{
  ...
  const auto polarization_scale_dtheta_data = Zeeman::AllPolarization_dtheta(X.theta, X.eta);
  const auto polarization_scale_deta_data   = Zeeman::AllPolarization_deta(  X.theta, X.eta);
  
  for(auto polar : {Zeeman::Polarization::SigmaMinus, Zeeman::Polarization::Pi, Zeeman::Polarization::SigmaPlus}) {
    auto& pol         = Zeeman::SelectPolarization(polarization_scale_data,        polar);
    auto& dpol_dtheta = Zeeman::SelectPolarization(polarization_scale_dtheta_data, polar);
    auto& dpol_deta   = Zeeman::SelectPolarization(polarization_scale_deta_data,   polar);
                                    ...
                                    const Verbosity& verbosity)
{
  ...
  for(Index II = 0; II<abs_species.nelem(); II++) {
    ...
    if(not zero_values)
      for(auto& line: lines)
        line.ZeemanModelInit();
    else
      for(auto& line: lines)
        line.ZeemanModelInitZero();
