...
class Wsv {
  template <WorkspaceGroup T> static constexpr std::size_t ind = WorkspaceGroupInfo<T>::index;

  template <WorkspaceGroup T> static constexpr std::string_view name = WorkspaceGroupInfo<T>::name;
 ...
 public:
  ...
  //! Share
  template <WorkspaceGroup T> Wsv(std::shared_ptr<T>&& x) noexcept : data{std::move(x)}, index{ind<T>} {}

  //! Take over
  template <WorkspaceGroup T> Wsv(T&& x) noexcept : Wsv(std::shared_ptr<T>(new T{std::forward<T>(x)})) {}

  //! Share (for agenda sharing)
  template <WorkspaceGroup T> Wsv(T* x) noexcept : Wsv(std::shared_ptr<T>(x, [](void*) {})) {}

  //! Copy
  template <WorkspaceGroup T> Wsv(const T& x) : Wsv(std::shared_ptr<T>(new T{x})) {}

  [[nodiscard]] bool holds_same(const Wsv& x) const noexcept { return index == x.index; }

  template <WorkspaceGroup T> [[nodiscard]] bool holds() const noexcept { return index == ind<T>; }

  //! Unsafely share into the variant
  template <WorkspaceGroup T> [[nodiscard]] std::shared_ptr<T> share_unsafe() const {
    assert(holds<T>());
  ...
  template <WorkspaceGroup T> [[nodiscard]] std::shared_ptr<T> share() const {
    if (not holds<T>()) throw std::runtime_error(std::format(R"(Casting "{}" to "{}")", name<T>, type_name()));
