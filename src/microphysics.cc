  ...
  for (Index i = 0; i < N_se; i++) {
    if (std::isnan(scat_meta[scat_species][i].mass)) {
      ...
      os << "Use of size distribution " << psdname << " (as requested for\n"
         ...
         << "But mass is not given for scattering elements #" << i << "!";
      ...
      for (Index lat = limits[2]; lat < limits[3]; lat++) {
        for (Index lon = limits[4]; lon < limits[5]; lon++) {
          ...
          if (IWC_field(p, lat, lon) > 0.) {
            ...
            psd_cloudice_MH97(dNdD,
                              diameter_mass_equivalent,
                              IWC_field(p, lat, lon),
                              T,
                              noisy);
            ...
            if (dNdD.sum() ==
                0.0) {  // no particles at all were produced. means, there's some
            ...
            for (Index i = 0; i < N_se; i++) {
              pnd_field(intarr[i] + scat_data_start,
                        p - limits[0],
                        lat - limits[2],
                        lon - limits[4]) = pnd[i];
            }
  ...
  for (Index i = 0; i < N_se; i++) {
    if (std::isnan(scat_meta[scat_species][i].diameter_max)) {
      ...
      os << "Use of size distribution " << psdname << " (as requested for\n"
         ...
         << "But maximum diameter is not given for scattering elements #" << i
         << "!";
      throw runtime_error(os.str());
    }
      ...
      for (Index lat = limits[2]; lat < limits[3]; lat++) {
        for (Index lon = limits[4]; lon < limits[5]; lon++) {
          ...
          // values!). Calculating dNdD.
            ...
            for (Index i = 0; i < diameter_max.nelem();
                 i++)  //loop over number of scattering elements
            {
...
void pnd_fieldH13Shape(Tensor4View pnd_field,
  ...
  Vector Rarea(N_se,
               0.0);  // Area ratio = diameter_area_equivalent^2/diameter_max^2
  String partfield_name;
  ...
  for (Index i = 0; i < N_se; i++) {
    ...
    if (std::isnan(scat_meta[scat_species][intarr[i]]
    ...
    diameter_area_equivalent[i] = scat_meta[scat_species][intarr[i]]
                                      .diameter_area_equ_aerodynamical;  // [m]
  ...
  for (Iterator1D it = diameter_max.begin(); it != diameter_max.end(); ++it)
    if (find(diameter_max_in.begin(), diameter_max_in.end(), *it) ==
        diameter_max_in.end())
  ...
  if (diameter_max[0] < 7.7 * 1e-5) {
    ostringstream os;
    os << "The " << psdname
       << " parametrization is only valid for particles with\n"
      ...
      for (Index lat = limits[2]; lat < limits[3]; lat++) {
        for (Index lon = limits[4]; lon < limits[5]; lon++) {
          ...
          // values!). Calculating dNdD.
            ...
            // iteration over all given size bins
            for (Index i = 0; i < diameter_max_input.nelem();
                 i++)  //loop over number of scattering elements
            {
              // calculate particle size distribution for H13Shape
              // [# m^-3 m^-1]
              dNdD[i] = IWCtopnd_H13Shape(diameter_max_input[i],
                                          t_field(p, lat, lon));

              // calculate Area ratio distribution for H13Shape
              Ar[i] =
                  area_ratioH13(diameter_max_input[i], t_field(p, lat, lon));
            }
...
void pnd_fieldF07(Tensor4View pnd_field,
  ...
  // diameter_max.nelem()=0 implies no selected scattering element for the respective
  // scattering species field. should not occur anymore.
  if (diameter_max.nelem() == 0) return;
    ...
    for (Index p = limits[0]; p < limits[1]; p++) {
      for (Index lat = limits[2]; lat < limits[3]; lat++) {
        for (Index lon = limits[4]; lon < limits[5]; lon++) {
          ...
          if (std::isnan(WC_field(p, lat, lon)) ||
              std::isnan(N_field(p, lat, lon))) {
            if (logic_M) {
              ostringstream os;
              os << "Size distribution " << psdname
                 << " requires knowledge of mass "
...
void pnd_fieldMGD_LWC(Tensor4View pnd_field,
  ...
  for (Index i = 0; i < N_se; i++) {
    ...
    diameter_volume_equ_unsorted[i] =
        (scat_meta[scat_species][i].diameter_volume_equ);
  }
  get_sorted_indexes(intarr, diameter_volume_equ_unsorted);

  // extract scattering meta data
  for (Index i = 0; i < N_se; i++) {
    diameter_volume_equ[i] =
        scat_meta[scat_species][intarr[i]].diameter_volume_equ;  // [m]
      ...
      for (Index lat = limits[2]; lat < limits[3]; lat++) {
        for (Index lon = limits[4]; lon < limits[5]; lon++) {
          ...
          // values!). Calculating dNdD.
            ...
            // iteration over all given size bins
            for (Index i = 0; i < diameter_volume_equ.nelem();
                 i++)  //loop over number of scattering elements
            {
              // calculate particle size distribution for H11
              // [# m^-3 m^-1]
              dNdD[i] = LWCtopnd_MGD_LWC(
                  diameter_volume_equ[i], mean_rho, LWC_field(p, lat, lon));
            }
        ...
        for (Index lon = limits[4]; lon < limits[5]; lon++) {
          ...
          // values!). Calculating dNdD.
                 ...
                 i++)  //loop over number of scattering elements
            {
              // calculate particle size distribution for H11
              // [# m^-3 m^-1]
              dNdD[i] = IWCtopnd_MGD_IWC(
                  diameter_volume_equ[i], mean_rho, IWC_field(p, lat, lon));
            }
...
void pnd_fieldMP48(Tensor4View pnd_field,
  ...
  for (Index i = 0; i < N_se; i++) {
    mass[i] = scat_meta[scat_species][intarr[i]].mass;  // [kg]
    diameter_melted_equivalent[i] =
        pow(6. * mass[i] / PI / DENSITY_OF_WATER, 1. / 3.);  // [m]
    ...
    if (std::isnan(scat_meta[scat_species][intarr[i]].mass)) {
      ...
      os << "Use of size distribution " << psdname << " (as requested for\n"
         ...
         << "But volume is not given for scattering elements #" << i << "!";
      throw runtime_error(os.str());
    }
    vol[i] = PI / 6. *
             pow(scat_meta[scat_species][intarr[i]].diameter_volume_equ,
                 3.);  // [m3]
  }
  ...
  */
  ...
  const Numeric lambda_fac =
      41. * 1e2;  // [cm^-1] converted to [m^-1] to fit d[m]
        ...
        for (Index lon = limits[4]; lon < limits[5]; lon++) {
          ...
          if (PR_field(p, lat, lon) > 0.) {
            ...
            // did bulk mean density change?
            {
              if (n_it > 10) {
                ostringstream os;
                os << "ERROR: Bulk mean density for " << psdname
                   << " distribution is"
                   << " not converging.\n"
                   << "fractional change: "
            ...
            chk_pndsum(pnd, PWC, mass, p, lat, lon, partfield_name, verbosity);
      ...
      for (Index lat = limits[2]; lat < limits[3]; lat++) {
        for (Index lon = limits[4]; lon < limits[5]; lon++) {
          ...
          if (RWC_field(p, lat, lon) > 0.) {
            psd_rain_W16(
                dNdD, diameter_mass_equivalent, RWC_field(p, lat, lon));
...
void pnd_fieldH98(Tensor4View pnd_field,
  ...
  for (Index i = 0; i < N_se; i++) {
    ...
    diameter_volume_equivalent_unsorted[i] =
        (scat_meta[scat_species][i].diameter_volume_equ);
  }
...
*/
Numeric IWCtopnd_H13(const Numeric diameter_max, const Numeric t) {
  Numeric dNdD;
  Numeric la;
  Numeric mu;
