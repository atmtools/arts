 ...
 */
template <typename Scalar> std::complex<Scalar> refr_index_water_ellison07(Scalar frequency, Scalar temperature) {
  ...
  // static permittivity
  const Numeric epsilon_s = 87.9144 - 0.404399 * t_cels - 9.58726e-4 * pow2(t_cels) - 1.32802e-6 * pow3(t_cels);
  // Model parameters
  ...
  const Numeric tau4   = p7 + p8 * t_cels + p9 * pow2(t_cels) + p10 * pow3(t_cels);
  ...
  const Numeric epsilon_real =
      ...
      pow2(two_pi * tau4) * delta4 / 2. *
          (frequency * (f0 + frequency) / (1. + pow2(two_pi * tau4 * (f0 + frequency))) -
           frequency * (f0 - frequency) / (1. + pow2(two_pi * tau4 * (f0 - frequency)))) -
      pow2(two_pi * tau5) * delta5 / 2. *
          (frequency * (f1 + frequency) / (1. + pow2(two_pi * tau5 * (f1 + frequency))) -
           frequency * (f1 - frequency) / (1. + pow2(two_pi * tau5 * (f1 - frequency))));
  ...
  const Numeric epsilon_imag =
      ...
      pi * frequency * tau4 * delta4 *
          (1. / (1. + pow2(two_pi * tau4 * (f0 + frequency))) + 1. / (1. + pow2(two_pi * tau4 * (f0 - frequency)))) +
      pi * frequency * tau5 * delta5 *
          (1. / (1. + pow2(two_pi * tau5 * (f1 + frequency))) + 1. / (1. + pow2(two_pi * tau5 * (f1 - frequency))));
...
std::complex<Scalar> refr_index_ice_matzler06(Scalar frequency, Scalar temperature) {
...
template <typename Scalar> using VectorGen = matpack::data_t<Scalar, 1>;
template <typename Scalar> using MatrixGen = matpack::data_t<Scalar, 2>;
 ...
 */
template <std::floating_point Scalar>
VectorGen<std::complex<Scalar>> log_derivative(std::complex<Scalar> rho, size_t n_steps) {
 ...
 */
template <std::floating_point Scalar> class MieSphere {
 public:
  /** Create a Mie sphere.
     *
     ...
     */
  MieSphere(Scalar lambda, Scalar radius, std::complex<Scalar> n, StridedConstVectorView theta)
      : lambda_(lambda),
        r_(radius),
  ...
  /** Calculate scattering properties of a spherical water particle.
     ...
     */
  static MieSphere Liquid(Scalar frequency, Scalar temperature, Scalar radius, StridedConstVectorView theta) {
  ...
  /** Calculate scattering properties of a spherical ice particle.
     ...
     */
  static MieSphere Ice(Scalar frequency, Scalar temperature, Scalar radius, VectorGen<Scalar> theta) {
    Scalar       c      = 2.99792458e8;
    Scalar       lambda = c / frequency;
  ...
  Scalar get_scattering_coeff() { return q_sca_ * std::numbers::pi_v<Scalar> * pow(r_, 2); }

  /// The scattering efficiency.
  Scalar get_scattering_eff() { return q_sca_; }

  /// The back-scattering coefficient.
  Scalar get_backscattering_coeff() { return q_back_ * std::numbers::pi_v<Scalar> * pow(r_, 2); }

  /// The back-scattering efficiency.
  Scalar get_backscattering_eff() { return q_back_; }

  /// The extinction coefficient.
  Scalar get_extinction_coeff() { return q_ext_ * std::numbers::pi_v<Scalar> * pow(r_, 2); }

  /// The extinction efficiency.
  Scalar get_extinction_eff() { return q_ext_; }

  /// The extinction coefficient.
  Scalar get_absorption_coeff() { return (q_ext_ - q_sca_) * std::numbers::pi_v<Scalar> * pow(r_, 2); }
  ...
  VectorGen<Scalar> get_phase_function() {
    ...
    for (Index i = 0; i < results.size(); ++i) {
      results[i] = 0.5 * (std::pow(std::abs(s_1_[i]), 2) + std::pow(std::abs(s_2_[i]), 2)) / (k * k);
    }
    return results;
  }
  ...
  MatrixGen<Scalar> get_scattering_matrix_compact() {
    ...
    for (Index i = 0; i < n_angles; ++i) {
      s11[i] = 0.5 * (std::pow(std::abs(s_1_[i]), 2) + std::pow(std::abs(s_2_[i]), 2));
      s12[i] = 0.5 * (std::pow(std::abs(s_2_[i]), 2) - std::pow(std::abs(s_1_[i]), 2));
      s33[i] = 0.5 * std::real(s_1_[i] * std::conj(s_2_[i]) + s_2_[i] * std::conj(s_1_[i]));
      s34[i] = -0.5 * std::imag(s_1_[i] * std::conj(s_2_[i]) - s_2_[i] * std::conj(s_1_[i]));
    }
 ...
 private:
  ...
  void calculate_mie_parameters() {
    ...
    std::complex<Numeric> a_n{0.0}, b_n{0.0}, a_n_1{0.0}, b_n_1{0.0}, q_back_acc{0.0};

    VectorGen<Numeric> mu = theta_;
    std::transform(mu.begin(), mu.end(), mu.begin(), [](Numeric x) { return cos(Conversion::deg2rad(x)); });
    ...
    for (size_t step = 0; step < n_steps_x; ++step) {
      ...
      g_sca_            += (n2p1_nnp1 * (a_n.real() * b_n.real() + a_n.imag() * b_n.imag()));
      if (step > 0) {
        g_sca_ +=
            (((step_f - 1.0) * (step_f + 1.0) / step_f) * (a_n_1.real() * a_n.real() + a_n_1.imag() * a_n.imag() +
      ...
      for (Index i = 0; i < n_angs; ++i) { pi[i] = (n2p1 * mu[i] * pi_1[i] - (step_f + 1.0) * pi_2[i]) / step_f; }
    }
