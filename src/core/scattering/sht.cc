...
Vector SHT::get_azimuth_angle_grid(Index n_aa, bool radians) {
  ...
  for (Index i = 0; i < n_aa; ++i) { result[i] = dx * static_cast<double>(i); }
  if (radians) { result *= Conversion::deg2rad(1.0); }
  return result;
}

FejerGrid SHT::get_zenith_angle_grid(bool radians) const { return get_zenith_angle_grid(n_za_, radians); }

std::shared_ptr<const ZenithAngleGrid> SHT::get_za_grid_ptr() const { return za_grid_; }
...
std::array<Index, 4> SHT::get_config_lm(Index l_max, Index m_max) {
  return {l_max, m_max, (m_max > 0) ? 2 * m_max + 2 : 1, (l_max > 0) ? 2 * l_max + 2 : 1};
}
...
Vector SHT::get_azimuth_angle_grid(bool radians) { return SHT::get_azimuth_angle_grid(n_aa_, radians); }
...
void SHT::set_spectral_coeffs(const matpack::strided_view_t<const Complex, 1> &view) const {
...
void SHT::set_spectral_coeffs_cmplx(const matpack::strided_view_t<const Complex, 1> &view) const {
  // Input size must match number of spectral coefficients of SHT.
...
StridedConstMatrixView SHT::get_spatial_coeffs() const {
  return StridedConstMatrixView{matpack::mdview_t<Numeric, 2>(spatial_coeffs_, std::array{n_aa_, n_za_})};
}

StridedConstComplexMatrixView SHT::get_spatial_coeffs_cmplx() const {
  return StridedConstComplexMatrixView{matpack::mdview_t<Complex, 2>(spatial_coeffs_cmplx_, std::array{n_aa_, n_za_})};
}

StridedConstComplexVectorView SHT::get_spectral_coeffs_cmplx() const {
  return StridedConstComplexVectorView{
      matpack::mdview_t<Complex, 1>(spectral_coeffs_cmplx_, std::array{n_spectral_coeffs_cmplx_})};
}
...
Index SHT::get_n_spectral_coeffs_cmplx() const { return n_spectral_coeffs_cmplx_; }
...
StridedConstComplexVectorView SHT::get_spectral_coeffs() const {
  return StridedConstComplexVectorView{matpack::mdview_t<Complex, 1>(spectral_coeffs_, std::array{n_spectral_coeffs_})};
}
...
ComplexVector SHT::transform_cmplx(const StridedConstComplexMatrixView &view [[maybe_unused]]) {
#ifdef ARTS_NO_SHTNS
  ARTS_USER_ERROR("Not compiled with SHTNS or FFTW support.");
#else
  if (is_trivial_) {
...
ComplexMatrix SHT::synthesize_cmplx(const StridedConstComplexVectorView &view [[maybe_unused]]) {
#ifdef ARTS_NO_SHTNS
  ARTS_USER_ERROR("Not compiled with SHTNS or FFTW support.");
#else
  if (is_trivial_) {
...
Numeric SHT::evaluate(const StridedConstComplexVectorView &view [[maybe_unused]],
  ...
  ARTS_USER_ERROR("Not compiled with SHTNS or FFTW support.");
#else
  if (is_trivial_) { return view[0].real(); }
  set_spectral_coeffs(view);
  auto shtns = ShtnsHandle::get(l_max_, m_max_, n_aa_, n_za_);
...
Vector SHT::evaluate(const StridedComplexVectorView &view [[maybe_unused]],
  ...
  for (auto i = 0; i < n_points; ++i) {
    result[i] = SH_to_point(shtns, spectral_coeffs_, std::cos(points[i, 1]), points[i, 0]);
  }
  shtns_mutex.lock();
...
Vector SHT::evaluate(const StridedConstComplexVectorView &view [[maybe_unused]],
  ...
  if (is_trivial_) {
    ...
    for (Size i = 0; i < view.size(); ++i) { results[i] = view[i].real(); }
    return results;
  }
  assert(m_max_ == 0);
  set_spectral_coeffs(view);
  ...
  shtns_mutex.lock();
  for (Size i = 0; i < n_points; ++i) { result[i] = SH_to_point(shtns, spectral_coeffs_, cos(thetas[i]), 0.0); }
  shtns_mutex.unlock();
...
struct FFTWDeleter {
  template <typename T> void operator()(T *t [[maybe_unused]]) {
#ifdef ARTS_NO_SHTNS
    ARTS_USER_ERROR("Not compiled with SHTNS or FFTW support.");
#else
    if (t) {
...
template <typename Numeric> FFTWArray<Numeric>::FFTWArray(Index n [[maybe_unused]]) : ptr_(nullptr) {
#ifdef ARTS_NO_SHTNS
  ARTS_USER_ERROR("Not compiled with SHTNS or FFTW support.");
#else
  if (n > 0) {
    shtns_mutex.lock();
    ptr_ = std::shared_ptr<Numeric>(reinterpret_cast<Numeric *>(fftw_malloc(n * sizeof(Numeric))), FFTWDeleter());
    shtns_mutex.unlock();
  }
#endif
}
...
SHT::SHT(Index l_max, Index m_max, Index n_aa, Index n_za) : l_max_(l_max), m_max_(m_max), n_aa_(n_aa), n_za_(n_za) {
#ifdef ARTS_NO_SHTNS
  ARTS_USER_ERROR("Not compiled with SHTNS or FFTW support.");
#else
  if (l_max == 0) {
    ...
    spectral_coeffs_cmplx_   = sht::FFTWArray<std::complex<double> >(n_spectral_coeffs_cmplx_);
...
SHT::SHT(Index l_max, Index m_max)
    : SHT(l_max, m_max, (m_max > 0) ? 2 * m_max + 2 : 1, (l_max > 0) ? 2 * l_max + 2 : 1) {}
...
Vector SHT::get_cos_za_grid() {
  ...
  ARTS_USER_ERROR("Not compiled with SHTNS or FFTW support.");
#else
  if (is_trivial_) { return Vector(1, 0.0); }
  auto   shtns = ShtnsHandle::get(l_max_, m_max_, n_aa_, n_za_);
  Vector result(n_za_);
...
ArrayOfIndex SHT::get_l_indices() {
  ...
  ARTS_USER_ERROR("Not compiled with SHTNS or FFTW support.");
#else
  if (is_trivial_) { return {0}; }
  auto         shtns = ShtnsHandle::get(l_max_, m_max_, n_aa_, n_za_);
  ArrayOfIndex result(n_spectral_coeffs_);
  for (Index i = 0; i < n_spectral_coeffs_; ++i) { result[i] = shtns->li[i]; }
  return result;
#endif
}
...
ArrayOfIndex SHT::get_m_indices() {
  ...
  auto         shtns = ShtnsHandle::get(l_max_, m_max_, n_aa_, n_za_);
  ArrayOfIndex result(n_spectral_coeffs_);
  for (Index i = 0; i < n_spectral_coeffs_; ++i) { result[i] = shtns->mi[i]; }
  return result;
#endif
}
...
std::shared_ptr<SHT> SHTProvider::get_instance(SHTParams params [[maybe_unused]]) {
  ...
  if (sht_instances_.count(params) == 0) {
    sht_instances_[params] = std::make_shared<SHT>(params[0], params[1], params[2], params[3]);
  }
