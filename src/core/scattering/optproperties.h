...
template <> struct std::formatter<ParticleSSDMethod> {
  format_tags tags;
  ...
  template <class FmtContext> FmtContext::iterator format(const ParticleSSDMethod& v, FmtContext& ctx) const {
    const std::string_view quote = tags.quote();

    switch (v) {
      case PARTICLE_SSDMETHOD_NONE:    tags.format(ctx, quote, "PARTICLE_SSDMETHOD_NONE"sv, quote); break;
      case PARTICLE_SSDMETHOD_TMATRIX: tags.format(ctx, quote, "PARTICLE_SSDMETHOD_TMATRIX"sv, quote); break;
      default:                         tags.format(ctx, quote, "PARTICLE_SSDMETHOD_UNKNOWN"sv, quote); break;
    }
...
template <> struct std::formatter<ScatteringMetaData> {
  format_tags tags;
  ...
  template <class FmtContext> FmtContext::iterator format(const ScatteringMetaData& v, FmtContext& ctx) const {
...
template <> struct std::formatter<PType> {
  format_tags tags;
  ...
  template <class FmtContext> FmtContext::iterator format(const PType& v, FmtContext& ctx) const {
    const std::string_view quote = tags.quote();

    switch (v) {
      case PTYPE_GENERAL:     tags.format(ctx, quote, "PTYPE_GENERAL"sv, quote); break;
      case PTYPE_AZIMUTH_RND: tags.format(ctx, quote, "PTYPE_AZIMUTH_RND"sv, quote); break;
      case PTYPE_TOTAL_RND:   tags.format(ctx, quote, "PTYPE_TOTAL_RND"sv, quote); break;
...
template <> struct std::formatter<SingleScatteringData> {
  format_tags tags;
  ...
  template <class FmtContext> FmtContext::iterator format(const SingleScatteringData& v, FmtContext& ctx) const {
...
template <> struct xml_io_stream<SingleScatteringData> {
  ...
  static void read(std::istream& is, SingleScatteringData& x, bifstream* pbifs = nullptr);
};

template <> struct xml_io_stream<ScatteringMetaData> {
  ...
  static void read(std::istream& is, ScatteringMetaData& x, bifstream* pbifs = nullptr);
};
