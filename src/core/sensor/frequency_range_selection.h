...
struct FrequencyResponsePath {
  ...
  [[nodiscard]] std::optional<Numeric> map_to_local(Numeric global_frequency) const;
...
struct HeterodyneFrequencyRange final : FrequencyRange {
  ...
  HeterodyneFrequencyRange(Numeric clock_frequency, const Vector2& bandpass_range);
  ...
  [[nodiscard]] Vector local_response(ConstVectorView local_frequency_grid, Size path_index = 0) const;
  [[nodiscard]] Vector global_response(ConstVectorView global_frequency_grid, Size path_index = 0) const;
};
}  // namespace sensor

// FrequencyResponsePath format tags and XML I/O

template <> struct format_tag_aggregate<sensor::FrequencyResponsePath> {
  constexpr static bool value = true;
};

template <> struct xml_io_stream_name<sensor::FrequencyResponsePath> {
  static constexpr std::string_view name = "SensorFrequencyResponsePath";
};

template <> struct xml_io_stream_aggregate<sensor::FrequencyResponsePath> {
  static constexpr bool value = true;
};

// FrequencyRange format tags and XML I/O

template <> struct format_tag_aggregate<sensor::FrequencyRange> {
  constexpr static bool value = true;
};

template <> struct xml_io_stream_name<sensor::FrequencyRange> {
  static constexpr std::string_view name = "SensorFrequencyRange";
};

template <> struct xml_io_stream_aggregate<sensor::FrequencyRange> {
  static constexpr bool value = true;
};

// HeterodyneFrequencyRange format tags and XML I/O

template <> struct std::formatter<sensor::HeterodyneFrequencyRange>
    : format_tag_inherit<sensor::FrequencyRange, sensor::HeterodyneFrequencyRange> {};

template <> struct xml_io_stream<sensor::HeterodyneFrequencyRange>
    : xml_io_stream_inherit<sensor::FrequencyRange, sensor::HeterodyneFrequencyRange> {};
