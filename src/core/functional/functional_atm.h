...
struct External {
  ...
  [[nodiscard]] std::vector<std::pair<Index, Numeric>> w(Numeric, Numeric, Numeric) const;
};
};  // namespace Atm

template <> struct xml_io_stream_name<Atm::External> {
  static constexpr std::string_view name = "External";
};

template <> struct xml_io_stream_aggregate<Atm::External> {
