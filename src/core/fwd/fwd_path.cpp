...
constexpr Size xpos(const Numeric x, const Vector& x_grid) try {
  ...
  return std::min<Size>(std::max<Index>(0, std::distance(x_grid.begin(), stdr::lower_bound(x_grid, x)) - 1),
                        static_cast<Size>(x_grid.size()) - 2);
}
ARTS_METHOD_ERROR_CATCH

constexpr Numeric xweight(const Numeric x, const Vector& x_grid, const Size i0) try {
...
constexpr void check_grid(const Numeric     x [[maybe_unused]],
  ...
  ARTS_USER_ERROR_IF(x_grid.front() > x, "Below lower {} point; {} < {}", name, x, x_grid.front())
  ARTS_USER_ERROR_IF(x_grid.back() < x, "Above upper {} point; {} > {}", name, x, x_grid.back());
}

constexpr path find_path(
    const Vector3 pos, const Vector2 los, const AscendingGrid& alt, const LatGrid& lat, const LonGrid& lon) try {
  ...
  if (alt.size() > 1 and out.alt_weight == 0.0 and not stdr::binary_search(alt, pos[0])) {
    out.alt_index  -= 1;
    out.alt_weight  = 1.0;
  }

  if (alt.size() > 1 and out.lat_weight == 0.0 and not stdr::binary_search(lat, pos[1])) {
    out.lat_index  -= 1;
    out.lat_weight  = 1.0;
  }

  if (alt.size() > 1 and out.lon_weight == 0.0 and not stdr::binary_search(lon, pos[2])) {
    out.lon_index  -= 1;
    out.lon_weight  = 1.0;
  }
...
void path_from_propagation_path(std::vector<path>&                 out,
  ...
  std::transform(propagation_path.begin(), propagation_path.end(), out.begin(), [&](const PropagationPathPoint& pp) {
  ...
  if (propagation_path.size()) {
    ...
    for (Size i = 1; i < out.size(); i++) {
      out[i].distance = ::path::distance(out[i - 1].point.pos, out[i].point.pos, ellipsoid);
    }
  }
}
ARTS_METHOD_ERROR_CATCH

std::vector<path> path_from_propagation_path(const ArrayOfPropagationPathPoint& propagation_path,
                                             const AscendingGrid&               alt,
                                             const LatGrid&                     lat,
                                             const LonGrid&                     lon,
...
std::vector<path> geometric_planar(
    const Vector3 pos, const Vector2 los, const AscendingGrid& alt, const LatGrid& lat, const LonGrid& lon) {
  ...
  if (up_looking) {
    while (path.back().point.altitude() != alt.back()) {
      ...
      newpos.distance          = std::abs(newpos.point.altitude() - oldpos.point.altitude()) * csc;
    }
  ...
  while (path.size() > 1 and path[1].distance == 0.0) { path.erase(path.begin()); }
