...
struct spectral_rad {
  ...
  matpack::data_t<std::function<Stokvec(Numeric, Vector2)>, 2> spectral_rad_surface;
  matpack::data_t<std::function<Stokvec(Numeric, Vector2)>, 2> spectral_rad_space;
  ...
  StokvecVector operator()(const Numeric f, const std::vector<path>& path_points, spectral_rad::as_vector) const;
  ...
  void from_path(std::vector<path>& out, const ArrayOfPropagationPathPoint& propagation_path) const;

  [[nodiscard]] std::array<weighted_position, 8> pos_weights(const path& pp) const;

  [[nodiscard]] Stokvec B(const Numeric f, const std::array<weighted_position, 8>& pos) const;

  [[nodiscard]] Stokvec Iback(const Numeric f, const std::array<weighted_position, 8>& pos, const path& pp) const;

  [[nodiscard]] std::pair<Propmat, Stokvec> PM(const Numeric                           f,
...
template <> struct std::formatter<SpectralRadianceOperator> {
  format_tags tags;
  ...
  template <class FmtContext> FmtContext::iterator format(const SpectralRadianceOperator& v, FmtContext& ctx) const {
    return tags.format(ctx, "operator over\n  alt: "sv, v.alt, "\n  lat: "sv, v.lat, "\n  lon: "sv, v.lon);
  }
};

template <> struct xml_io_stream<SpectralRadianceOperator> {
  ...
  static void read(std::istream& is, SpectralRadianceOperator& x, bifstream* pbifs = nullptr);
};
