...
template <typename T> struct xml_io_stream_name {
  static constexpr std::string_view name = "<unknown>"sv;
};

template <typename T> constexpr std::string_view xml_io_stream_name_v = xml_io_stream_name<T>::name;
 ...
 */
template <typename T> struct xml_io_stream {
  constexpr static std::string_view type_name = xml_io_stream_name_v<T>;

  // Core IO
  static void write(std::ostream&, const T&, bofstream* = nullptr, std::string_view = ""sv) = delete;
  ...
  // XML extension (optional)
  static void extend(std::istream&, T&, bifstream* = nullptr) = delete;
  static void append(std::istream&, T&, bifstream* = nullptr) = delete;
};
...
template <typename T>
concept arts_xml_ioable = xml_io_writable<T> and xml_io_readable<T> and xml_io_nameable<T>;
...
concept arts_xml_appendable = arts_xml_ioable<T> and requires(T a) {
  ...
  xml_io_stream<T>::append(std::declval<std::istream&>(), a);
};

//! Test that the type is ready for binary data also
template <typename T>
concept xml_io_binary = arts_xml_ioable<T> and requires(std::span<const T, 1> a) {
...
template <typename T>
concept xml_io_parseable = requires(std::span<T> b, std::istream& is) { xml_io_stream<T>::parse(b, is); };
