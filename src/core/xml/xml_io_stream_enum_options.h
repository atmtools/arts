...
template <xml_enum_option T> struct xml_io_stream_name<T> {
  static constexpr std::string_view name = enumdocs<T>::name;
};

template <xml_enum_option T> struct xml_io_stream<T> {
  static constexpr std::string_view type_name = xml_io_stream_name_v<T>;

  static void write(std::ostream& os, const T& x, bofstream* = nullptr, std::string_view name = ""sv) {
  ...
  static void read(std::istream& is, T& x, bifstream* = nullptr) try {
    ...
    try {
      ...
      throw std::runtime_error(std::format("Failed to convert '{}' to enum {}:\n{}", value, type_name, e.what()));
    }

    tag.read_from_stream(is);
    tag.check_end_name(type_name);
  } catch (const std::exception& e) {
    throw std::runtime_error(std::format("Cannot read enum {}:\n{}", type_name, e.what()));
  }
};
