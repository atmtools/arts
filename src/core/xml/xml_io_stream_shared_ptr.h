...
template <typename T> struct xml_io_stream_name<std::shared_ptr<T>> {
  static constexpr std::string_view name = "Shared"sv;
};

template <typename T> requires(arts_xml_ioable<std::remove_const_t<T>> and std::is_default_constructible_v<T>)
struct xml_io_stream<std::shared_ptr<T>> {
  using mutT = std::remove_const_t<T>;

  static constexpr std::string_view type_name = xml_io_stream_name_v<std::shared_ptr<T>>;

  static void write(std::ostream&             os,
                    const std::shared_ptr<T>& x,
                    bofstream*                pbofs = nullptr,
                    std::string_view          name  = ""sv) {
    XMLTag tag{type_name, "name", name, "type", xml_io_stream<mutT>::type_name, "notnull", Index{bool{x}}};
  ...
  static void read(std::istream& is, std::shared_ptr<T>& x, bifstream* pbifs = nullptr) try {
    XMLTag tag;
