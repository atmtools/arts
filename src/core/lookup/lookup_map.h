...
struct table {
  ...
  */
  ...
  [[nodiscard]] std::array<lagrange_interp::lag_t<-1>, 1> pressure_lagrange(const Numeric& pressure,
                                                                            const Index    interpolation_order,
                                                                            const Numeric& extpolation_factor) const;

  [[nodiscard]] std::vector<lagrange_interp::lag_t<-1>> frequency_lagrange(const Vector&  freq_grid,
                                                                           const Index    interpolation_order,
                                                                           const Numeric& extpolation_factor) const;
...
template <> struct std::formatter<AbsorptionLookupTable> {
  format_tags tags;
  ...
  template <class FmtContext> FmtContext::iterator format(const AbsorptionLookupTable& v, FmtContext& ctx) const {
...
template <> struct xml_io_stream<AbsorptionLookupTable> {
  ...
  static void read(std::istream& is, AbsorptionLookupTable& x, bifstream* pbifs = nullptr);
};
