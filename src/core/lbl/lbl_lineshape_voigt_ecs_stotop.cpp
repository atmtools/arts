...
std::function<Numeric(Rational)> erot_selection(const SpeciesIsotope& isot) {
  ...
  if (isot.spec == SpeciesEnum::Ammonia and isot.isotname == "4111") {
  ...
  if (isot.spec == SpeciesEnum::Phosphine and isot.isotname == "1111") {
    return [](const Rational J) -> Numeric {
  ...
  ARTS_USER_ERROR(
      "{} has no rotational energies for symmetric top ECS in ARTS",
      isot.FullName())
  return [](const Rational J) -> Numeric {
...
void relaxation_matrix_offdiagonal(MatrixView& W,
  ...
  const std::array rats{bnd.max(QuantumNumberType::J), maxK};
  ...
  for (Size i = 0; i < n; i++) {
    ...
    for (Size j = i + 1; j < n; j++) {
      ...
      if (std::abs(sumlw) <= std::abs(sumup) * 1e-6) {
        ...
        W[i, j] =
            W[j, i] *
            std::exp((erot(Ji) - erot(Jj)) / kelvin2joule(T));
      }
    }
  }
}
