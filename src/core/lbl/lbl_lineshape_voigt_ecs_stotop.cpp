...
std::function<Numeric(Rational)> erot_selection(const SpeciesIsotope& isot) {
  ...
  if (isot == "NH3-4111"_isot) {
    return [](const Rational J) -> Numeric { return Conversion::kaycm2joule(9.9402) * Numeric(J * (J + 1)); };
  }

  // PH3 main isotopologue (31P-1H3)
  if (isot == "PH3-1111"_isot) {
    return [](const Rational J) -> Numeric { return Conversion::kaycm2joule(4.4522) * Numeric(J * (J + 1)); };
  }

  ARTS_USER_ERROR("{} has no rotational energies for symmetric top ECS in ARTS", isot.FullName())
...
Numeric reduced_dipole(const Rational Jf, const Rational Ji, const Rational K) {
  ...
  if (not iseven(Jf + K + 1)) return -sqrtr(2 * Jf + 1) * wigner3j(Jf, Rational{1}, Ji, K, Rational{0}, -K);
