...
std::function<Numeric(Rational)> erot_selection(const SpeciesIsotope& isot) {
  ...
  if (isot == "CH4-211"_isot) {
    return [](const Rational J) -> Numeric { return Conversion::kaycm2joule(5.2410) * Numeric(J * (J + 1)); };
  }

  ARTS_USER_ERROR("{} has no rotational energies for spherical top ECS in ARTS", isot.FullName())
...
Numeric reduced_dipole(const Rational Jf, const Rational Ji) {
  ...
  if (not iseven(Jf + 1))
    return -sqrtr(2 * Jf + 1) * wigner3j(Jf, Rational{1}, Ji, Rational{0}, Rational{0}, Rational{0});
  return +sqrtr(2 * Jf + 1) * wigner3j(Jf, Rational{1}, Ji, Rational{0}, Rational{0}, Rational{0});
}
...
void relaxation_matrix_offdiagonal(MatrixView&                     W,
  ...
  for (Size i = 0; i < n; i++) {
    ...
    for (Size j = 0; j < n; j++) {
      ...
      Numeric sum = 0;
      for (; L <= Lf; L += 2) {
        const Numeric a  = wig3(Ji, Ji_p, Rational{L}, Rational{0}, Rational{0}, Rational{0});
        const Numeric b  = wig3(Jf, Jf_p, Rational{L}, Rational{0}, Rational{0}, Rational{0});
