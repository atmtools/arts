...
using QuantumIdentifierGriddedField1Map = std::unordered_map<QuantumIdentifier, SortedGriddedField1>;
...
QuantumIdentifierNumericMap createBji(const QuantumIdentifierNumericMap& Bij, const AbsorptionBands& abs_bands);
...
Vector nlte_ratio_sum(const ArrayOfAtmPoint& atm_path, const ArrayOfQuantumLevelIdentifier& levels);

struct UppLow {
  Size upp, low;
};

Size band_level_mapUniquestIndex(const std::unordered_map<QuantumIdentifier, UppLow>& band_level_map);

std::unordered_map<QuantumIdentifier, UppLow> band_level_mapFromLevelKeys(
    const AbsorptionBands& abs_bands, const ArrayOfQuantumLevelIdentifier& level_keys);

Size level_count(const std::unordered_map<QuantumIdentifier, UppLow>& band_level_map);
...
Numeric set_nlte(AtmPoint& atm_point, const ArrayOfQuantumLevelIdentifier& level_keys, const Vector& x);
}  // namespace lbl::nlte
