...
void calculate(PropmatVectorView        pm,
  ...
  auto voigt_abs_ecs_data    = init_voigt_abs_ecs_data(f_grid[f_range], bnds, atm, los);

  const auto calc_voigt_lte =
      [&](const QuantumIdentifier& bnd_key, const band_data& bnd, const ZeemanPolarization pol) {
        voigt::lte::calculate(
            pm, dpm, *voigt_lte_data, f_grid, f_range, jac_targets, bnd_key, bnd, atm, pol, no_negative_absorption);
      };
  ...
  const auto calc_voigt_line_nlte =
      [&](const QuantumIdentifier& bnd_key, const band_data& bnd, const ZeemanPolarization pol) {
  ...
  const auto calc_switch = [&](const QuantumIdentifier& bnd_key, const band_data& bnd, const ZeemanPolarization pol) {
    switch (bnd.lineshape) {
      case LineByLineLineshape::VP_LTE:          calc_voigt_lte(bnd_key, bnd, pol); break;
  ...
  for (auto pol : {ZeemanPolarization::pi, ZeemanPolarization::sm, ZeemanPolarization::sp}) {
