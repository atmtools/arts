...
namespace {
Numeric level_density(Numeric T, Numeric g, Numeric E, const SpeciesIsotope& spec) {
  using Constant::k;
  return g * std::exp(-E / (k * T)) / PartitionFunctions::Q(T, spec);
}

AtmFunctionalData level_density(const AtmFunctionalData& T, Numeric g, Numeric E, const SpeciesIsotope& spec) {
  return {.f = [g, E, spec, T](Numeric alt, Numeric lat, Numeric lon) -> Numeric {
    return level_density(T(alt, lat, lon), g, E, spec);
  }};
}

GeodeticField3 level_density(const GeodeticField3& T, Numeric g, Numeric E, const SpeciesIsotope& spec) {
  GeodeticField3 out(T);
  out.data_name = "NLTE";

  stdr::transform(T.data | by_elem, out.data.elem_begin(), [g, E, spec](Numeric T) -> Numeric {
...
std::unordered_map<QuantumLevelIdentifier, AtmData> from_lte(const AtmField&        atm_field,
  ...
  for (const auto& [key, band] : abs_bands) {
    ...
    if (not out.contains(upp)) {
      ...
      out[upp] = std::visit([g, E, spec](auto& T) { return AtmData{::level_density(T, g, E, spec)}; }, t_data.data);
    }

    if (not out.contains(low)) {
      const Numeric g = line.gl;
      const Numeric E = line.e0;
      out[low] = std::visit([g, E, spec](auto& T) { return AtmData{::level_density(T, g, E, spec)}; }, t_data.data);
    }
  }
  ...
  if (normalizing_factor > 0.0) {
    ...
    for (auto& [key, data] : out) {
      ...
      if (auto lhs = std::get_if<GeodeticField3>(&data.data), rhs = std::get_if<GeodeticField3>(&x.data); lhs and rhs) {
        lhs->data /= rhs->data;
        continue;
      }

      if (auto lhs = std::get_if<Numeric>(&data.data), rhs = std::get_if<Numeric>(&x.data); lhs and rhs) {
        *lhs /= *rhs;
        continue;
      }
...
QuantumIdentifierNumericMap createBji(const QuantumIdentifierNumericMap& Bij, const AbsorptionBands& abs_bands) try {
...
QuantumIdentifierVectorMap createCij(const AbsorptionBands&                   abs_bands,
  ...
  for (const auto& [key, data] : abs_bands) {
    ...
    for (auto& atm_point : atm_path) {
      ...
      x.push_back(interp(coll.data, coll.grid<0>().lag<1, lagrange_interp::grid_identity>(atm_point.temperature)) *
                  numden);
    }
  }
...
Vector nlte_ratio_sum(const ArrayOfAtmPoint& atm_path, const ArrayOfQuantumLevelIdentifier& levels) try {
  return Vector(std::from_range, atm_path | stdv::transform([&levels](const AtmPoint& atm) {
                                   Numeric s{0.0};
...
Size band_level_mapUniquestIndex(const std::unordered_map<QuantumIdentifier, UppLow>& band_level_map) try {
  ...
  return stdr::min_element(unique_level_map, {}, [](auto& x) { return x.second; })->first;
}
ARTS_METHOD_ERROR_CATCH

std::unordered_map<QuantumIdentifier, UppLow> band_level_mapFromLevelKeys(
    const AbsorptionBands& abs_bands, const ArrayOfQuantumLevelIdentifier& level_keys) try {
  ...
  for (const auto& key : abs_bands | stdv::keys) {
    ...
    ARTS_USER_ERROR_IF(lower_find == stdr::end(level_keys), "Lower level {} not found in level keys", lower)
    ...
    ARTS_USER_ERROR_IF(upper_find == stdr::end(level_keys), "Upper level {} not found in level keys", upper)
  }
...
Numeric set_nlte(AtmPoint& atm_point, const ArrayOfQuantumLevelIdentifier& level_keys, const Vector& x) try {
