...
class band_matrix {
 ...
 public:
  ...
  template <std::integral A, std::integral B> [[nodiscard]] constexpr Numeric& operator[](const A& i, const B& j) {
  ...
  template <typename T> void zero(this T&& self) { std::forward<T>(self).AB = 0.0; }
