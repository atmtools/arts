...
template <typename T>
concept RationalFriend = not std::is_same_v<std::remove_cvref_t<T>, Rational> and std::constructible_from<Rational, T>;
...
struct Rational {
  ...
  template <RationalFriend T> constexpr Rational& operator=(T&& v) noexcept {
    return *this = Rational(std::forward<decltype(v)>(v));
  }

  [[nodiscard]] constexpr Index toIndex(Index n = 1) const noexcept { return (n * numer) / denom; }

  [[nodiscard]] constexpr Numeric toNumeric() const noexcept { return Numeric(numer) / Numeric(denom); }

  [[nodiscard]] constexpr int toInt(Index n = 1) const noexcept { return static_cast<int>(toIndex(n)); }
  ...
  template <RationalFriend T> constexpr Rational& operator+=(T&& a) noexcept {
  ...
  template <RationalFriend T> constexpr Rational operator+(T&& a) const noexcept {
    return *this + Rational(std::forward<T>(a));
  }
  ...
  template <RationalFriend T> friend constexpr Rational operator+(T&& a, const Rational& b) noexcept {
  ...
  constexpr Rational operator++(int) const noexcept { return {numer + denom, denom}; }
  ...
  friend constexpr Rational operator%(const Rational a, const Rational b) noexcept {
    return (a.denom == b.denom) ? Rational(a.numer % b.numer, a.denom)
                                : Rational((a.numer * b.denom) % (a.denom * b.numer), a.denom * b.denom);
  }

  template <RationalFriend T> friend constexpr Rational operator%(const Rational a, T&& b) noexcept {
  ...
  constexpr auto operator<=>(const Rational& b) const noexcept {
    ...
    return (denom == b.denom) ? (numer <=> b.numer) : [*this, b] {
      ...
      auto  res2 = r1 <=> r2;
      return res2 != std::strong_ordering::equal ? res2 : ((numer % denom) * b.denom) <=> ((b.numer % b.denom) * denom);
    }();
  };

  constexpr bool operator==(const Rational& b) const noexcept { return numer == b.numer and denom == b.denom; }

  template <RationalFriend T> constexpr auto operator<=>(T&& b) const noexcept {
    return *this <=> Rational(std::forward<T>(b));
  }

  template <RationalFriend T> constexpr bool operator==(T&& b) const noexcept {
    return *this == Rational(std::forward<T>(b));
  }

  template <RationalFriend T> friend constexpr auto operator<=>(T&& b, const Rational& a) noexcept {
    return Rational{std::forward<T>(b)} <=> a;
  }

  template <RationalFriend T> friend constexpr bool operator==(T&& b, const Rational& a) noexcept {
    return Rational{std::forward<T>(b)} == a;
  }
 ...
 */
constexpr const Rational& maxr(const Rational& a, const Rational& b) noexcept { return a < b ? b : a; }
 ...
 */
constexpr const Rational& minr(const Rational& a, const Rational& b) noexcept { return a < b ? a : b; }
 ...
 */
constexpr bool iseven(const Rational r) noexcept { return (r.numer % (2 * r.denom) == 0); }

/** Multiplication with numeric */
constexpr Numeric operator*(Rational y, Numeric x) noexcept { return static_cast<Numeric>(y) * x; }
constexpr Numeric operator*(Numeric x, Rational y) noexcept { return x * static_cast<Numeric>(y); }
...
inline Numeric sqrtr(const Rational r) { return std::sqrt(static_cast<Numeric>(r)); }

inline Numeric powr(const Rational base, Numeric exp) { return nonstd::pow(static_cast<Numeric>(base), exp); }

inline Numeric powr(Numeric base, const Rational exp) { return nonstd::pow(base, static_cast<Numeric>(exp)); }

inline Numeric powr(const Rational base, const Rational exp) { return powr(base, static_cast<Numeric>(exp)); }

template <> struct std::formatter<Rational> {
  format_tags tags;
  ...
  template <class FmtContext> FmtContext::iterator format(const Rational& v, FmtContext& ctx) const {
    if (v.denom == 1) return tags.format(ctx, v.numer);
    return tags.format(ctx, v.numer, "/"sv, v.denom);
  }
};

template <> struct xml_io_stream<Rational> {
  static constexpr std::string_view type_name = "Rational"sv;

  static void write(std::ostream& os, const Rational& x, bofstream* pbofs = nullptr, std::string_view name = ""sv);
