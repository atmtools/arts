...
Matrix band_matrix::mat(Index KL, Index KU, Index N) { return Matrix(N, 2 * KL + KU + 1); }

band_matrix::band_matrix(Index ku, Index kl, Index m, Index n)
    : KU(ku), KL(kl), M(m), N(n), AB(mat(KL, KU, N)), ipiv(N) {}

band_matrix::band_matrix(const Matrix& ab) : KU(0), KL(0), M(ab.nrows()), N(ab.ncols()), AB(0, 0), ipiv(N) {
  for (Index i = 0; i < M; ++i) {
    for (Index j = 0; j < N; ++j) {
      if (i == j) continue;
  ...
  for (Index j = 0; j < N; j++) {
    ...
    for (Index i = start_row(j); i < maxi; i++) { this->operator[](i, j) = ab[i, j]; }
  }
}
