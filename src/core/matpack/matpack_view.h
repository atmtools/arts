...
struct matpack_strided_access {
  ...
  constexpr matpack_strided_access(Index i0, Joker, Index d = 1) noexcept
      : offset(i0), stride(d) {}
  ...
  constexpr matpack_strided_access(Index max_size,
                                   const matpack_strided_access& r)
      ARTS_NOEXCEPT : offset(r.offset),
                      extent(r.extent),
                      stride(r.stride) {
    // Start must be >= 0:
  ...
  constexpr matpack_strided_access(const matpack_strided_access& p,
                                   const matpack_strided_access& n)
      ARTS_NOEXCEPT : offset(p.offset + n.offset * p.stride),
                      extent(n.extent),
...
constexpr stdx::strided_slice<Index, Index, Index> substride(
    Index max, const matpack_strided_access& x) noexcept {
...
[[nodiscard]] constexpr bool check_index_sizes(const auto& arr,
                                               Index r,
                                               T first,
                                               Ts... rest) {
  constexpr bool test = integral<T>;
  if constexpr (test) {
    if (static_cast<Index>(first) >= static_cast<Index>(arr.extent(r)))
      return false;
  }
...
struct subhelper {
  ...
  using mutable_type = std::conditional_t<
      ranged,
      matpack_view<T, N, constant, true>,
      std::
          conditional_t<N == 1, T&, matpack_view<T, N - 1, constant, strided>>>;

  using constant_type = std::conditional_t<
      ranged,
      matpack_view<T, N, true, true>,
      std::conditional_t<N == 1,
                         const T&,
...
template <typename T,
          ...
          bool strided,
          access_operator... access>
using mutable_access_type = typename access_retval::
    helper<T, N, constant, strided, access...>::mutable_type;
...
template <typename T,
          ...
          bool strided,
          access_operator... access>
using constant_access_type = typename access_retval::
    helper<T, N, constant, strided, access...>::constant_type;
...
template <typename T,
          ...
          bool strided,
          access_operator access>
using mutable_left_access_type = typename access_retval::
    subhelper<T, N, constant, strided, access>::mutable_type;
...
template <typename T,
          ...
          bool strided,
          access_operator access>
using constant_left_access_type = typename access_retval::
    subhelper<T, N, constant, strided, access>::constant_type;
...
std::array<Index, N> upview(const std::array<Index, M>& low) {
  ...
  for (Index i = 0; i < M; i++) sz[i] = low[i];
  return sz;
}

namespace detail {
template <access_operator first, access_operator... rest>
void access_str(std::string& out, first ind [[maybe_unused]], rest... inds) {
...
class matpack_view {
  ...
  template <access_operator... access>
  using mutable_access =
      mutable_access_type<T, N, constant, strided, access...>;

  //! Helper to return correct constant type
  template <access_operator... access>
  using constant_access =
      constant_access_type<T, N, constant, strided, access...>;

  //! Helper to return correct mutable type
  template <access_operator access>
  using mutable_left_access =
      mutable_access_type<T, N, constant, strided, access>;

  //! Helper to return correct constant type
  template <access_operator access>
  using constant_left_access =
      constant_access_type<T, N, constant, strided, access>;
 ...
 public:
  ...
  constexpr matpack_view(T* ptr, std::array<Index, N> sz)
      : matpack_view(exhaustive_view{ptr, sz}) {}

  //! Construct this from another compatible view
  constexpr matpack_view(const me_view& x) : view(x.view) {}

  //! Construct this from another compatible view
  constexpr matpack_view(const ce_view& x)
    requires(constant)
      : view(x.view) {}

  //! Construct this from another compatible view
  constexpr matpack_view(const ms_view& x)
    requires(strided)
      : view(x.view) {}

  //! Construct this from another compatible view
  constexpr matpack_view(const cs_view& x)
    requires(constant and strided)
      : view(x.view) {}

  //! Construct this from the same size data view
  constexpr matpack_view(const data_t& x) noexcept
    requires(strided or constant)
      : matpack_view(x.view) {}

  //! Construct this from a smaller view by upping the rank with padded 1-extents to the right
  template <Index M>
  explicit constexpr matpack_view(const matpack_data<T, M>& x)
    requires(N > M)
      : matpack_view(x.data_handle(), upview<N, M>(x.shape())) {}

  //! Construct this by viewing another view with padded 1-extents to the right
  template <Index M>
  explicit matpack_view(const matpack_view<T, M, constant, strided>& x)
    requires(N > M)
  {
    ...
    if constexpr (not strided) {
      ...
      view =
          view_type{x.unsafe_data_handle(),
                    map_t{upview<N, M>(x.shape()), upview<N, M>(x.strides())}};
    }
  }

  //! Mutable view of x
  explicit constexpr matpack_view(T& x)
    requires(not constant)
      : matpack_view(&x, constant_array<N, 1>()) {}

  //! Constant view of x
  explicit constexpr matpack_view(const T& x)
      : matpack_view(const_cast<T*>(&x), constant_array<N, 1>()) {}
  ...
  constexpr matpack_view& operator=(me_view&& x)
    requires(constant)
  = delete;
  constexpr matpack_view& operator=(ce_view&& x)
    requires(constant)
  = delete;
  constexpr matpack_view& operator=(ms_view&& x)
    requires(constant)
  = delete;
  constexpr matpack_view& operator=(cs_view&& x)
    requires(constant)
  = delete;
  constexpr matpack_view& operator=(data_t&& x)
    requires(constant)
  = delete;
  ...
  [[nodiscard]] constexpr auto inner_map() const {
    if constexpr (N == 1) {
      ...
      using map_t = typename matpack_view<T, N - 1, constant, strided>::
  ...
  [[nodiscard]] constexpr Index ncols() const
    requires(N >= 2)
  {
    return view.extent(N - 1);
  }
  ...
  [[nodiscard]] constexpr Index nrows() const
    requires(N >= 2)
  {
    return view.extent(N - 2);
  }

  //! Return the right-most extent bar 2 of the multidimensional object
  [[nodiscard]] constexpr Index npages() const
    requires(N >= 3)
  {
    return view.extent(N - 3);
  }

  //! Return the right-most extent bar 3 of the multidimensional object
  [[nodiscard]] constexpr Index nbooks() const
    requires(N >= 4)
  {
    return view.extent(N - 4);
  }

  //! Return the right-most extent bar 4 of the multidimensional object
  [[nodiscard]] constexpr Index nshelves() const
    requires(N >= 5)
  {
    return view.extent(N - 5);
  }

  //! Return the right-most extent bar 5 of the multidimensional object
  [[nodiscard]] constexpr Index nvitrines() const
    requires(N >= 6)
  {
    return view.extent(N - 6);
  }

  //! Return the right-most extent bar 6 of the multidimensional object
  [[nodiscard]] constexpr Index nlibraries() const
    requires(N >= 7)
  {
    return view.extent(N - 7);
  }
 ...
 private:
  ...
  constexpr auto submdspan_substride_extents(
      std::integer_sequence<Index, ints...>, access&&... ind) const
 ...
 public:
  ...
  using iterator = matpack_mditer<
      0,
      false,
      matpack_view,
      matpack_view<T, std::max<Index>(N - 1, 1), false, strided>>;
  using const_iterator = matpack_mditer<
      0,
      true,
      const matpack_view,
      const matpack_view<T, std::max<Index>(N - 1, 1), true, strided>>;

  //! Iterate over this object by left-most dimension --- return the first of these iterators
  [[nodiscard]] constexpr auto begin()
    requires(not constant)
  {
    if constexpr (not strided and N == 1)
      return data_handle();
    else
  ...
  using const_elem_iterator =
      matpack_elemwise_mditer<T, true, const matpack_view>;
    ...
    requires(not constant)
  {
    if constexpr (not strided)
      ...
      return elem_iterator{*this} + size();
  }

  //! Iterate over this object element-wise --- return the first of these iterators
  [[nodiscard]] constexpr auto elem_begin() const {
    if constexpr (not strided)
      return data_handle();
    else
      return const_elem_iterator{*this};
  }
  ...
  [[nodiscard]] constexpr auto elem_end() const {
    if constexpr (not strided)
      ...
      return const_elem_iterator{*this} + size();
  }
 ...
 private:
  ...
  constexpr std::pair<std::array<Index, N>, std::array<Index, N>> cmplx_sz()
      const {
    return {shape(), [s = strides()]() {
              auto out = s;
              std::transform(
                  out.begin(), out.end(), out.begin(), [](auto& str) {
                    return 2 * str;
                  });
              return out;
            }()};
  ...
  template <bool impl_constant>
  constexpr auto real_impl() const {
    return cmpl_impl<impl_constant, 0>();
  }

  //! Helper wrapper for imag
  template <bool impl_constant>
  constexpr auto imag_impl() const {
 ...
 public:
  ...
  [[nodiscard]] constexpr auto real()
    requires(complex_type<T> and not constant)
  {
  ...
  constexpr matpack_view(const std::vector<T>& x)
    requires(constant and N == 1)
      : matpack_view(const_cast<T*>(x.data()), {static_cast<Index>(x.size())}) {
    // The cast here should be OK since you are not able to change any values
    // later on as the type is constant
  }
  ...
  constexpr void swap(matpack_view& other) noexcept
    requires(not constant)
  {
...
using ExhaustiveConstVectorView =
    matpack::matpack_view<Numeric, 1, true, false>;

//! A constant continuous view of a matrix of Numeric
using ExhaustiveConstMatrixView =
    matpack::matpack_view<Numeric, 2, true, false>;

//! A constant continuous view of a tensor of Numeric of rank 3
using ExhaustiveConstTensor3View =
    matpack::matpack_view<Numeric, 3, true, false>;

//! A constant continuous view of a tensor of Numeric of rank 4
using ExhaustiveConstTensor4View =
    matpack::matpack_view<Numeric, 4, true, false>;

//! A constant continuous view of a tensor of Numeric of rank 5
using ExhaustiveConstTensor5View =
    matpack::matpack_view<Numeric, 5, true, false>;

//! A constant continuous view of a tensor of Numeric of rank 6
using ExhaustiveConstTensor6View =
    matpack::matpack_view<Numeric, 6, true, false>;

//! A constant continuous view of a tensor of Numeric of rank 7
using ExhaustiveConstTensor7View =
    matpack::matpack_view<Numeric, 7, true, false>;
...
using ExhaustiveComplexVectorView =
    matpack::matpack_view<Complex, 1, false, false>;

//! A mutable continuous view of a matrix of Complex
using ExhaustiveComplexMatrixView =
    matpack::matpack_view<Complex, 2, false, false>;

//! A mutable continuous view of a tensor of Complex of rank 3
using ExhaustiveComplexTensor3View =
    matpack::matpack_view<Complex, 3, false, false>;

//! A constant continuous view of a vector of Complex
using ExhaustiveConstComplexVectorView =
    matpack::matpack_view<Complex, 1, true, false>;

//! A constant continuous view of a matrix of Complex
using ExhaustiveConstComplexMatrixView =
    matpack::matpack_view<Complex, 2, true, false>;
