...
template <class Compare> struct xml_io_stream<matpack::grid_t<Compare>> {
  constexpr static std::string_view type_name = xml_io_stream_name_v<matpack::data_t<Numeric, 1>>;
  ...
  static void read(std::istream& is, matpack::grid_t<Compare>& x, bifstream* pbifs = nullptr) {
    Vector y;
...
template <typename T, typename... Grids> struct xml_io_stream_name<matpack::gridded_data_t<T, Grids...>> {
  static constexpr std::string_view name = "GriddedField"sv;
};

template <arts_xml_ioable T, arts_xml_ioable... Grids> struct xml_io_stream_gridded_field {
  constexpr static std::string_view type_name   = xml_io_stream_name_v<matpack::gridded_data_t<T, Grids...>>;
  ...
  static void parse(std::istream& is, matpack::gridded_data_t<T, Grids...>& gf, bifstream* pbifs, XMLTag& tag) {
    constexpr Size M = sizeof...(Grids);
  ...
  static void read(std::istream& is, matpack::gridded_data_t<T, Grids...>& gf, bifstream* pbifs = nullptr) try {
    XMLTag tag;
    tag.read_from_stream(is);
...
template <arts_xml_ioable T, arts_xml_ioable... Grids> struct xml_io_stream<matpack::gridded_data_t<T, Grids...>> {
  ...
  static void read(std::istream& is, matpack::gridded_data_t<T, Grids...>& gf, bifstream* pbifs = nullptr) {
    GF::read(is, gf, pbifs);
  }
};

#define GF_IO(GF) template <> void xml_io_stream<GF>::read(std::istream& is_xml, GF& gf, bifstream* pbifs);
