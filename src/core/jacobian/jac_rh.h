...
void make_rhfit(Jacobian::AtmTarget&, const AtmField&, const NumericUnaryOperator&, const bool);

template <> struct xml_io_stream_name<rhfwd> {
  constexpr static std::string_view name = "rhfwd";
};

template <> struct xml_io_stream_name<rhinv> {
  constexpr static std::string_view name = "rhinv";
};

template <> struct xml_io_stream_aggregate<rhfwd> {
  constexpr static bool value = true;
};

template <> struct xml_io_stream_aggregate<rhinv> {
  constexpr static bool value = true;
};
