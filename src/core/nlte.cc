...
void setCji(Vector& Cji,
            ...
            const ArrayOfArrayOfAbsorptionLines& abs_lines,
            const Numeric& T) {
  static constexpr Numeric c0 =
      -Constant::planck_constant / Constant::boltzmann_constant;
  ...
  for (auto& lines : abs_lines) {
    for (auto& band : lines) {
      for (Index k = 0; k < band.NumLines(); k++) {
        Cji[i] = Cij[i] * exp(constant * band.lines[k].F0) *
                 band.lines[k].gupp / band.lines[k].glow;
        i++;
      }
    }
  }
        ...
        for (auto& band : lines) {
          ...
          for (Index k = 0; k < band.NumLines(); k++) {
            ...
            const Quantum::Number::StateMatch lt(
                transition, band.lines[k].localquanta, band.quantumidentity);
    ...
    for (const AbsorptionLines& band : lines) {
      for (Index k = 0; k < band.NumLines(); k++) {
        for (Index iq = 0; iq < nq; iq++) {
          const Quantum::Number::StateMatch lt(
              nlte_qid[iq], band.lines[k].localquanta, band.quantumidentity);
