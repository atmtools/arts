...
void compute(PropmatVector& propmat_clearsky, const Vector& f_grid, const AtmPoint& atm_point) {
  ...
  if (lwc < LIQUID_AND_ICE_TREAT_AS_ZERO) { return; }

  constexpr Numeric m = 1.00e3;  // specific weight of the droplet,  fixed value:  1.00e3 kg/m3
  constexpr Numeric low_lim_den =
      -LIQUID_AND_ICE_TREAT_AS_ZERO;         // lower limit of suspended droplet particle density vector [kg/m3]
  constexpr Numeric high_lim_den = 5.00e-3;  // upper limit of suspended droplet particle density vector [kg/m3]
  ...
  ARTS_USER_ERROR_IF(t < 210 or t > 373, "Only valid for temperatures 210-373 K, not for your value of {} K", t)
  ...
  for (Size s = 0; s < f_grid.size(); ++s) {
    ...
    const Numeric Reepsilon =
        epsilon_s -
        pow2((two_pi * f_grid[s])) * (pow2(tau1) * delta1 / (1. + pow2(two_pi * f_grid[s] * tau1)) +
                                      pow2(tau2) * delta2 / (1. + pow2(two_pi * f_grid[s] * tau2)) +
                                      pow2(tau3) * delta3 / (1. + pow2(two_pi * f_grid[s] * tau3))) -
        pow2(two_pi * tau4) * delta4 / 2. *
            (f_grid[s] * (f0 + f_grid[s]) / (1. + pow2(two_pi * tau4 * (f0 + f_grid[s]))) -
             f_grid[s] * (f0 - f_grid[s]) / (1. + pow2(two_pi * tau4 * (f0 - f_grid[s])))) -
        pow2(two_pi * tau5) * delta5 / 2. *
            (f_grid[s] * (f1 + f_grid[s]) / (1. + pow2(two_pi * tau5 * (f1 + f_grid[s]))) -
             f_grid[s] * (f1 - f_grid[s]) / (1. + pow2(two_pi * tau5 * (f1 - f_grid[s]))));
    ...
    const Numeric Imepsilon =
        ...
        pi * f_grid[s] * tau4 * delta4 *
            (1. / (1. + pow2(two_pi * tau4 * (f0 + f_grid[s]))) + 1. / (1. + pow2(two_pi * tau4 * (f0 - f_grid[s])))) +
        pi * f_grid[s] * tau5 * delta5 *
            (1. / (1. + pow2(two_pi * tau5 * (f1 + f_grid[s]))) + 1. / (1. + pow2(two_pi * tau5 * (f1 - f_grid[s]))));
    ...
    const Numeric ImNw = 1.500 / m * (3.000 * Imepsilon / (pow2((Reepsilon + 2.000)) + pow2(Imepsilon)));
    ...
    propmat_clearsky[s].A() += lwc * 1.000e6 * dB_km_to_1_m * 0.1820 * (f_grid[s] * 1e-9) * ImNw;
  }
}
}  // namespace Absorption::PredefinedModel::ELL07
