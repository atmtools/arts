...
void oxygen(PropmatVector& propmat_clearsky, const Vector& f_grid, const AtmPoint& atm_point) {
  ...
  static constexpr std::array tre05{std::array{50.474214, 0.975 / 10, 9.651, 0.669, 0.0, 0.2566, 0.685},     // 37-
                                    std::array{50.987745, 2.529 / 10, 8.653, 0.717, 0.0, 0.2246, 0.680},     // 35-
                                    std::array{51.503360, 6.193 / 10, 7.709, 0.764, 0.0, 0.1947, 0.6729},    // 33-
                                    std::array{52.021429, 14.32 / 10, 6.819, 0.811, 0.0, 0.1667, 0.6640},    // 31-
                                    std::array{52.542418, 31.24 / 10, 5.983, 0.858, 0.0, 0.1388, 0.6526},    // 29-
                                    std::array{53.066934, 64.29 / 10, 5.201, 0.906, 0.0, 0.1349, 0.6206},    // 27-
                                    std::array{53.595775, 124.6 / 10, 4.474, 0.955, 0.0, 0.2227, 0.5085},    // 25-
                                    std::array{54.130025, 227.3 / 10, 3.800, 0.996, 0.0, 0.3170, 0.3750},    // 23-
                                    std::array{54.671180, 389.7 / 10, 3.182, 1.037, 0.0, 0.3558, 0.2654},    // 21-
                                    std::array{55.221384, 627.1 / 10, 2.618, 1.089, 0.0, 0.2560, 0.2952},    // 19-
                                    std::array{55.783815, 945.3 / 10, 2.109, 1.134, 0.0, -0.1172, 0.6135},   // 17-
                                    std::array{56.264774, 543.4 / 10, 0.014, 1.703, 0.0, 0.3525, -0.0978},   //  1+
                                    std::array{56.363399, 1331.8 / 10, 1.654, 1.189, 0.0, -0.2378, 0.6547},  // 15-
                                    std::array{56.968211, 1746.6 / 10, 1.255, 1.223, 0.0, -0.3545, 0.6451},  // 13-
                                    std::array{57.612486, 2120.1 / 10, 0.910, 1.262, 0.0, -0.5416, 0.6056},  // 11-
                                    std::array{58.323877, 2363.7 / 10, 0.621, 1.295, 0.0, -0.1932, 0.0436},  //  9-
                                    std::array{58.446588, 1442.1 / 10, 0.083, 1.491, 0.0, 0.6768, -0.1273},  //  3+
                                    std::array{59.164204, 2379.9 / 10, 0.387, 1.353, 0.0, -0.6561, 0.2309},  //  7-
                                    std::array{59.590983, 2090.7 / 10, 0.207, 1.408, 0.0, 0.6957, -0.0776},  //  5+
                                    std::array{60.306056, 2103.4 / 10, 0.207, 1.415, 0.0, -0.6395, 0.0699},  //  5-
                                    std::array{60.434778, 2438.0 / 10, 0.386, 1.339, 0.0, 0.6342, -0.2825},  //  7+
                                    std::array{61.150562, 2479.5 / 10, 0.621, 1.292, 0.0, 0.1014, -0.0584},  //  9+
                                    std::array{61.800158, 2275.9 / 10, 0.910, 1.263, 0.0, 0.5014, -0.6619},  // 11+
                                    std::array{62.411220, 1915.4 / 10, 1.255, 1.217, 0.0, 0.3029, -0.6759},  // 13+
                                    std::array{62.486253, 1503.0 / 10, 0.083, 1.513, 0.0, -0.4499, 0.0844},  //  3-
                                    std::array{62.997984, 1490.2 / 10, 1.654, 1.174, 0.0, 0.1856, -0.6675},  // 15+
                                    std::array{63.568526, 1078.0 / 10, 2.108, 1.134, 0.0, 0.0658, -0.6139},  // 17+
                                    std::array{64.127775, 728.7 / 10, 2.617, 1.088, 0.0, -0.3036, -0.2895},  // 19+
                                    std::array{64.678910, 461.3 / 10, 3.181, 1.038, 0.0, -0.3968, -0.2590},  // 21+
                                    std::array{65.224078, 274.0 / 10, 3.800, 0.996, 0.0, -0.3528, -0.3680},  // 23+
                                    std::array{65.764779, 153.0 / 10, 4.473, 0.955, 0.0, -0.2548, -0.5002},  // 25+
                                    std::array{66.302096, 80.40 / 10, 5.200, 0.906, 0.0, -0.1660, -0.6091},  // 27+
                                    std::array{66.836834, 39.80 / 10, 5.982, 0.858, 0.0, -0.1680, -0.6393},  // 29+
                                    std::array{67.369601, 18.56 / 10, 6.818, 0.811, 0.0, -0.1956, -0.6475},  // 31+
                                    std::array{67.900868, 8.172 / 10, 7.708, 0.764, 0.0, -0.2216, -0.6545},  // 33+
                                    std::array{68.431006, 3.397 / 10, 8.652, 0.717, 0.0, -0.2492, -0.660},   // 35+
                                    std::array{68.960312, 1.334 / 10, 9.650, 0.669, 0.0, -0.2773, -0.665},   // 37+
                                    std::array{118.750334, 940.3 / 10, 0.010, 1.664, 0.0, -0.0439, 0.0079},  //  1-
  ...
  constexpr Numeric S0 = 6.140e-5;  // line strength                        [ppm]
  constexpr Numeric G0 = 0.560e-3;  // line width                           [GHz/hPa]
  ...
  ARTS_USER_ERROR_IF(oxygen_vmr < VMRCalcLimit,
                     ...
                     oxygen_vmr,
                     VMRCalcLimit)
  constexpr Numeric dB_km_to_1_m = (1.00000e-3 / (10.0 * Constant::log10_euler));
  ...
  for (Size s = 0; s < f_grid.size(); ++s) {
    ...
    const Numeric Nppc = strength_cont * ff * gam_cont / (pow(ff, (Numeric)2.) + pow(gam_cont, (Numeric)2.));

    // Loop over TRE05 O2 spectral lines:
    Numeric Nppl = 0.0;
    for (const auto& l : tre05) {
      // line strength [ppm]   S=A(1,I)*P*V**3*EXP(A(2,I)*(1.-V))*1.E-6
      const Numeric strength = 1.000e-6 * pda_dummy * l[1] / l[0] * pow(theta, (Numeric)3.) * exp(l[2] * (1.0 - theta));
      // line broadening parameter [GHz]
      const Numeric gam = (l[3] * 0.001 * ((pda * pow(theta, ((Numeric)0.8 - l[4]))) + (1.10 * pwv * theta)));
      // line mixing parameter [1]
      //      if (l < 11) CD = 1.1000;
      const Numeric delta = ((l[5] + l[6] * theta) * (pda + pwv) * pow(theta, (Numeric)0.8) * (Numeric)0.001);
    ...
    // cross section: pxsec = absorption / var
    // the vmr of O2 will be multiplied at the stage of absorption calculation:
    propmat_clearsky[s].A() += oxygen_vmr * dB_km_to_1_m * 0.1820 * ff * (Nppl + Nppc) / VMRISO;
  }
}
}  // namespace Absorption::PredefinedModel::TRE05
