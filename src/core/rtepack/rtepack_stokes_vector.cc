...
std::function<stokvec(const stokvec, const Numeric)> unit_converter(const SpectralRadianceUnitType type,
  ...
  switch (type) {
    ...
    case RJBT: return [](const stokvec v, const Numeric f) { return invrayjean(1.0, f) * v; };
    case PlanckBT:
    ...
    case SpectralRadianceUnitType::W_m2_m_sr:
      return [n2 = n * n](const stokvec v, const Numeric f) { return n2 * v * (f * (f / Constant::c)); };
    case SpectralRadianceUnitType::W_m2_m1_sr:
      return [n2 = n * n](const stokvec v, const Numeric) { return n2 * v * Constant::c; };
  }
...
std::function<stokvec(const stokvec, const stokvec, const Numeric)> dunit_converter(const SpectralRadianceUnitType type,
  ...
  switch (type) {
    ...
    case RJBT: return [](const stokvec dv, const stokvec, const Numeric f) { return invrayjean(1.0, f) * dv; };
    case PlanckBT:
      return [](const stokvec dv, const stokvec v, const Numeric f) {
        return stokvec{dinvplanckdI(v.I(), f) * dv.I(),
                       (dinvplanckdI(0.5 * (v.I() + v.Q()), f) - dinvplanckdI(0.5 * (v.I() - v.Q()), f)) * dv.Q(),
                       (dinvplanckdI(0.5 * (v.I() + v.U()), f) - dinvplanckdI(0.5 * (v.I() - v.U()), f)) * dv.U(),
                       (dinvplanckdI(0.5 * (v.I() + v.V()), f) - dinvplanckdI(0.5 * (v.I() - v.V()), f)) * dv.V()};
      };
    case SpectralRadianceUnitType::W_m2_m_sr:
      return
          [n2 = n * n](const stokvec dv, const stokvec, const Numeric f) { return n2 * dv * (f * (f / Constant::c)); };
    case SpectralRadianceUnitType::W_m2_m1_sr:
      return [n2 = n * n](const stokvec dv, const stokvec, const Numeric) { return n2 * dv * Constant::c; };
  }
