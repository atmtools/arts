...
//! modification.
template <typename... T> struct Supergeneric {
  std::variant<std::shared_ptr<T>...> value;

  operator const std::variant<std::shared_ptr<T>...>&() const { return value; }
};

template <typename... T> bool has_selected_value(const std::variant<std::shared_ptr<T>...>& x) {
  return std::visit([](const auto& ptr) { return static_cast<bool>(ptr); }, x);
}

template <typename... T> bool has_selected_value(const Supergeneric<T...>& x) { return has_selected_value(x.value); }

template <typename... T> std::string type(const Supergeneric<T...>* const x) { return type(x ? &x->value : nullptr); }
...
template <typename... T> std::variant<std::shared_ptr<T>...>& select_gout(std::variant<std::shared_ptr<T>...>* const x,
                                                                          Workspace&,
                                                                          const char* const name) {
  if (not x or not has_selected_value(*x)) { throw std::runtime_error(std::format("Unknown output: \"{}\"", name)); }

  return *x;
}

template <typename... T>
std::variant<std::shared_ptr<T>...>& select_gout(Supergeneric<T...>* const x, Workspace& ws, const char* const name) {
...
template <typename... T>
const std::variant<std::shared_ptr<T>...>& select_gin(const std::variant<std::shared_ptr<T>...>* const x,
                                                      const char* const                                name) {
  if (not x or not has_selected_value(*x)) { throw std::runtime_error(std::format("Unknown input: \"{}\"", name)); }

  return *x;
}

template <typename... T>
const std::variant<std::shared_ptr<T>...>& select_gin(const Supergeneric<T...>* const x, const char* const name) {
...
//! shared_ptr caster.
template <typename... T> struct type_caster<Python::Supergeneric<T...>> {
  using Value   = Python::Supergeneric<T...>;
  using Variant = std::variant<std::shared_ptr<T>...>;

  static constexpr auto Name = union_name(make_caster<std::shared_ptr<T>>::Name...);
 ...
 private:
  template <typename Alt> bool try_alt(handle src, uint8_t flags, cleanup_list* cleanup) noexcept {
    ...
    src.inc_ref();
    value.value.template emplace<std::shared_ptr<Alt>>(std::shared_ptr<Alt>(ptr, [src = src.ptr()](void*) noexcept {
      if (is_alive()) {
