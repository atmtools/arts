...
void internalPWR2021(py::module_& m) {
  m.def(
      "get_h2o_pwr2021",
      [](const Vector& f, Numeric p, Numeric t, Numeric x) -> Vector {
        PropagationMatrix pm(f.nelem());
        Absorption::PredefinedModel::PWR2021::compute_h2o(pm, f, p, t, x);
        return std::move(pm.Data()).flatten();
      },
      py::arg("f_grid"),
      py::arg("rtp_pressure"),
...
Parameters:
  ...
  m.def(
      ...
      [](const Vector& f, Numeric p, Numeric t, Numeric x, Numeric h2o) -> Vector {
        PropagationMatrix pm(f.nelem());
        Absorption::PredefinedModel::PWR2021::compute_o2(
            pm, f, p, t, x, h2o);
        return std::move(pm.Data()).flatten();
      },
      py::arg("f_grid"),
...
Parameters:
  ...
  m.def(
      ...
      [](const Vector& f, Numeric p, Numeric t, Numeric x, Numeric h2o) -> Vector {
        PropagationMatrix pm(f.nelem());
        Absorption::PredefinedModel::PWR2021::compute_n2(pm, f, p, t, x, h2o);
        return std::move(pm.Data()).flatten();
      },
      py::arg("f_grid"),
      py::arg("rtp_pressure"),
