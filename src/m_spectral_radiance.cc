...
void spectral_radStepByStepEmission(StokvecVector&             spectral_rad,
  ...
  rte_emission(spectral_rad, spectral_rad_jac_path, spectral_tramat, spectral_rad_srcvec);
}
ARTS_METHOD_ERROR_CATCH

void spectral_radCumulativeTransmission(StokvecVector&             spectral_rad,
                                        StokvecTensor3&            spectral_rad_jac_path,
...
single_propmat_jac_path: (N, nq) = {:B,}
  ...
  const Index temperature_derivative_position = jac_targets.target_position(AtmKey::t);
...
void single_tramat_cumulative_pathFromPath(MuelmatVector&       single_tramat_cumulative_path,
...
void single_rad_srcvec_jacFromPropmat(StokvecVector&         single_rad_srcvec_jac,
...
void single_radStepByStepEmission(Stokvec&              single_rad,
...
single_tramat_cumulative_path (N)        = {:B,}
  ...
  for (Size i = N - 2; i < N; i--) {
    ...
    for (Size iq = 0; iq < nq; iq++) {
      dI[i, iq]     += Pi[i] * (dTs[i, 0, iq] * I + 0.5 * (dJs[i, iq] - Ts[i + 1] * dJs[i, iq]));
      dI[i + 1, iq] += Pi[i] * (dTs[i + 1, 1, iq] * I + 0.5 * (dJs[i + 1, iq] - Ts[i + 1] * dJs[i + 1, iq]));
    }
...
void single_rad_jacFromBackground(StokvecVector& single_rad_jac, const Muelmat& background_transmittance) try {
  ARTS_TIME_REPORT
  for (auto& dI : single_rad_jac) dI = background_transmittance * dI;
}
ARTS_METHOD_ERROR_CATCH

void single_rad_jacAddPathPropagation(StokvecVector&                     single_rad_jac,
...
void single_radFromPropagation(Stokvec&                           single_rad,
  ...
  single_tramat_pathFromPath(single_tramat_path,
                             ...
                             jac_targets,
                             hse_derivative);
  single_tramat_cumulative_pathFromPath(single_tramat_cumulative_path, single_tramat_path);
  const auto& transmission_single_matrix_background = single_tramat_cumulative_path.back();
  ...
  single_radStepByStepEmission(single_rad,
                               ...
                               single_rad_srcvec_jac_path);
  single_rad_jacFromBackground(single_rad_jac, transmission_single_matrix_background);
  single_rad_jacAddPathPropagation(single_rad_jac, single_rad_jac_path, jac_targets, atm_field, ray_path);
}
ARTS_METHOD_ERROR_CATCH
}  // namespace

void spectral_radSetToBackground(StokvecVector&       spectral_rad,
...
void spectral_radSinglePathEmissionFrequencyLoop(StokvecVector&                     spectral_rad,
                                                 StokvecMatrix&                     spectral_rad_jac,
                                                 const JacobianTargets&             jac_targets,
                                                 const ArrayOfPropagationPathPoint& ray_path,
...
#pragma omp parallel for firstprivate(single_freq_path,                  \
                                          single_rad_jac,                \
                                          single_rad_srcvec_jac,         \
                                          single_rad_srcvec_jac_path,    \
                                          single_rad_jac_path,           \
  ...
  for (Size f = 0; f < nf; f++) {
    try {
      ...
      for (Size i = 0; i < N; i++) {
        ...
        single_nlte_srcvec_jac_path[i] = spectral_nlte_srcvec_jac_path[i][joker, f];
      }
...
#pragma omp critical
  ...
  ARTS_USER_ERROR_IF(not error.empty(), "Error in spectral_radSinglePathEmissionFrequencyLoop: {:s}", error);
}
ARTS_METHOD_ERROR_CATCH

void single_radClearskyEmissionPropagation(const Workspace&             ws,
                                           Stokvec&                     single_rad,
                                           StokvecVector&               single_rad_jac,
  ...
  ARTS_USER_ERROR_IF(N <= 0 or max_stepsize <= 0.0 or max_tau <= 0.0 or cutoff_tau <= 0.0,
  ...
  ray_path.reserve(N);
  ray_path.resize(1);
  ray_path[0] = path::init_with_lostype(obs_pos, obs_los, atm_field, surf_field, true);

  if (ray_path.back().los_type == PathPositionType::space) {
    single_rad_space_agendaExecute(
        ws, single_rad, single_rad_jac, frequency_, jac_targets, ray_path.back(), single_rad_space_agenda);
    return;
  }
  ...
  while (true) {
    ...
    if (A == 0.0) {
      ...
      ray_point_back_propagation_agendaExecute(ws,
                                               tmp_point,
                                               ray_path,
                                               single_dispersion,
    ...
    if (ray_path.back().los_type == PathPositionType::space) {
      single_rad_space_agendaExecute(
          ws, single_rad, single_rad_jac, frequency, jac_targets, ray_path.back(), single_rad_space_agenda);
      break;
    }
...
void spectral_radClearskyEmissionFrequencyDependentPropagation(const Workspace&                    ws,
...
#pragma omp critical
      if (error.empty()) { error = std::format("Fail at index {}:\n{}", f, e.what()); }
    }
  }
...
void freq_gridFromSingleFrequency(AscendingGrid& freq_grid, const Numeric& frequency) try {
  ARTS_TIME_REPORT
