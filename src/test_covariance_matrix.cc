...
RetrievalData setup_retrieval_1D() {
  ...
  for (Index i = 0; i < n_rqs; i++) {
    ...
    rqs.emplace_back(JacobianTarget(), "subtag", "subsubtag", "mode", 0.0, gvs);
  }
...
void test_workspace_methods() {
  ...
  RetrievalQuantity rq_1(JacobianTarget(), "st", "sst", "m", 0.1, grids_1);
  RetrievalQuantity rq_2(JacobianTarget(), "st", "sst", "m", 0.1, grids_2);
