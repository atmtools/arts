...
void test_limb_finder(Size n) {
  ...
  while (n-- != 0) {
    ...
    const Numeric               za  = path::geometric_tangent_zenith(pos, surf_field.ellipsoid, tangent_altitude, az);
    const Vector2               los = path::mirror({za, az});
    ArrayOfPropagationPathPoint x{path::init(pos, los, atm_field, surf_field)};
    path::set_geometric_extremes(x, atm_field, surf_field);
    path::fill_geometric_stepwise(x, surf_field, 10e3);
    path::keep_only_atm(x);
    const Numeric computed_tangent_altitude = path::find_geometric_limb(x, surf_field).pos[0];
    ARTS_USER_ERROR_IF(nonstd::abs(tangent_altitude - computed_tangent_altitude) > 1.0,
...
void test_geometric_fill(Size n) {
  ...
  const auto test = [&](const ArrayOfPropagationPathPoint& x, const Numeric start_alt) {
    for (auto& p : x) {
      ...
      const bool at_alt   = std::any_of(alts.begin(), alts.end(), [&](Numeric a) { return a == p.pos[0]; });
      const bool at_lat   = std::any_of(lats.begin(), lats.end(), [&](Numeric a) { return a == p.pos[1]; });
      const bool at_lon   = std::any_of(lons.begin(), lons.end(), [&](Numeric a) { return a == p.pos[2]; });
      const bool at_start = p.pos[0] == start_alt;
      const bool any_nan  = nonstd::isnan(p.pos[0]) or nonstd::isnan(p.pos[1]) or nonstd::isnan(p.pos[2]) or
                            nonstd::isnan(p.los[0]) or nonstd::isnan(p.los[1]);
