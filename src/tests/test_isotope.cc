...
int main() {
  ...
  for (std::size_t i = 0; i < Species::Isotopologues.size(); i++) {
    ...
    if (not good_enum(x.spec)) { std::cout << "Position: " << i << " has bad values\n"; }
  }
  ...
  for (size_t i = 0; i < enumsize::SpeciesEnumSize; i++) {
    ...
    if (not good_enum(c) or c not_eq a) { std::cout << i << ' ' << a << ' ' << b << ' ' << c << '\n'; }
  }
  ...
  for (std::size_t i = 0; i < Species::Isotopologues.size() - 1; i++) {
    ...
    if (std::size_t(a.spec) > std::size_t(b.spec)) { std::cout << a.FullName() << ' ' << b.FullName() << '\n'; }
  }
  ...
  for (std::size_t i = 0; i < Species::Isotopologues.size() - 1; i++) {
    ...
    auto& b = Species::Isotopologues[i + 1];
    if (a.spec == b.spec) {
      if (a.isotname.compare(b.isotname) >= 0) { std::cout << a.FullName() << ' ' << b.FullName() << '\n'; }
    }
  }
  ...
  for (Index i = 0; i < iso_rat.maxsize; i++) {
    if (not Species::Isotopologues[i].is_predefined() and not Species::Isotopologues[i].is_joker() and
