...
auto linear_lag_runtime_view(const ConstVectorView& xs,
...
auto linear_lag_compile_view_1lin(const ConstVectorView& xs,
                                  const ConstVectorView& xi) {
  return my_interp::lagrange_interpolation_list<LinearFixed1>(xs, xi);
}
...
auto linear_lag_compile_view_5lin(const ConstVectorView& xs,
                                  const ConstVectorView& xi) {
  return my_interp::lagrange_interpolation_list<LinearFixed5>(xs, xi);
}
...
std::vector<Timing> test_linear_startup_cost(Index n) {
  ...
  out.emplace_back("gridpos")([&]() {
    ArrayOfGridPos gp(xs.size());
    gridpos(gp, xi, xs);
    X = std::transform_reduce(
        gp.begin(), gp.end(), Numeric{}, std::plus<>(), [](auto& g) {
          return g.fd[0];
        });
  });
  xvec.push_back(X);
  ...
  out.emplace_back("compile-order-1")([&]() {
    const auto lag =
        my_interp::lagrange_interpolation_list<LinearFixed1>(xs, xi);
    X = std::transform_reduce(
        lag.begin(), lag.end(), Numeric{}, std::plus<>(), [](auto& g) {
          return g.lx[0];
        });
  });
  xvec.push_back(X);
...
std::vector<Timing> test_tensor5_reinterp(Index n) {
  ...
  std::transform(x.begin(), x.end(), ys.elem_begin(), [](auto X) {
    return std::sin(std::get<0>(X)) * std::sin(std::get<1>(X)) *
           std::sin(std::get<2>(X)) * std::sin(std::get<3>(X)) *
           std::sin(std::get<4>(X));
  });
...
int main(int argc, char** c) {
  ...
  std::cout << n << " interpolation-performance-tests\n\n";
  for (Index i = 0; i < n; i++) {
    std::cout << N[0] << " test_2x_reinterp_vector\n"
              << test_2x_reinterp_vector(N[0]) << '\n';
    std::cout << N[1] << " test_linear_startup_cost\n"
              << test_linear_startup_cost(N[1]) << '\n';
    std::cout << N[2] << " test_linear_interpweights_cost\n"
              << test_linear_interpweights_cost(N[2]) << '\n';
    std::cout << N[3] << " test_tensor5_reinterp\n"
