 ...
 */
Matrix evaluate_spherical_harmonic(Index l,
                                   Index m,
  ...
  double norm = sqrt((2 * static_cast<Numeric>(l) + 1.0) / (4.0 * M_PI));
 ...
 * and std. dev.
 */
Vector random_vector(Index size, Numeric mean = 0.0, Numeric sigma = 3.0) {
 ...
 */
ComplexVector random_complex_vector(Index size,
                                    Numeric mean = 3.0,
                                    Numeric sigma = 3.0) {
  std::random_device rd{};
  std::mt19937 gen{rd()};
 ...
 */
ComplexVector random_spectral_coeffs(Index l_max, Index m_max) {
 ...
 */
template <matpack::any_matpack_type Tensor, typename Fun>
 ...
 */
template <Index axis, matpack::any_matpack_type Tensor>
 ...
 * @param t The tensor to fill.
 */
template <matpack::any_matpack_type Tensor>
