...
bool test_extinction_matrix_tro() {
  using ExtinctionMatrix =
      scattering::ExtinctionMatrixData<Numeric, scattering::Format::TRO, scattering::Representation::Gridded>;
  ...
  auto weights = calc_regrid_weights(t_grid, f_grid, nullptr, nullptr, nullptr, nullptr, grids);

  auto    extinction_matrix_interp = extinction_matrix.regrid(grids, weights);
  Numeric err                      = std::abs(extinction_matrix[0, 0, 0] - extinction_matrix_interp[0, 0, 0]);
  if (err > 1e-10) { return false; }

  fill_along_axis<0>(*t_grid);
  fill_along_axis<0>(*f_grid);

  fill_along_axis<0>(reinterpret_cast<matpack::data_t<Numeric, 3>&>(extinction_matrix));
  (*t_grid_new)[0]         = 1.2345;
  (*f_grid_new)[0]         = 1.2346;
  ...
  fill_along_axis<1>(reinterpret_cast<matpack::data_t<Numeric, 3>&>(extinction_matrix));
  extinction_matrix_interp = extinction_matrix.regrid(grids, weights);
...
bool test_extinction_matrix_aro() {
  using ExtinctionMatrix =
      scattering::ExtinctionMatrixData<Numeric, scattering::Format::ARO, scattering::Representation::Gridded>;
  ...
  scattering::ScatteringDataGrids grids(t_grid_new, f_grid_new, za_inc_grid_new, nullptr, nullptr);
  auto weights = calc_regrid_weights(t_grid, f_grid, nullptr, za_inc_grid, nullptr, nullptr, grids);

  auto    extinction_matrix_interp = extinction_matrix.regrid(grids, weights);
  Numeric err                      = std::abs(extinction_matrix[0, 0, 0, 0] - extinction_matrix_interp[0, 0, 0, 0]);
  if (err > 1e-10) { return false; }
  ...
  fill_along_axis<0>(reinterpret_cast<matpack::data_t<Numeric, 4>&>(extinction_matrix));
  (*t_grid_new)[0]         = 1.2345;
  (*f_grid_new)[0]         = 1.2346;
  (*za_inc_grid_new)[0]    = 1.2347;
  grids                    = scattering::ScatteringDataGrids{t_grid_new, f_grid_new, za_inc_grid_new, nullptr, nullptr};
  weights                  = calc_regrid_weights(t_grid, f_grid, nullptr, za_inc_grid, nullptr, nullptr, grids);
  extinction_matrix_interp = extinction_matrix.regrid(grids, weights);
  err                      = std::abs(extinction_matrix_interp[0, 0, 0, 0] - 1.2345);
  if (err > 1e-10) { return false; }

  fill_along_axis<1>(reinterpret_cast<matpack::data_t<Numeric, 4>&>(extinction_matrix));
  extinction_matrix_interp = extinction_matrix.regrid(grids, weights);
  err                      = std::abs(extinction_matrix_interp[0, 0, 0, 0] - 1.2346);
  if (err > 1e-10) { return false; }

  fill_along_axis<2>(reinterpret_cast<matpack::data_t<Numeric, 4>&>(extinction_matrix));
  extinction_matrix_interp = extinction_matrix.regrid(grids, weights);
