...
using PhaseMatrixTROSpectral = PhaseMatrixData<Numeric, Format::TRO, Representation::Gridded>;
...
bool test_phase_matrix_tro() {
  ...
  for (Index i_t = 0; i_t < phase_matrix_spectral.extent(0); ++i_t) {
    for (Index i_f = 0; i_f < phase_matrix_spectral.extent(1); ++i_f) {
      ...
      for (Index i_s = 0; i_s < phase_matrix_spectral.extent(3); ++i_s) {
        Numeric err = max_error<ComplexVector>(coeffs_ref,
                                               static_cast<ComplexVector>(phase_matrix_spectral[i_t, i_f, joker, i_s]));
  ...
  auto phase_matrix_liquid_sht       = make_phase_matrix_liquid_sphere(t_grid, f_grid, za_scat_grid);
  auto phase_matrix_liquid_spectral  = phase_matrix_liquid_sht.to_spectral(sht);
  auto backscatter_matrix_2          = phase_matrix_liquid_spectral.extract_backscatter_matrix();
  err                                = max_error<Tensor3>(backscatter_matrix, backscatter_matrix_2);
  Tensor3 delta                      = backscatter_matrix;
  delta                             -= backscatter_matrix_2;
  if (err > 1e-15) return false;

  auto forwardscatter_matrix_2 = phase_matrix_liquid_spectral.extract_forwardscatter_matrix();
...
bool test_phase_matrix_copy_const_tro() {
  ...
  PhaseMatrixData<Numeric, Format::TRO, Representation::Spectral> phase_matrix_spectral_2 = phase_matrix_gridded_2;
  err = max_error(phase_matrix_spectral.extract_stokes_coeffs(), phase_matrix_spectral_2);
  if (err > 1e-6) { return false; }
