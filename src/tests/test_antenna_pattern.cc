...
namespace {
void assert_stokvec(Stokvec actual, Stokvec expected, Numeric tol, std::string_view name) {
  for (Size i = 0; i < 4; ++i) {
    if (std::abs(actual[i] - expected[i]) > tol)
      throw std::runtime_error(std::format("{} mismatch at {}:\nactual {} expected {}", name, i, actual, expected));
  }
}
...
Stokvec sum_column_weights(const sensor::Obsel& obsel, Size ifreq) try {
  ...
  throw std::runtime_error(std::format("sum_column_weights failed for ifreq {}:\n{}", ifreq, e.what()));
}
...
Numeric gaussian_airy_expected_gain(Numeric zenith_deg, Numeric frequency, Numeric aperture_diameter) try {
  ...
  const Numeric     hwhm_deg = Conversion::rad2deg(gaussian_airy_hwhm_factor * wavelength / aperture_diameter);
  const Numeric     ratio    = zenith_deg / hwhm_deg;

  return std::exp(-std::numbers::ln2 * ratio * ratio);
} catch (const std::exception& e) {
  throw std::runtime_error(std::format("gaussian_airy_expected_gain failed for zenith {} frequency {} aperture {}:\n{}",
                                       zenith_deg,
                                       frequency,
                                       aperture_diameter,
                                       e.what()));
...
void test_gaussian_airy_is_frequency_dependent() try {
  ...
  const sensor::GaussianAiryAntenna ant{ZenGrid{{0.0, 0.2}}, 1.0, 1, peak_weight};
  const sensor::BoxChannel          channel{AscendingGrid{100.0e9, 200.0e9}};

  const auto obsel = ant(channel, {600e3, 10.0, 20.0}, {45.0, 30.0}, {Body::Earth::a, Body::Earth::b});

  const auto low_weight  = obsel.weight_matrix()[1, 0];
  const auto high_weight = obsel.weight_matrix()[1, 1];
  if (low_weight[0] <= high_weight[0])
    throw std::runtime_error("Higher frequency Gaussian Airy sample must be narrower");
} catch (const std::exception& e) {
  throw std::runtime_error(std::format("test_gaussian_airy_is_frequency_dependent failed:\n{}", e.what()));
}
...
void test_degenerate_azimuth_ring_is_collapsed() try {
  ...
  const auto                    gaussian_obsel =
      gaussian(sensor::DiracChannel{100.0e9}, {600e3, 10.0, 20.0}, {45.0, 30.0}, {Body::Earth::a, Body::Earth::b});

  if (gaussian_obsel.poslos_grid().size() != 4) {
    throw std::runtime_error(std::format("Expected zero-zenith azimuth collapse to leave 4 LOS samples, got {}",
  ...
  const auto                        airy_obsel =
      airy(sensor::DiracChannel{100.0e9}, {600e3, 10.0, 20.0}, {45.0, 30.0}, {Body::Earth::a, Body::Earth::b});
  ...
  if (airy_obsel.poslos_grid().size() != 4) {
    throw std::runtime_error(
        std::format("Expected Gaussian Airy zero-zenith azimuth collapse to leave 4 LOS samples, got {}",
  ...
  throw std::runtime_error(std::format("test_degenerate_azimuth_ring_is_collapsed failed:\n{}", e.what()));
}

void test_gaussian_does_not_double_count_degenerate_azimuth() try {
  const sensor::GaussianAntenna gaussian{ZenGrid{{0.0, 1.0}}, 1.0};
  const auto                    obsel =
      gaussian(sensor::DiracChannel{100.0e9}, {600e3, 10.0, 20.0}, {45.0, 0.0}, {Body::Earth::a, Body::Earth::b});

  if (obsel.poslos_grid().size() != 3) {
    throw std::runtime_error(
        std::format("Expected degenerate azimuth collapse to leave 3 LOS samples, got {}", obsel.poslos_grid().size()));
  }

  const Numeric wing = 0.5 * std::exp(-0.5) / (1 + std::exp(-0.5)) / (1 - gaussian.data[0, 0]);
  ...
  assert_stokvec(obsel.weight_matrix()[1, 0], {wing, 0.0, 0.0, 0.0}, 1e-12, "gaussian wing at azimuth 0");
  assert_stokvec(obsel.weight_matrix()[2, 0], {wing, 0.0, 0.0, 0.0}, 1e-12, "gaussian wing at azimuth 180");
} catch (const std::exception& e) {
  throw std::runtime_error(std::format("test_gaussian_does_not_double_count_degenerate_azimuth failed:\n{}", e.what()));
}
...
void test_gaussian_airy_rejects_nonpositive_frequencies() try {
  ...
  bool threw = false;
  try {
    static_cast<void>(ant(sensor::DiracChannel{}, {600e3, 10.0, 20.0}, {0.0, 0.0}, {Body::Earth::a, Body::Earth::b}));
  } catch (const std::runtime_error&) { threw = true; }

  if (not threw) throw std::runtime_error("Gaussian Airy antenna must reject nonpositive channel frequencies");
} catch (const std::exception& e) {
  throw std::runtime_error(std::format("test_gaussian_airy_rejects_nonpositive_frequencies failed:\n{}", e.what()));
}
}  // namespace
