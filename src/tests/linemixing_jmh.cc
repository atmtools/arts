    ...
    for (Index Deltal : {0, 1}) {
      ...
      for (Index itemp = 0; itemp < 8; itemp++) {
        ...
        for (Index ipp = 0; ipp < 2; ipp++) {
          ...
          const std::array aa{35.46e-3,
                              19.39e-3,
                              0.0180 * std::pow(T0 / temp, 0.85),
                              0.0168 * std::pow(T0 / temp, 0.5)};
          const std::array alc{0.53, 3.0, 2.2, 2.4};
          const std::array alp{1.062,
                               0.848,
                               0.81 * std::pow(T0 / temp, 0.0152),
                               0.82 * std::pow(T0 / temp, -0.0910)};
          const std::array bbet{0., 0.02, 0.008, 0.007};
          const std::array dx{
              0.0, aa[iPert], alc[iPert], alp[iPert], bbet[iPert]};
