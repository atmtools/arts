...
constexpr Numeric FECS(Numeric OMGA, Numeric EC) noexcept {
  return 1. / (1. + EC * OMGA * OMGA) / (1. + EC * OMGA * OMGA);
}
...
Numeric relaxation_matrix_element(const Index   li,
  ...
  for (Index L = Ldeb; L <= Lfin; L += idl) {
    Numeric       RJ = wigner3j(Rational{jji}, Rational{jjip}, Rational{L}, Rational{li}, Rational{-li}, Rational{0});
    const Numeric SS = RJ * QL[L] / ECS[L];
    RJ               = wigner3j(Rational{jjf}, Rational{jjfp}, Rational{L}, Rational{lf}, Rational{-lf}, Rational{0});
    som += RJ * SS * wigner6j(Rational{jji}, Rational{jjf}, Rational{1}, Rational{jjfp}, Rational{jjip}, Rational{L});
  }
    ...
    for (Index Deltal : {0, 1}) {
      ...
      for (Index itemp = 0; itemp < 8; itemp++) {
        ...
        for (Index ipp = 0; ipp < 2; ipp++) {
          ...
          const std::array aa{
              35.46e-3, 19.39e-3, 0.0180 * std::pow(T0 / temp, 0.85), 0.0168 * std::pow(T0 / temp, 0.5)};
      ...
      for (Index iR = 0; iR <= nraies; iR++) {
        for (Index iRp = 0; iRp <= nraies; iRp++) {
          ...
          char str[200];
          std::sprintf(str,
                       " %0.12E %0.12E %0.12E %0.12E %0.12E %0.12E %0.12E %0.12E   %" PRId64 "   %" PRId64 "   %" PRId64
                       "   %" PRId64 "\n",
                       W[iRp, iR, 0],
                       W[iRp, iR, 1],
                       W[iRp, iR, 2],
