...
void pndFromdNdD(  //WS Output:
  ...
  chk_pndsum(
      pnd, total_content, scatelem_content, 0, 0, 0, "your field", verbosity);
}
...
void pndAdjustFromScatMeta(
  ...
  if (pnd_agenda_input.nrows() != np)
    throw runtime_error(
        "Number of rows in *pnd_data* and *pnd_agenda_input*"
        " must be equal.");
  ...
  for (Index i = 0; i < nse; i++) {
    if (std::isnan(scat_meta[scat_index][i].mass)) {
      ostringstream os;
      os << "No mass data available for scattering element #" << i
  ...
  for (Index ip = 0; ip < np; ip++) {
    ...
    chk_pndsum(pnd,
               pnd_agenda_input(ip, fieldID),
               mass,
               ip,
               -1,
               -1,
               pnd_agenda_input_tag,
               verbosity);
