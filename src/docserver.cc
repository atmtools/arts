...
void Docserver::end_page()
{
  get_os() << "<div class=\"footer\">Page generated by " << ARTS_FULL_VERSION
           << " - <a href=\"" << mbaseurl << "/doccheck\">Check docs</a></div>\n" << endl;
...
/** 
  ...
  \author Oliver Lemke
  */
String Docserver::description_add_links(const String& desc, const String& mname)
{
  string ret;
  string link;
  ...
  if (it != MdRawMap.end())
    {
      ...
      while (indent.length() < mdr.Name().length() + 2)
        indent += ' ';
      ...
      for (Index i = 0; i < mdr.GInType().nelem(); ++i)
        {
          ...
          if (mdr.GIn()[i].length()) param << mdr.GIn()[i];
          else param << "gin" << i;
      ...
      for (Index i = 0; i < mdr.GOut().nelem(); ++i)
        {
          ...
          if (mdr.GOutType()[i] == get_wsv_group_id("Any") && mdr.GOutSpecType()[i].nelem())
            {
              bool firstarg = true;
          ...
          if (!fit)
          ...
          else
            {
              ...
              else buf << description_add_links(desc);
            }
  ...
  if (mi != Workspace::WsvMap.end())
    {
      ...
      get_os() << "<h3>Generic and supergeneric methods that can generate " << vname << "</h3>"
               << endl;
      get_os() << "<ul>" << endl;
      for (Index i = 0; i < md_data_raw.nelem(); ++i)
        {
      ...
      // List generic methods:
      hitcount = 0;
      get_os() << "<h3>Generic and supergeneric methods that can use " << vname << "</h3>"
               << endl;
      get_os() << "<ul>" << endl;
      for (Index i = 0; i < md_data_raw.nelem(); ++i)
        {
          ...
          if (count(mdd.GInType().begin(),
                    mdd.GInType().end(), Workspace::wsv_data[wsv_key].Group()))
            {
              get_os() << "<li>" << insert_wsm_link(mdd.Name()) << endl;
              ++hitcount;
            }
  ...
  if (it != Workspace::WsvMap.end())
    {
      ...
      get_os() << "<p><b>Group: </b>"
               << insert_group_link(wsv_group_names[Workspace::wsv_data[it->second].Group()])
               << endl;
  ...
  if (gid != -1)
    {
      ...
      // List generic methods:
      get_os() << "<h3>Generic and supergeneric methods that can generate " << gname << "</h3>"
               << endl;
      get_os() << "<ul>" << endl;
      for (Index i = 0; i < md_data_raw.nelem(); ++i)
        {
          ...
          else if  (count(mdd.GOutType().begin(), mdd.GOutType().end(), get_wsv_group_id("Any")))
            {
              for (Index j = 0; j < mdd.GOutType().nelem(); j++)
                {
                  if (mdd.GOutType()[j] == get_wsv_group_id("Any"))
      ...
      if (gname != "Any")
        {
          hitcount = 0;
          get_os() << "<h3>Specific methods that require variables of group " << gname << "</h3>"
                   << endl;
          get_os() << "<ul>" << endl;
          for (Index i = 0; i < md_data_raw.nelem(); ++i)
            {
      ...
      for (Index i = 0; i < md_data_raw.nelem(); ++i)
        {
          ...
          else if  (count(mdd.GInType().begin(), mdd.GInType().end(), get_wsv_group_id("Any")))
            {
              for (Index j = 0; j < mdd.GInType().nelem(); j++)
                {
                  if (mdd.GInType()[j] == get_wsv_group_id("Any"))
...
void Docserver::insert_breadcrumb_token(size_t token_id)
{
  ...
  else if (tokens[token_id-1] == "methods") get_os() << "Methods";
  else if (tokens[token_id-1] == "variables") get_os() << "Variables";
  else if (tokens[token_id-1] == "agendas") get_os() << "Agendas";
  else if (tokens[token_id-1] == "groups") get_os() << "Groups";
  else if (tokens[token_id-1] == "all") get_os() << "All";
  else if (tokens[token_id-1] == "doccheck") get_os() << "Doc Check";
  else get_os() << tokens[token_id-1];
...
void Docserver::insert_broken_doc_links()
{
  ...
  for (Index i = 0; i < Workspace::wsv_data.nelem(); ++i)
    {
      ...
      broken_links = find_broken_description_links(Workspace::wsv_data[i].Description());
  ...
  for (Array<MdRecord>::const_iterator mit = md_data_raw.begin();
    ...
    {
      vector<string> broken_links;
      broken_links = find_broken_description_links(mit->Description(), mit->Name());
...
/** 
  ...
  \author Oliver Lemke
  */
vector<string> Docserver::find_broken_description_links(const String& desc, const String& mname)
{
  vector<string> broken_links;
  string ret;
  string link;
          ...
          if (*it == '*')
            {
              ...
              if (!found)
                broken_links.push_back("<span class=\"brokendoclink\">*" + link + "*</span>");
