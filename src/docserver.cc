...
void Docserver::end_page() {
  get_os() << "<div class=\"footer\">Page generated by "
           << arts_get_version_string() << "</div>\n"
           << endl;
...
void Docserver::list_groups() {
  ...
  get_os() << "<div class=\"firstcol\">" << endl << "<ul>" << endl;
  for (i = 0; i < wsv_groups.nelem(); ++i) {
    get_os() << "<li>" << insert_group_link(wsv_groups[i].name) << "</li>"
             << endl;

    if (i + 1 == wsv_groups.nelem() / 2)
      get_os() << "</ul>" << endl
               << "</div>" << endl
               << "<div class=\"secondcol\">" << endl
               << "<ul>" << endl;
    ...
    for (Index i = 0; i < mdr.GOut().nelem(); ++i) {
      ...
      if (mdr.GOutType()[i] == get_wsv_group_id("Any") &&
        ...
        for (Index j = 0; j < mdr.GOutSpecType()[i].nelem(); j++) {
          ...
          buf << insert_group_link(wsv_groups[mdr.GOutSpecType()[i][j]].name);
        }
      } else {
        buf << insert_group_link(wsv_groups[mdr.GOutType()[i]].name);
      }
      ...
      if (mdr.GInType()[i] == get_wsv_group_id("Any") &&
        ...
        for (Index j = 0; j < mdr.GInSpecType()[i].nelem(); j++) {
          if (!firstarg)
            ...
            firstarg = false;
          buf << insert_group_link(wsv_groups[mdr.GInSpecType()[i][j]].name);
        }
      } else {
        buf << insert_group_link(wsv_groups[mdr.GInType()[i]].name);
      }
...
/** 
  Returns a list of all ** links pointing to non-existent WSVs, WSMs, Agendas
  or groups.
 
  \author Oliver Lemke
  */
std::tuple<size_t, std::vector<string> >
Docserver::list_broken_description_links() {
  size_t counter = 0;
  vector<string> all_broken_links;

  auto insert_broken = [](vector<string> brk_links) {
    ostringstream os;
    bool first_broken = true;
    for (auto&& broken_link : brk_links) {
      if (!first_broken)
        os << ", ";
      else
        first_broken = false;
  ...
  for (auto&& ait : agenda_data) {
    ...
    if (broken_links.size()) {
      if (first) {
        first = false;
        all_broken_links.push_back("");
        all_broken_links.push_back("Agenda descriptions");
        all_broken_links.push_back("-------------------");
      }
      all_broken_links.push_back(ait.Name() + ": " +
                                 insert_broken(broken_links));
    }
  }
  ...
  for (auto&& mit : md_data_raw) {
    ...
    if (broken_links.size()) {
      if (first) {
        first = false;
        all_broken_links.push_back("");
        all_broken_links.push_back("WSM descriptions");
        all_broken_links.push_back("----------------");
      }
      all_broken_links.push_back(mit.Name() + ": " +
                                 insert_broken(broken_links));
    }
  }

  return std::make_tuple(counter, all_broken_links);
}
