...
spectral_nlte_srcvec_jac_profile size {}
  ...
  const std::vector<Size> ips{
      std::from_range,
      ray_path | stdv::transform([&](const PropagationPathPoint& ray_point)
                                     -> Size {
        const Size ip =
            stdr::distance(alt_grid.begin(),
                           stdr::lower_bound(alt_grid, ray_point.altitude()));
        ...
        ARTS_USER_ERROR_IF(
            alt_grid[ip] != ray_point.altitude(),
...
void ProfileFromAltitude(const Workspace& ws,
  ...
  const ArrayOfVector3 freq_wind_shift_jac_path(ray_path.size(),
                                                Vector3{0, 0, 0});
