...
void apply_options(ArrayOfPropagationPathPoint& rad_path,
  ...
  if (static_cast<bool>(add_limb))
    path::fill_geometric_limb(rad_path, surface_field);

  if (static_cast<bool>(remove_non_atm)) path::keep_only_atm(rad_path);
}
...
void rad_pathGeometric(ArrayOfPropagationPathPoint& rad_path,
  ...
  apply_options(rad_path, surface_field, add_limb, remove_non_atm);
}
...
void rad_pathGeometricTangentAltitude(ArrayOfPropagationPathPoint& rad_path,
  ...
  ARTS_USER_ERROR_IF(
      ...
      "Path extremes contains surface point(s):\n",
      rad_path);
