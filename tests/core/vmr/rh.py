...
if PLOT:
    ...
    plt.show()
    plt.semilogx(
        ws.model_state_vec_apriori,
        field.data.grids[0],
        ":",
        lw=3,
        label="apriori",
    )
    plt.plot(ws.model_state_vec, field.data.grids[0], label="fitted")
    plt.plot(orig, field.data.grids[0], label="true")
    plt.legend()
    plt.show()
