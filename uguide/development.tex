...
most GNU packages. The top-level ARTS directory (simply called \verb|arts|)
contains various sub-directories, notably \verb|doc| for
documentation, \verb|src| for the C++ source code, and \verb|data| for
auxiliary data, such as model atmospheres. This document, the ARTS
User Guide, is at the moment located in \verb|arts/uguide|, but will
be moved to \verb|arts/doc/uguide|. FIXME: Update this after \verb|uguide|
has been moved.

In the future, there will be two different versions of the ARTS
package: The developers version and the end-user version. Both will
contain the complete source code, the only difference is that the
developers version also includes the CVS housekeeping data. If you
want to join in the ARTS development (which we of course encourage you
to do), you should write an email to the authors to obtain access to
the developers version, which makes it easier to merge your changes
with the `official' ARTS program. Furthermore, for serious development
work you need a computer running Unix, the GNU development tools,
LaTeX, and the DOC++ program. All this is freely and easily available
on the Internet.
